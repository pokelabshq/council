...
await build({
  ...
  // Skip the prebuilt @council/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@council/ink': resolve(root, 'packages/council-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The council_cli launcher
