import type { RunExternalProcess } from '@council/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`council ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching council: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`council ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
