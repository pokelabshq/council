...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.COUNCIL_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Council on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via COUNCIL_TUI_TRUECOLOR=1.
