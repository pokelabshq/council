 ...
 * (`openCommand` returned null), or (c) `spawn()` threw synchropokely
...
export function openExternalUrl(rawUrl: string, dependencies: OpenDependencies = {}): boolean {
  ...
  try {
    ...
    // Async failure path: spawn returns a ChildProcess synchropokely even
    ...
    // consumer; we already returned `true` synchropokely, so the user
    ...
    // spawn can also throw synchropokely on argv-validation failures
