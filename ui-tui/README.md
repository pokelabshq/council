...
React + Ink terminal UI for Council. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `COUNCIL_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.council/.council_history` or under `COUNCIL_HOME`.
...
```text
ui-tui/
  packages/council-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      council-ink.d.ts    type declarations for @council/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent CouncilCLI subprocess for slash commands
```
