...
Usage (via cron with --no-agent):
    ...
    council cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$COUNCIL_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Council-Watcher/1.0"})
