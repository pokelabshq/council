...
Usage (via cron with --no-agent):
    ...
    council cron create council-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$COUNCIL_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name council-issues --repo pokelabshq/council --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.council/.env to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Council-Watcher/1.0",
    }
