...
$COUNCIL_OSINT_CACHE/icij/ (default: ~/.cache/council-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("COUNCIL_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "council-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "pokelabs-council osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $COUNCIL_OSINT_CACHE/icij or ~/.cache/council-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
