#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `council kanban list` and
...
def council_available() -> bool:
    return shutil.which("council") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed council CLI."""
    try:
        out = subprocess.run(
            ["council", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `council kanban list`
    out = subprocess.run(
        ["council", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["council", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not council_available():
        print("ERROR: 'council' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
