...
Maps each role archetype to the Council skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public pokelabs-council
...
## Council skills relevant to video production

### Visual / rendering skills (`pokelabs-council/skills/creative/`)
...
### Design / pre-production skills (`pokelabs-council/skills/creative/`)
...
### Audio / media skills (`pokelabs-council/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`pokelabs-council/skills/devops/`)
...
These are **not** Council skills but external CLIs / APIs that profiles invoke.
...
## Built-in Council tools for media review

These are native Council tools — not invoked via terminal but through their own
...
key is present in `~/.council/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Council profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.council/.env` → environment variable.
...
When a new Council-public video skill ships:
