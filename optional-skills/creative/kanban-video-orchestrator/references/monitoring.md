...
council kanban watch --tenant <project-slug>

# Snapshot of the board
council kanban list --tenant <project-slug>
council kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
council kanban stats --tenant <project-slug>

# Visual dashboard (browser)
council dashboard

# Inspect a specific task (includes comments + events)
council kanban show <task-id>

# Follow a single task's event stream
council kanban tail <task-id>
```

Verify available subcommands with `council kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `council kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `council kanban block <id>` then `council kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `council kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `council kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `council profile list` (profile exists?), `council status` (gateway/dispatcher up?), and verify tenant. |
...
council kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
council kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(council kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
council kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
council kanban block <task-id>
council kanban archive <task-id>

# Diagnose what happened
council kanban show <task-id>      # task body, comments, recent events
council kanban tail <task-id>      # follow the live event stream
council kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `council kanban create` for the director
...
while true; do
    clear
    council kanban list --tenant <slug>
    echo "---"
    council kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `council kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `council profile list` and start
