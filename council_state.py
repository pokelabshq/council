#!/usr/bin/env python3
"""
SQLite State Store for Poke Council.
...
DEFAULT_DB_PATH = get_council_home() / "state.db"
...
# council_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``council_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see council_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple council processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``council_state._set_last_init_error(None)`` explicitly.
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    ...
                    "disabled. This usually means Council is running on an "
    ...
    def get_messages_as_conversation(
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Poke) receive
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Council, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Council session.

        A Council session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Council session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``COUNCIL_DISABLE_FTS_TRIGRAM`` or not yet created), so
