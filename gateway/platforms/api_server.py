...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Council-Session-Id header; opt-in long-term memory scoping via X-Council-Session-Key header)
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_council_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted COUNCIL_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # council_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same Council session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Council-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Council-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Council-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Council-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Council-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``council sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the council-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Council-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Council version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Council AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Council-Session-Id",
                "session_key_header": "X-Council-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Council sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty Council session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({"object": "council.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "council.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "council.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "council.session", "session": self._session_response(fork)}, status=201)

    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions/{session_id}/chat — one synchropoke agent turn."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        headers = {"X-Council-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Council-Session-Key"] = gateway_session_key
        return web.json_response(
            {
                "object": "council.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Council-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Council-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Council-Session-Key.  This
        # is independent of X-Council-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Council-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Council-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Council-Session-Id rejected: "
            ...
            # conversation map to the same Council session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``council.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Council-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Council-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["council"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Council-Completed"] = "false"
            response_headers["X-Council-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Council-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Council-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Council-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Council-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Council-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: council.tool.progress`` SSE event so
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Council-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Council-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            # (e.g. X-Council-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Council-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "council.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Council-Relay
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from council_cli.auth import has_usable_secret
