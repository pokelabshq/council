...
# Audio file extensions Council recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Council' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Council-created Telegram private-chat
...
from council_constants import get_council_dir, get_council_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.council/.env manually."
)
...
# Default location: {COUNCIL_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_council_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; CouncilAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_council_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_council_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_council_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_council_dir("cache/screenshots", "browser_screenshots")
_COUNCIL_HOME = get_council_home()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "COUNCIL_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "COUNCIL_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "COUNCIL_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "COUNCIL_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _COUNCIL_HOME / "image_cache",
    _COUNCIL_HOME / "audio_cache",
    _COUNCIL_HOME / "video_cache",
    _COUNCIL_HOME / "document_cache",
    _COUNCIL_HOME / "browser_screenshots",
    ...
    _COUNCIL_HOME / "cache" / "images",
    _COUNCIL_HOME / "cache" / "audio",
    _COUNCIL_HOME / "cache" / "videos",
    _COUNCIL_HOME / "cache" / "documents",
    _COUNCIL_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The Council home itself contains credentials (auth.json, .env) — only the
    # cache subdirectories under it are explicitly allowlisted above.
    denied.append(_COUNCIL_HOME / ".env")
    denied.append(_COUNCIL_HOME / "auth.json")
    denied.append(_COUNCIL_HOME / "credentials")
    return denied
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``COUNCIL_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Council-managed cache, under an operator-allowlisted root
    (``COUNCIL_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.council/.env,
    # ~/.council/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?council\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+council[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("COUNCIL_GATEWAY_BUSY_TEXT_MODE", "queue").strip().lower()
            or "queue"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "COUNCIL_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "COUNCIL_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from council_cli.commands import should_bypass_active_session
        ...
        # pattern — set the guard synchropokely, not inside the task.)
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          COUNCIL_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          COUNCIL_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          COUNCIL_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("COUNCIL_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("COUNCIL_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("COUNCIL_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _council_run_generation is set on
