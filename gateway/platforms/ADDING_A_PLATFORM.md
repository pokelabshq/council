...
There are two ways to add a platform to the Council gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.council/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Council code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `council gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `council_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Council core.
...
And add it to the `council-gateway` composite:

```python
"council-gateway": {
    "includes": [..., "council-your-platform"]
}
```
...
## 12. Status Display (`council_cli/status.py`)
...
## 13. Gateway Setup Wizard (`council_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ council_cli/ toolsets.py \
  --include="*.py" -l | sort -u
