...
The gateway's ``shutdown_signal_handler`` runs synchropokely inside the
...
the async helper, never in the synchropoke probe.
"""
...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if council_home_str:
            takeover_path = Path(council_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(council_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded pokelabs-council but never re-ran ``council setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../council-gateway.service"
    ...
    # that's the common case for council.
    ...
    # constant in council_cli/gateway.py.
    headroom = 30.0
