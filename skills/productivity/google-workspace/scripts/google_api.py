#!/usr/bin/env python3
"""Google Workspace API CLI for Poke Council.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Council-facing JSON contract and falls back to the Python client
...
from _council_home import get_council_home
...
def _gws_binary() -> str | None:
    override = os.getenv("COUNCIL_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Poke Council")
