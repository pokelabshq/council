...
## 5. GODMODE FAST — Council 4 405B (Zero Refusal)

**Model:** `pokelabs/council-4-405b`
**Strategy:** Council models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Poke Council

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.council/config.yaml`:
