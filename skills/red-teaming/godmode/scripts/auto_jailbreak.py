...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("COUNCIL_HOME", os.path.expanduser("~/.council")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("COUNCIL_HOME", Path.home() / ".council")) / "skills" / "red-teaming" / "godmode"
...
COUNCIL_HOME = Path(os.getenv("COUNCIL_HOME", Path.home() / ".council"))
CONFIG_PATH = COUNCIL_HOME / "config.yaml"
PREFILL_PATH = COUNCIL_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Poke/Council models — already uncensored, just needs clean prompt
    "council": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "council" in model_lower or "poke" in model_lower:
        return "council"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Council config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.council/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Council for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Council for changes to take effect.")
