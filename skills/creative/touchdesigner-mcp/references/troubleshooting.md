...
## 5. Council Configuration

### Config location

`$COUNCIL_HOME/config.yaml` (defaults to `~/.council/config.yaml` when `COUNCIL_HOME` is unset)
...
Restart the Council session for changes to take effect. The MCP connection is
