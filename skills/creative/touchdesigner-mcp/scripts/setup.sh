...
COUNCIL_HOME_DIR="${COUNCIL_HOME:-$HOME/.council}"
COUNCIL_CFG="${COUNCIL_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Council config has twozero_td MCP entry ──
if [[ ! -f "$COUNCIL_CFG" ]]; then
    echo -e " ${FAIL} Council config not found at ${COUNCIL_CFG}"
    manual_steps+=("Create ${COUNCIL_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$COUNCIL_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Council config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Council config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$COUNCIL_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${COUNCIL_CFG} manually"); }
    manual_steps+=("Restart Council session to pick up config change")
fi
