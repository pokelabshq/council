...
_COUNCIL_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (COUNCIL_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `council tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``council tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (COUNCIL_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Council toolsets (CLI + messaging platforms)
    #
    ...
    "council-cli": {
        ...
        "tools": _COUNCIL_CORE_TOOLS,
        "includes": []
    },

    "council-cron": {
        # Mirrors council-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `council tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as council-cli; gated by `council tools`",
        "tools": _COUNCIL_CORE_TOOLS,
        "includes": []
    },
    ...
    "council-email": {
        "description": "Email bot toolset - interact with Council via email (IMAP/SMTP)",
        "tools": _COUNCIL_CORE_TOOLS,
        "includes": []
    },
    ...
    "council-sms": {
        "description": "SMS bot toolset - interact with Council via SMS (Twilio)",
        "tools": _COUNCIL_CORE_TOOLS,
        "includes": []
    },
    ...
    "council-webhook": {
        ...
        "tools": _COUNCIL_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "council-gateway": {
        ...
        "tools": [],
        "includes": ["council-telegram", "council-discord", "council-whatsapp", "council-slack", "council-signal", "council-bluebubbles", "council-homeassistant", "council-email", "council-sms", "council-mattermost", "council-matrix", "council-dingtalk", "council-feishu", "council-wecom", "council-wecom-callback", "council-weixin", "council-qqbot", "council-webhook", "council-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (council-<name>).
        # Gives them _COUNCIL_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("council-"):
            platform_name = name[len("council-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_COUNCIL_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
