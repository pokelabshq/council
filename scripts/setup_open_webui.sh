...
# Bootstrap Open WebUI against Poke Council's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.council/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-council.sh
...
#   OPEN_WEBUI_NAME='Johnny Council'
...
#   COUNCIL_API_PORT=8642
#   COUNCIL_API_HOST=127.0.0.1
#   COUNCIL_API_MODEL_NAME='Poke Council'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Poke Council WebUI}"
...
COUNCIL_ENV_FILE="${COUNCIL_ENV_FILE:-$HOME/.council/.env}"
COUNCIL_API_PORT="${COUNCIL_API_PORT:-8642}"
COUNCIL_API_HOST="${COUNCIL_API_HOST:-127.0.0.1}"
COUNCIL_API_CONNECT_HOST="${COUNCIL_API_CONNECT_HOST:-127.0.0.1}"
COUNCIL_API_MODEL_NAME="${COUNCIL_API_MODEL_NAME:-Poke Council}"
COUNCIL_API_BASE_URL="http://${COUNCIL_API_CONNECT_HOST}:${COUNCIL_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-council.sh"
LOG_DIR="$HOME/.council/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$COUNCIL_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$COUNCIL_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.council.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.council</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.council"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.council"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-council.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Poke Council
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-council.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.council/logs/openwebui.log
StandardError=append:%h/.council/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-council.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$COUNCIL_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Council API server is configured...'
  upsert_env API_SERVER_ENABLED true "$COUNCIL_ENV_FILE"
  upsert_env API_SERVER_HOST "$COUNCIL_API_HOST" "$COUNCIL_ENV_FILE"
  upsert_env API_SERVER_PORT "$COUNCIL_API_PORT" "$COUNCIL_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$COUNCIL_API_MODEL_NAME" "$COUNCIL_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$COUNCIL_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Council gateway so API server settings take effect...'
  council gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${COUNCIL_API_CONNECT_HOST}:${COUNCIL_API_PORT}/health" >/dev/null; then
    log 'Council API server did not answer on the first check. Trying to start gateway in the background...'
    nohup council gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${COUNCIL_API_CONNECT_HOST}:${COUNCIL_API_PORT}/health" >/dev/null
  ...
  log "Council API endpoint: ${COUNCIL_API_BASE_URL}"
