...
# Python-driven tool session (e.g. Council terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:pokelabshq/council.git"
REPO_URL_HTTPS="https://github.com/pokelabshq/council.git"
COUNCIL_HOME="${COUNCIL_HOME:-$HOME/.council}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${COUNCIL_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$COUNCIL_INSTALL_DIR"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   Installs optional deps + runs council setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Poke Council Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Poke Labs.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# Decide where the repo checkout + venv live, and where the `council` command
...
resolve_install_layout() {
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        ...
        # /usr/local/bin/council wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/council"
        log_info "  Data:    $COUNCIL_HOME (unchanged)"
...
get_council_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/council" ]; then
        echo "$link_dir/council"
    else
        echo "council"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/pokelabshq/council/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/council-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/council-uv-installer.$$.sh")"
...
# module compiled in. Council' session store (council_state.py) creates FTS5
...
ensure_fts5() {
    ...
    if [ -n "${PYTHON_PATH:-}" ] && _python_has_fts5 "$PYTHON_PATH"; then
        ...
        # etc.). Install proceeds — Council degrades gracefully and disables
        # only full-text session search — but warn so it isn't a silent gap.
        log_warn "Could not obtain an FTS5-capable Python. Council will run, but"
...
check_node() {
    ...
    if [ -x "$COUNCIL_HOME/node/bin/node" ]; then
        export PATH="$COUNCIL_HOME/node/bin:$PATH"
        local found_ver=$("$COUNCIL_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Council-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.council/node/..."
    ...
    # Place into ~/.council/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$COUNCIL_HOME/node"
    mkdir -p "$COUNCIL_HOME"
    mv "$extracted_dir" "$COUNCIL_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$COUNCIL_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$COUNCIL_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$COUNCIL_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$COUNCIL_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$COUNCIL_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.council/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Council install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Poke Council itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="council-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Council behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up council command..."

    if [ "$USE_VENV" = true ]; then
        COUNCIL_BIN="$INSTALL_DIR/venv/bin/council"
    else
        COUNCIL_BIN="$(which council 2>/dev/null || echo "")"
        if [ -z "$COUNCIL_BIN" ]; then
            log_warn "council not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$COUNCIL_BIN" ]; then
        log_warn "council entry point not found at $COUNCIL_BIN"
    ...
    # Create a user-facing shim for the council command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $COUNCIL_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$COUNCIL_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/council"
    cat > "$command_link_dir/council" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$COUNCIL_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/council"
    log_success "Installed council launcher → $command_link_display_dir/council"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "council command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v council' \
                >/dev/null 2>&1; then
        ...
        log_info "council not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Poke Council — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Poke Council — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Poke Council — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so council works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.council directory structure (config at top level, code in subdir)
    mkdir -p "$COUNCIL_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.council/.env (top level, easy to find)
    if [ ! -f "$COUNCIL_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$COUNCIL_HOME/.env"
            log_success "Created ~/.council/.env from template"
        else
            touch "$COUNCIL_HOME/.env"
            log_success "Created ~/.council/.env"
        fi
    else
        log_info "~/.council/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$COUNCIL_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.council/config.yaml (top level, easy to find)
    if [ ! -f "$COUNCIL_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$COUNCIL_HOME/config.yaml"
            log_success "Created ~/.council/config.yaml from template"
        fi
    else
        log_info "~/.council/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$COUNCIL_HOME/SOUL.md" ]; then
        cat > "$COUNCIL_HOME/SOUL.md" << 'SOUL_EOF'
...
-->
SOUL_EOF
        log_success "Created ~/.council/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.council/"

    # Seed bundled skills into ~/.council/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.council/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.council/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$COUNCIL_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$COUNCIL_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.council/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$COUNCIL_HOME/.env"
    ...
    {
        echo ""
        echo "# Poke Council browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Council will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (council --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'council setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m council_cli.main setup < /dev/tty
    else
        python -m council_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$COUNCIL_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Council to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$COUNCIL_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'council whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                COUNCIL_CMD="$(get_council_command_path)"
                $COUNCIL_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'council whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'council gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $COUNCIL_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $COUNCIL_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: council gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: council gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $COUNCIL_CMD gateway > "$COUNCIL_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.council/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: council gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: council gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $COUNCIL_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $COUNCIL_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $COUNCIL_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}council${NC}              Start chatting"
    echo -e "   ${GREEN}council setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}council config${NC}       View/edit configuration"
    echo -e "   ${GREEN}council config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}council gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}council update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'council' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'council' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'council' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$COUNCIL_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$COUNCIL_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$COUNCIL_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$COUNCIL_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$COUNCIL_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Council for pip install"
    ...
    COUNCIL_CMD="$(command -v council 2>/dev/null || echo "")"
    if [ -n "$COUNCIL_CMD" ]; then
        log_info "Running council setup..."
        "$COUNCIL_CMD" setup
    else
        log_warn "council command not found on PATH"
        log_info "Try: python -m council_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$COUNCIL_HOME/.install_method"
}
