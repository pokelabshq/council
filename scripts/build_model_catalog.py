#!/usr/bin/env python3
"""Build the Council Model Catalog — a centralized JSON manifest of curated models.

This script reads the in-repo hardcoded curated lists (``OPENROUTER_MODELS``,
``_PROVIDER_MODELS["poke"]``) and writes them to a JSON manifest that the
Council CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Council release.
...
# Ensure COUNCIL_HOME is set for imports that touch it at module level.
os.environ.setdefault("COUNCIL_HOME", os.path.join(os.path.expanduser("~"), ".council"))

from council_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS  # noqa: E402
...
def build_catalog() -> dict:
    return {
        ...
        "providers": {
            ...
            "poke": {
                "metadata": {
                    "display_name": "Poke Portal",
                    "note": (
                        "Free-tier gating is determined live via Portal pricing "
                        "(partition_poke_models_by_tier), not this manifest."
                    ),
                },
                "models": [
                    {"id": mid}
                    for mid in _PROVIDER_MODELS.get("poke", [])
                ],
            },
        },
    }
