# ============================================================================
# Poke Council Installer for Windows
# ============================================================================
...
# 5.1's progress UI repaints synchropokely on every received byte, which
...
$RepoUrlSsh = "git@github.com:pokelabshq/council.git"
$RepoUrlHttps = "https://github.com/pokelabshq/council.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Poke Council Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Poke Labs.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $CouncilHome)) {
        New-Item -ItemType Directory -Force -Path $CouncilHome | Out-Null
    }
    $envFile = Join-Path $CouncilHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $CouncilHome "node"
...
function Install-Git {
    ...
    which Council uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\council\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Council needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Council copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\council\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``COUNCIL_GIT_BASH_PATH`` (User scope) so Council can find it in a fresh
    ...
    # Download PortableGit into $CouncilHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $CouncilHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Council features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Council needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``COUNCIL_GIT_BASH_PATH`` (User env scope) so Council can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $CouncilHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $CouncilHome\git\usr\bin\bash.exe
    $candidates += "$CouncilHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$CouncilHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("COUNCIL_GIT_BASH_PATH", $candidate, "User")
            $env:COUNCIL_GIT_BASH_PATH = $candidate
            Write-Info "Set COUNCIL_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Council may not find Git Bash."
    Write-Info "If needed, set COUNCIL_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$CouncilHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$CouncilHome\node;$env:Path"
        Write-Success "Node.js $version found (Council-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $CouncilHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $CouncilHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\council-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$CouncilHome\node") { Remove-Item -Recurse -Force "$CouncilHome\node" }
                Move-Item $extractedDir.FullName "$CouncilHome\node"
                ...
                $version = & "$CouncilHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $CouncilHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running council processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/pokelabshq/council/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/pokelabshq/council/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `council.exe` exists
...
"@ 2>$null
    ...
    # isn't set, leaving venv\ empty and council.exe broken with
    ...
    # here, not 30 seconds later when the user runs `council`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `council dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `council dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `council dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up council command..."
    
    if ($NoVenv) {
        $councilBin = "$InstallDir"
    } else {
        $councilBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so council is globally available
    # On Windows, the council.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$councilBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$councilBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $councilBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set COUNCIL_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\council instead
    # of the Unix default ~/.council
    $currentCouncilHome = [Environment]::GetEnvironmentVariable("COUNCIL_HOME", "User")
    if (-not $currentCouncilHome -or $currentCouncilHome -ne $CouncilHome) {
        [Environment]::SetEnvironmentVariable("COUNCIL_HOME", $CouncilHome, "User")
        Write-Success "Set COUNCIL_HOME=$CouncilHome"
    }
    $env:COUNCIL_HOME = $CouncilHome
    
    # Update current session
    $env:Path = "$councilBin;$env:Path"
    
    Write-Success "council command ready"
}
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$CouncilHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CouncilHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CouncilHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CouncilHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CouncilHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CouncilHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CouncilHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CouncilHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CouncilHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$CouncilHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.council/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.council/.env"
        }
    } else {
        Write-Info "~/.council/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$CouncilHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.council/config.yaml from template"
        }
    } else {
        Write-Info "~/.council/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Council's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$CouncilHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        $soulContent = @"
# Poke Council Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Council communicates with you.
...
"@
        ...
        Write-Success "Created ~/.council/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.council/"
    
    # Seed bundled skills into ~/.council/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.council/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.council/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$CouncilHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.council/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'council setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\council-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\council-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\council-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.council/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'council setup'."
        return
    }
    ...
    # Run council setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m council_cli.main setup
    } else {
        python -m council_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $councilCmd = "$InstallDir\venv\Scripts\council.exe"
    if (-not (Test-Path $councilCmd)) {
        $councilCmd = "council"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$CouncilHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'council whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: council gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$CouncilHome\logs\gateway.log"
            Start-Process -FilePath $councilCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$CouncilHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: council gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: council gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$CouncilHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$CouncilHome\.env"
    ...
    Write-Host ""
    Write-Host "   council              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   council setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   council config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   council config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   council gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   council update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Council repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Council to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/pokelabshq/council/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
