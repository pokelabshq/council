#!/usr/bin/env python3
"""Live test harness for Poke Council's Tool Search feature.
...
# Force-isolate the test environment BEFORE any council imports.
ORIGINAL_HOME = os.environ.get("COUNCIL_HOME")
ORIGINAL_AUTH = Path.home() / ".council" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.council/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.council/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="council_ts_live_"))
    council_home = home_dir / ".council"
    council_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, council_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".council" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, council_home / ".env")
        ...
        load_council_dotenv(council_home=str(Path.home() / ".council"))
    ...
    (council_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return council_home
...
def reset_module_state():
    """Drop cached modules so the new COUNCIL_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "council_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["COUNCIL_HOME"] = str(home)
...
def main():
    ...
    # Restore original COUNCIL_HOME
    if ORIGINAL_HOME is not None:
        os.environ["COUNCIL_HOME"] = ORIGINAL_HOME
    else:
