...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$COUNCIL_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$COUNCIL_HOME/plugins/model-providers/`` if it exists."""
    try:
        from council_constants import get_council_home

        d = get_council_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple COUNCIL_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_council_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$COUNCIL_HOME/plugins/model-providers/<name>/``
    ...
    # 2. User plugins — under $COUNCIL_HOME/plugins/model-providers/<name>/.
