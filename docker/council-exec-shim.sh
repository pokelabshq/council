#!/bin/sh
# shellcheck shell=sh
# /opt/council/bin/council — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `council` user (UID 10000). When an operator runs `docker exec <c> council ...`
# the default UID is root (0), and any file the command writes under
# $COUNCIL_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> council login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Council is not logged into Poke
# Portal" on every incoming message — even though `docker exec <c> council
...
# Fix
# ---
# This shim sits at /opt/council/bin/council and is placed earliest on PATH.
# When invoked as root, it drops to the council user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $COUNCIL_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user council`, kanban subagents, etc. — it short-circuits
...
# (/opt/council/.venv/bin/council), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set COUNCIL_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the council CLI. Default is to drop.

set -e

REAL=/opt/council/.venv/bin/council

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "council-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user council`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${COUNCIL_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the council user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "council-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "council-shim: re-run with --user council or set COUNCIL_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the council user's home before dropping privileges. Without
