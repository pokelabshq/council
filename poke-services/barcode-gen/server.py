...
def encode_ean13(text):
    ...
    for i, p in enumerate(parity):
        d = int(text[i + 1])
        bars += EAN13_LEFT_ODD[d] if p == 'L' else EAN13_LEFT_EVEN[d]
    bars += '01010'
    for i in range(6):
        bars += EAN13_RIGHT[int(text[i + 7])]
    bars += '101'
    return bars, None
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.path == '/api/health':
            self._respond(200, {'ok': True, 'service': 'barcode', 'v': 1, 'formats': ['code39', 'code128', 'ean13']})
