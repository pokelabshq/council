"""Tests for the ``council send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``council_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from council_cli import send_cmd
...
def test_load_council_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``council send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``council config set``) lives
    ...
    council_home.mkdir()
    (council_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_council_home() to re-resolve under the patched env.
    from importlib import reload

    import council_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_council_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_council_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    council_home = tmp_path / ".council"
    council_home.mkdir()
    (council_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_council_env_handles_missing_files(tmp_path, monkeypatch):
