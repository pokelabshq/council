...
from council_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/tmp/council-launcher-venv")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")

        hm._cmd_update_pip(mock_args)

        assert mock_run.call_count == 1
        assert mock_run.call_args.args[0] == ["/usr/bin/uv", "pip", "install", "--upgrade", "ai-council"]
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/tmp/council-launcher-venv"
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind pokelabshq/council silently misses updates.
        """
        from council_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/ai-council.git",
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "council_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "council_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("council_cli.config.check_config_version", return_value=(1, 2)), patch(
            "council_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("council_cli.main.sys") as mock_sys:
            ...
            from council_cli.config import migrate_config
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.council"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.council/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.council/profiles/work"))
        ...
        with (
            patch("council_cli.profiles.list_profiles", return_value=all_profiles),
            patch("council_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("council_cli.profiles.list_profiles", return_value=[default_p]),
            patch("council_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``council update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``council update --check --branch <name>`` honors the branch override.
    ...
    @patch("council_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
    ...
    @patch("council_cli.config.detect_install_method", return_value="pip")
    @patch("council_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``council update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from council_cli.main import _update_via_zip
