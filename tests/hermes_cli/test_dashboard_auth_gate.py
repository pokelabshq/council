...
def test_loopback_protected_route_accepts_session_token(client_loopback):
    ...
    r = client_loopback.get(
        "/api/sessions",
        headers={"X-Council-Session-Token": web_server._SESSION_TOKEN},
    )
...
def test_loopback_index_injects_session_token(client_loopback):
    ...
    assert "__COUNCIL_SESSION_TOKEN__" in r.text
...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    ("ai-council-prod-abc.fly.dev", False, True),
])
def test_should_require_auth_truth_table(host, allow_public, expected):
    from council_cli.web_server import should_require_auth
...
def test_start_server_public_without_insecure_records_auth_required(monkeypatch):
    ...
    """
    from council_cli.dashboard_auth import clear_providers
    clear_providers()
    _stub_uvicorn_run(monkeypatch)
    web_server.app.state.auth_required = None
...
def test_start_server_surfaces_poke_skip_reason_when_unconfigured(monkeypatch):
    """When the bundled Poke plugin loaded but skipped registration (no
    ...
    'set COUNCIL_DASHBOARD_OAUTH_CLIENT_ID', not 'install a plugin'."""
    from council_cli.dashboard_auth import clear_providers
    from plugins.dashboard_auth import poke as poke_plugin
    ...
    poke_plugin.register(MagicMock())  # populates LAST_SKIP_REASON
    ...
    # than the generic "Install the default Poke provider" boilerplate.
    msg = str(exc_info.value)
    assert "COUNCIL_DASHBOARD_OAUTH_CLIENT_ID" in msg
    assert "poke:" in msg
