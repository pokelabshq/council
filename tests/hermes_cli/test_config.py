"""Tests for council_cli configuration management."""
...
class TestGetCouncilHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("COUNCIL_HOME", None)
            home = get_council_home()
            assert home == Path.home() / ".council"

    def test_env_override(self):
        with patch.dict(os.environ, {"COUNCIL_HOME": "/custom/path"}):
            home = get_council_home()
            assert home == Path("/custom/path")


class TestEnsureCouncilHome:
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``council logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from council_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"COUNCIL_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="council_cli.config"):
                config = load_config()
            ...
            # stands out at council startup before logging is configured)
            captured = capsys.readouterr()
            assert "council config:" in captured.err
    ...
    def test_dedup_on_repeated_load_same_file(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"COUNCIL_HOME": str(tmp_path)}):
            ...
            assert "council config:" in first
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"COUNCIL_HOME": str(tmp_path)}):
            ...
            assert "council config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"COUNCIL_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"COUNCIL_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"COUNCIL_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"COUNCIL_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"COUNCIL_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from council_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``COUNCIL_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Council
    process on next startup via the dotenv → ``os.environ`` chain in
    ``council_cli/env_loader.py``.
    ...
    def _council_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "COUNCIL_SPOTIFY_CLIENT_ID",
            "COUNCIL_QWEN_BASE_URL",
            "COUNCIL_MAX_ITERATIONS",
        ],
    )
    def test_council_integration_keys_still_writable(self, allowed_key):
        """``COUNCIL_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``COUNCIL_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``COUNCIL_*``) keep working. The denylist
