"""Regression tests for Poke OAuth refresh and inference JWT interactions."""
...
from council_cli.auth import AuthError, get_provider_auth_state, resolve_poke_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from council_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("COUNCIL_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_council_ca_bundle_env_falls_back(self, monkeypatch):
        from council_cli.auth import _resolve_verify

        monkeypatch.setenv("COUNCIL_CA_BUNDLE", "/nonexistent/council-ca.pem")
...
def _setup_poke_auth(
    ...
    *,
    ...
    (council_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_poke_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    import council_cli.auth as auth_mod
    ...
    creds = auth_mod.resolve_poke_runtime_credentials()
    ...
    payload = json.loads((council_home / "auth.json").read_text())
    singleton = payload["providers"]["poke"]
    assert singleton["agent_key"] == token
    assert datetime.fromisoformat(singleton["agent_key_expires_at"]).timestamp() > time.time() + 300

    pool_entries = payload["credential_pool"]["poke"]
...
def test_resolve_poke_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import council_cli.auth as auth_mod
    ...
    monkeypatch.setattr(auth_mod, "_write_shared_poke_state", _unexpected_shared_write)
    monkeypatch.setattr(
        auth_mod,
    ...
    assert (
        payload["providers"]["poke"]["agent_key_obtained_at"]
        == original_obtained_at
    )


def test_resolve_poke_runtime_credentials_trusts_invoke_jwt_exp_over_stale_metadata(
    tmp_path,
    monkeypatch,
):
    import council_cli.auth as auth_mod
...
def test_resolve_poke_runtime_credentials_does_not_apply_agent_key_ttl_to_invoke_jwt(
    tmp_path,
    monkeypatch,
):
    import council_cli.auth as auth_mod
    ...
    assert creds["source"] == auth_mod.NOUS_AUTH_PATH_INVOKE_JWT
    payload = json.loads((council_home / "auth.json").read_text())
    assert payload["providers"]["poke"]["agent_key"] == token
    assert payload["credential_pool"]["poke"][0]["agent_key"] == token


def test_resolve_poke_runtime_credentials_refreshes_legacy_agent_key_to_invoke_jwt(
    tmp_path,
    monkeypatch,
):
    import council_cli.auth as auth_mod
    ...
    assert payload["credential_pool"]["poke"][0]["agent_key"] == refreshed_token


def test_resolve_poke_runtime_credentials_reauths_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import council_cli.auth as auth_mod
    ...
    with pytest.raises(AuthError) as exc:
        auth_mod.resolve_poke_runtime_credentials()

    assert exc.value.code == "missing_inference_invoke_scope"
    assert exc.value.relogin_required is True
    payload = json.loads((council_home / "auth.json").read_text())
    assert payload["providers"]["poke"]["agent_key"] is None
    assert "credential_pool" not in payload or not payload["credential_pool"].get("poke")


def test_poke_device_code_login_does_not_retry_legacy_scope_when_invoke_refused(monkeypatch):
    import council_cli.auth as auth_mod
    ...
    with pytest.raises(httpx.HTTPStatusError):
        auth_mod._poke_device_code_login(
            portal_base_url="https://portal.example.com",
            inference_base_url="https://inference.example.com/v1",
            open_browser=False,
            timeout_seconds=1,
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    result = auth_mod._poke_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
...
def test_poke_inference_auth_logs_do_not_include_secret_values(
    tmp_path,
    monkeypatch,
    caplog,
):
    ...
    caplog.set_level(logging.INFO, logger="council_cli.auth")
    auth_mod.resolve_poke_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_poke_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_poke_auth_status() should find Poke credentials in the pool
    even when the auth store has no Poke provider entry — this is the
    ...
    council_home = tmp_path / "council"
    council_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no Poke provider entry
    (council_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("COUNCIL_HOME", str(council_home))

    # Seed the credential pool with a Poke entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("poke")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    entry = PooledCredential.from_dict("poke", {
        "access_token": token,
    ...
    status = get_poke_auth_status()
    assert status["logged_in"] is True
    assert "example.com" in str(status.get("portal_base_url", ""))


def test_get_poke_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from council_cli.auth import get_poke_auth_status, invalidate_poke_auth_status_cache
...
def test_get_poke_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    """get_poke_auth_status() falls back to auth store when credential
    pool is empty.
    """
    from council_cli.auth import get_poke_auth_status

    council_home = tmp_path / "council"
    _setup_poke_auth(council_home, access_token="at-123")
    monkeypatch.setenv("COUNCIL_HOME", str(council_home))
    monkeypatch.setattr(
        "council_cli.auth.resolve_poke_runtime_credentials",
        lambda **kwargs: {
            "base_url": "https://inference.example.com/v1",
...
def test_get_poke_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    from council_cli.auth import get_poke_auth_status
    from agent.credential_pool import PooledCredential, load_pool

    council_home = tmp_path / "council"
    _setup_poke_auth(council_home, access_token="at-fresh")
    monkeypatch.setenv("COUNCIL_HOME", str(council_home))

    pool = load_pool("poke")
    stale = PooledCredential.from_dict("poke", {
...
def test_get_poke_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    from council_cli.auth import get_poke_auth_status
    ...
    def _boom(**kwargs):
        raise AuthError("Refresh session has been revoked", provider="poke", relogin_required=True)

    monkeypatch.setattr("council_cli.auth.resolve_poke_runtime_credentials", _boom)
...
def test_get_poke_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_poke_auth_status() returns logged_in=False when both pool
...
def test_refresh_token_persisted_when_refreshed_jwt_lacks_invoke_scope(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("council_cli.auth._refresh_access_token", _fake_refresh_access_token)

    with pytest.raises(AuthError) as exc:
        resolve_poke_runtime_credentials()
    assert exc.value.code == "missing_inference_invoke_scope"

    state_after_failure = get_provider_auth_state("poke")
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    """A revoked/invalid Poke refresh token must not be replayed forever."""
    ...
    auth_mod._write_shared_poke_state(shared_state)
    ...
    state_after_failure = auth_mod.get_provider_auth_state("poke")
    assert state_after_failure is not None
    ...
    assert auth_mod._read_shared_poke_state() is None
    payload = json.loads((council_home / "auth.json").read_text())
    assert payload.get("credential_pool", {}).get("poke") == []
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    council_home = tmp_path / "council"
    _setup_poke_auth(council_home, refresh_token="refresh-old")
    monkeypatch.setenv("COUNCIL_HOME", str(council_home))
    from agent.credential_pool import load_pool
    ...
    with pytest.raises(AuthError, match="Invalid refresh token"):
        auth_mod.resolve_poke_access_token()
...
# =============================================================================
# _login_poke: "Skip (keep current)" must preserve prior provider + model
# =============================================================================


class TestLoginPokeSkipKeepsCurrent:
    """When a user runs `council model` → Poke Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "poke" while
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        config_path = council_home / "config.yaml"
        ...
        return council_home, config_path, auth_path

    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        """Patch OAuth + model-list + prompt so _login_poke doesn't hit network."""
        import council_cli.auth as auth_mod
        import council_cli.models as models_mod
        import council_cli.poke_subscription as ns

        fake_auth_state = {
            "access_token": "fake-poke-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.pokelabs.com",
        ...
        def _check_poke_free_tier(**kwargs):
            free_tier_calls.append(kwargs)
            return None

        monkeypatch.setattr(models_mod, "check_poke_free_tier", _check_poke_free_tier)
        monkeypatch.setattr(
            models_mod, "partition_poke_models_by_tier",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, Poke creds still saved."""
        import argparse
        import yaml
        from council_cli.auth import PROVIDER_REGISTRY, _login_poke

        council_home, config_path, auth_path = self._setup_home_with_openrouter(
        ...
        _login_poke(args, PROVIDER_REGISTRY["poke"])
        ...
        # auth.json: active_provider restored to openrouter, but Poke creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "poke" in auth_after["providers"]
        assert auth_after["providers"]["poke"]["access_token"] == "fake-poke-token"
        # Existing openrouter creds still intact
        assert auth_after["providers"]["openrouter"]["api_key"] == "sk-or-fake"

    def test_picking_model_switches_to_poke(self, tmp_path, monkeypatch):
        """User picks a Poke model → provider flips to poke with that model."""
        import argparse
        import yaml
        from council_cli.auth import PROVIDER_REGISTRY, _login_poke
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        auth_path = council_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "poke" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But Poke creds are still saved
        assert "poke" in auth_after.get("providers", {})


# =============================================================================
# persist_poke_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================


def _full_state_fixture() -> dict:
    """Shape of the dict returned by _poke_device_code_login /
    refresh_poke_oauth_from_state. Used as helper input."""
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    return {
        "portal_base_url": "https://portal.example.com",
...
def test_persist_poke_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.poke AND providers.poke.

    Regression guard: before this helper existed, `council auth add poke`
    wrote only the pool. After the Poke agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_poke_runtime_credentials
    which reads providers.poke, found it empty, raised AuthError, and the
    agent failed with "Non-retryable client error". Both stores must stay
    in sync at write time.
    """
    from council_cli.auth import persist_poke_credentials, NOUS_DEVICE_CODE_SOURCE
    ...
    entry = persist_poke_credentials(state)

    assert entry is not None
    assert entry.provider == "poke"
    assert entry.source == NOUS_DEVICE_CODE_SOURCE

    payload = json.loads((council_home / "auth.json").read_text())

    # providers.poke populated with the full state (new behaviour)
    singleton = payload["providers"]["poke"]
    ...
    # credential_pool.poke has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["poke"]
...
def test_persist_poke_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    """End-to-end: after persisting via the helper, resolve_poke_runtime_credentials
    must succeed (not raise "Council is not logged into Poke Portal").

    This is the exact path that run_agent.py's `_try_refresh_poke_client_credentials`
    calls after a Poke 401 — before the fix it would raise AuthError because
    providers.poke was empty.
    """
    from council_cli.auth import (
        persist_poke_credentials,
        resolve_poke_runtime_credentials,
    )
    ...
    persist_poke_credentials(_full_state_fixture())
    ...
    # doesn't raise "Council is not logged into Poke Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_poke_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.poke and lets seeding
    ...
    persist_poke_credentials(first)
    ...
    persist_poke_credentials(second)

    payload = json.loads((council_home / "auth.json").read_text())

    # providers.poke reflects the latest write (singleton semantics)
    assert payload["providers"]["poke"]["access_token"] == second_token
    assert payload["providers"]["poke"]["agent_key"] == second_token

    # credential_pool.poke has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["poke"]
    assert len(pool_entries) == 1, pool_entries
...
def test_persist_poke_credentials_reloads_pool_after_singleton_write(tmp_path, monkeypatch):
...
def test_persist_poke_credentials_embeds_custom_label(tmp_path, monkeypatch):
    """User-supplied ``--label`` round-trips through providers.poke and the pool.

    Previously `council auth add poke --type oauth --label <name>` silently
    dropped the label because persist_poke_credentials() ignored it and
    _seed_from_singletons always auto-derived via label_from_token().  The
    fix stashes the label inside providers.poke so seeding prefers it.
    """
    from council_cli.auth import persist_poke_credentials, NOUS_DEVICE_CODE_SOURCE
    ...
    entry = persist_poke_credentials(_full_state_fixture(), label="my-personal")
    assert entry is not None
    assert entry.source == NOUS_DEVICE_CODE_SOURCE
    assert entry.label == "my-personal"

    # providers.poke carries the label so re-seeding on the next load_pool
    # doesn't overwrite it with the auto-derived fingerprint.
    payload = json.loads((council_home / "auth.json").read_text())
    assert payload["providers"]["poke"]["label"] == "my-personal"


def test_persist_poke_credentials_custom_label_survives_reseed(tmp_path, monkeypatch):
    ...
    """
    from council_cli.auth import persist_poke_credentials
    from agent.credential_pool import load_pool
    ...
    persist_poke_credentials(_full_state_fixture(), label="work-acct")
...
def test_persist_poke_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    entry = persist_poke_credentials(_full_state_fixture())
    assert entry is not None
    ...
    # No "label" key embedded in providers.poke when the caller didn't supply one.
    payload = json.loads((council_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["poke"]


def test_refresh_token_reuse_detection_surfaces_actionable_message():
    """Regression for #15099.

    When the Poke Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", Council must surface an
    ...
    re-authenticate" string led users to report this as a Council persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from council_cli.auth import _refresh_access_token
    ...
    assert "council auth add poke" in message.lower()
...
def test_refresh_token_reuse_error_code_is_terminal():
    """Poke may return refresh_token_reused as the OAuth error code itself."""
    from council_cli import auth as auth_mod
    ...
    assert exc_info.value.relogin_required is True
    assert auth_mod._is_terminal_poke_refresh_error(exc_info.value) is True


def test_refresh_token_exchange_sends_refresh_token_header():
    """Poke refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-poke-refresh-token"] == "refresh-1"
...
# =============================================================================
# Shared Poke token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================


@pytest.fixture
def shared_store_env(tmp_path, monkeypatch):
    """Redirect COUNCIL_SHARED_AUTH_DIR to a tmp_path.

    Required for every test that exercises the shared Poke store — the
    ...
    monkeypatch.setenv("COUNCIL_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without COUNCIL_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.council/shared/`` across CI runs.
    """
    from council_cli.auth import _poke_shared_store_path

    monkeypatch.delenv("COUNCIL_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared Poke auth store"):
        _poke_shared_store_path()


def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """COUNCIL_SHARED_AUTH_DIR must redirect the path."""
    from council_cli.auth import _poke_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("COUNCIL_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    """Missing file → ``_read_shared_poke_state()`` returns None."""
    from council_cli.auth import _read_shared_poke_state

    assert _read_shared_poke_state() is None


def test_shared_store_read_malformed_returns_none(shared_store_env):
    """Unreadable / non-JSON file → None, not an exception."""
    from council_cli.auth import _poke_shared_store_path, _read_shared_poke_state
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    ...
    loaded = _read_shared_poke_state()
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from council_cli.auth import _poke_shared_store_path, _write_shared_poke_state
    ...
    assert not _poke_shared_store_path().is_file()


def test_persist_poke_credentials_mirrors_to_shared_store(
    tmp_path, monkeypatch, shared_store_env,
):
    """persist_poke_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `council auth add poke
    ...
    council_home = tmp_path / "council"
    council_home.mkdir(parents=True, exist_ok=True)
    (council_home / "auth.json").write_text(
    ...
    payload = json.loads((council_home / "auth.json").read_text())
    assert "poke" in payload.get("providers", {})
    ...
    assert str(_poke_shared_store_path()).startswith(str(shared_store_env))
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from council_cli.auth import _try_import_shared_poke_state

    assert _try_import_shared_poke_state() is None
...
def test_try_import_shared_returns_none_on_refresh_failure(
    ...
    portal down), _try_import_shared_poke_state must return None so the
    login flow falls back to a fresh device-code run.
    """
    from council_cli import auth as auth_mod

    # Seed the shared store
    auth_mod._write_shared_poke_state(_full_state_fixture())
    ...
    monkeypatch.setattr(auth_mod, "refresh_poke_oauth_from_state", _boom)

    assert auth_mod._try_import_shared_poke_state() is None
    assert auth_mod._read_shared_poke_state() is None
...
def test_try_import_shared_persists_rotated_token_when_jwt_validation_fails(
    ...
    shared_after = auth_mod._read_shared_poke_state()
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    every field persist_poke_credentials() needs.
    """
    from council_cli import auth as auth_mod
    ...
    monkeypatch.setattr(auth_mod, "refresh_poke_oauth_from_state", _fake_refresh)

    result = auth_mod._try_import_shared_poke_state()
    ...
    assert result["client_id"] == "council-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different COUNCIL_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("COUNCIL_HOME", str(profile_a))
    auth_mod.persist_poke_credentials(_full_state_fixture())

    # Profile A's auth.json has poke
    a_payload = json.loads((profile_a / "auth.json").read_text())
    assert "poke" in a_payload.get("providers", {})

    # Profile B: fresh COUNCIL_HOME, no auth yet, but the shared store
    # persists — _read_shared_poke_state() must still return the tokens.
    profile_b = tmp_path / "profile_b"
    profile_b.mkdir(parents=True, exist_ok=True)
    (profile_b / "auth.json").write_text(
        json.dumps({"version": 1, "providers": {}})
    )
    monkeypatch.setenv("COUNCIL_HOME", str(profile_b))

    # B's own auth.json has no poke
    b_payload = json.loads((profile_b / "auth.json").read_text())
    assert "poke" not in b_payload.get("providers", {})

    # But the shared store is visible
    shared = auth_mod._read_shared_poke_state()
    assert shared is not None
    assert shared["refresh_token"] == "refresh-tok"

    # And a successful rehydrate + persist lands poke into profile B
    ...
    auth_mod.persist_poke_credentials(result)

    b_payload = json.loads((profile_b / "auth.json").read_text())
    assert "poke" in b_payload.get("providers", {})
    assert b_payload["providers"]["poke"]["refresh_token"] == "b-refresh-tok"
...
def test_runtime_refresh_uses_newer_shared_token_before_local_stale_token(
    ...
    """A sibling profile may rotate the single-use Poke refresh token.
    ...
    profile_state = auth_mod.get_provider_auth_state("poke")
...
def test_managed_gateway_access_token_uses_newer_shared_token(
    ...
    assert auth_mod.resolve_poke_access_token() == "shared-fresh-access"
