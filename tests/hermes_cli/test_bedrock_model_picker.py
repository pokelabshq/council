...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (COUNCIL_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via COUNCIL_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in COUNCIL_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from council_cli.providers import COUNCIL_OVERLAYS
        assert "bedrock" in COUNCIL_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from council_cli.providers import COUNCIL_OVERLAYS
        assert COUNCIL_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from council_cli.providers import COUNCIL_OVERLAYS
        assert COUNCIL_OVERLAYS["bedrock"].auth_type == "aws_sdk"
