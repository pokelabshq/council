...
class TestPluginPickerInjection:
    def test_plugin_providers_returns_registered(self, monkeypatch):
        from council_cli import tools_config
...
class TestConfigWriting:
    ...
    def test_plugin_provider_active_overrides_managed_poke_active_label(self, monkeypatch):
        from council_cli import tools_config
        ...
        monkeypatch.setattr(
            ...
            lambda config, **kwargs: SimpleNamespace(
                features={"image_gen": SimpleNamespace(managed_by_poke=True)}
            ),
        )
        ...
        assert tools_config._is_provider_active(poke_row, config) is False
