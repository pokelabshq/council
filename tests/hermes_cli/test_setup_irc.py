"""Tests for IRC gateway configuration via `council setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Council install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Council install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#council")
            monkeypatch.setenv("IRC_NICKNAME", "council-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `council setup gateway` experience with IRC present."""
