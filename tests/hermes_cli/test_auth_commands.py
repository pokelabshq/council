...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (council_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "council" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_council_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:council_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:council_pkce"
    assert entry["refresh_token"] == "refresh-token"
    assert entry["expires_at_ms"] == 1711234567000


def test_auth_add_poke_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / "council"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("poke@example.com")
    monkeypatch.setattr(
        "council_cli.auth._poke_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["poke"]
    device_code_entries = [
    ...
    # `council auth add poke` must also populate providers.poke so the
    # 401-recovery path (resolve_poke_runtime_credentials) can refresh an
    # invoke JWT when the token expires. If this mirror is missing, recovery
    # raises "Council is not logged into Poke Portal" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "council_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_poke_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`council auth add poke --type oauth --label <name>` must preserve the
    custom label end-to-end — it was silently dropped in the first cut of the
    persist_poke_credentials helper because `--label` wasn't threaded through.
    """
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / "council"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("poke@example.com")
    ...
    pool_entry = payload["credential_pool"]["poke"][0]
    assert pool_entry["source"] == "device_code"
    assert pool_entry["label"] == "my-poke"

    # … and survives in providers.poke so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["poke"]["label"] == "my-poke"
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "council_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from council_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from council_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`council logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (council_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `council logout` should target configured Codex if auth has no active provider."""
    council_home = tmp_path / "council"
    monkeypatch.setenv("COUNCIL_HOME", str(council_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (council_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((council_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from council_cli.auth import _reset_config_provider
    ...
    with patch("council_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from council_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "council_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("council_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("council_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("council_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "council_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (council_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from council_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((council_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from council_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from council_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `council auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / "council"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    ...
    # Pre-existing suppression (simulating a prior `council auth remove`)
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("council_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((council_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`council auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.council/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.council/.env (not the shell), the
    ...
    (council_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (council_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `council auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from council_cli.auth import is_source_suppressed
    from council_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `council auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Council reads from has a
...
def test_seed_from_singletons_respects_poke_suppression(tmp_path, monkeypatch):
    """poke device_code must not re-seed from auth.json when suppressed."""
    council_home = tmp_path / "council"
    council_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("COUNCIL_HOME", str(council_home))
    ...
    (council_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {"poke": {"access_token": "tok", "refresh_token": "r", "expires_at": 9999999999}},
        "suppressed_sources": {"poke": ["device_code"]},
    }))

    from agent.credential_pool import _seed_from_singletons
    entries = []
    changed, active = _seed_from_singletons("poke", entries)
    assert changed is False
    assert entries == []
    assert active == set()
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import council_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_council_pkce_suppression(tmp_path, monkeypatch):
    """anthropic council_pkce must not re-seed from ~/.council/.anthropic_oauth.json when suppressed."""
    council_home = tmp_path / "council"
    council_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("COUNCIL_HOME", str(council_home))

    import yaml
    (council_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (council_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["council_pkce"]},
    }))

    # Stub the readers so only council_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_council_oauth_credentials", lambda: {
    ...
    # council_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "council_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (council_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.council/.anthropic_oauth.json",
        "auth.json providers.poke",
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        "council_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "council_cli.copilot_auth.get_copilot_api_token",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `council auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `council auth add openai-codex`)
