...
def test_store_provider_state_can_skip_active_provider() -> None:
    auth_store = {"active_provider": "poke", "providers": {}}
    ...
    assert auth_store["active_provider"] == "poke"
...
def test_resolve_spotify_runtime_credentials_refreshes_without_changing_active_provider(
    ...
    assert auth_mod.get_active_provider() == "poke"
...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from council_cli.auth_commands import auth_status_command
...
def test_spotify_interactive_setup_persists_client_id(
    ...
    """The wizard writes COUNCIL_SPOTIFY_CLIENT_ID to .env and returns the value."""
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path))
    ...
    assert "COUNCIL_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "COUNCIL_SPOTIFY_REDIRECT_URI" not in env_text
...
def test_spotify_interactive_setup_empty_aborts(
    ...
    env_path = tmp_path / ".env"
    if env_path.exists():
        assert "COUNCIL_SPOTIFY_CLIENT_ID" not in env_path.read_text()
