"""Tests for the Command Installation check in council doctor."""
...
import council_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal COUNCIL_HOME + PROJECT_ROOT for doctor tests."""
    ...
    council_bin = venv_bin_dir / "council"
    council_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    council_bin.chmod(0o755)
    ...
    return home, project, council_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, council_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "council"
        cmd_link.symlink_to(council_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "council doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "council"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == council_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "council"
        wrong_target = tmp_path / "wrong_council"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        council_bin = project / ".venv" / "bin" / "council"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "council"
        cmd_link.symlink_to(council_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/council" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/council is a regular file (not symlink), accept it."""
        home, project, council_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "council"
        cmd_link.write_text("#!/bin/sh\nexec python -m council_cli.main \"$@\"\n")
