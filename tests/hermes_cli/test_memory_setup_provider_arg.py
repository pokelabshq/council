"""Tests for `council memory setup [provider]` routing.
...
``council memory setup honcho``) without the interactive picker — which matters
because the per-provider ``council <provider>`` subcommand is only registered
...
from council_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "council memory setup" in out
