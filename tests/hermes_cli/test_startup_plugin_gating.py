...
``council_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``council --help``, ``council version``,
``council logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``council --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from council_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["council", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["council", "--tui", "chat"], "chat"),
        (["council", "-w", "logs"], "logs"),
        (["council", "chat", "hello world"], "chat"),
        (["council", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["council", "-m", "gpt5", "chat"], "chat"),
        (["council", "--model", "gpt5", "chat", "hi"], "chat"),
        (["council", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["council", "-z", "hello world"], None),
        (["council", "-z", "hello", "chat"], "chat"),
        (["council", "--model=gpt5", "chat"], "chat"),     # inline form
        (["council", "--", "chat"], "chat"),               # -- terminator
        (["council", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["council", "some-plugin-cmd"], "some-plugin-cmd"),
        (["council", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["council"],                          # bare → chat
        ["council", "--help"],                # top-level help
        ["council", "-h"],
        ["council", "version"],               # known built-in
        ["council", "logs"],
        ["council", "gateway", "run"],
        ["council", "--tui"],
        ["council", "-w", "--tui"],
        ["council", "chat", "hi"],
        ["council", "help"],                  # accepted built-in-ish
        ["council", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["council", "meet", "join"],          # potential google_meet plugin
        ["council", "honcho", "status"],      # potential memory plugin
        ["council", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``council help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"council_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
