"""Tests for council_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $COUNCIL_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Council-managed node dir.
"""
...
import council_cli.uninstall as uninstall
...
def _make_council_node(council_home: Path) -> Path:
    """Create a fake $COUNCIL_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = council_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_council_node(fake_home):
    council_home = fake_home / ".council"
    node_bin = _make_council_node(council_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(council_home)
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    council_home = fake_home / ".council"
    _make_council_node(council_home)
    local_bin = fake_home / ".local" / "bin"
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(council_home) == []


def test_removes_dangling_symlink_into_council_node(fake_home):
    """A link into the Council node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Council links that exist; ignores the ones that don't."""
    council_home = fake_home / ".council"
    node_bin = _make_council_node(council_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Council-managed; node is a real user binary.
