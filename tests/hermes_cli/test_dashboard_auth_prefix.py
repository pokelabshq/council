...
prefix (e.g. ``mission-control.tilos.com/council/*`` -> local Caddy ->
:9119), injecting ``X-Forwarded-Prefix: /council`` on every request.

The dashboard already honours this for the SPA bundle (rewriting asset
URLs and the bootstrap ``__COUNCIL_BASE_PATH__``). The OAuth gate must
honour it too:

  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/council/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/council/auth/callback`` and the user gets 404.
  4. Cookies must use ``Path=/council`` when behind a prefix so they
...
from council_cli.dashboard_auth import clear_providers, register_provider
from tests.council_cli.conftest_dashboard_auth import StubAuthProvider
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        r = gated_app_proxied.get(
            "/sessions",
            headers={"x-forwarded-prefix": "/council"},
        ...
        assert r.headers["location"].startswith("/council/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/council/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/council/auth/callback`` — the
        ...
        assert parsed.path == "/council/auth/callback", (
...
# ---------------------------------------------------------------------------
# COUNCIL_DASHBOARD_PUBLIC_URL / dashboard.public_url override
# ---------------------------------------------------------------------------


class TestPublicUrlOverride:
    """``dashboard.public_url`` (env override:
    ``COUNCIL_DASHBOARD_PUBLIC_URL``) lets an operator force the absolute
    ...
    def patch_config(self, monkeypatch):
        """Replace ``council_cli.config.load_config`` with a stub
    ...
    def test_public_url_config_yaml_used_when_env_unset(
        ...
        monkeypatch.delenv("COUNCIL_DASHBOARD_PUBLIC_URL", raising=False)
    ...
    def test_env_overrides_config_public_url(
        ...
        monkeypatch.setenv(
            "COUNCIL_DASHBOARD_PUBLIC_URL", "https://from-env.example",
        )
        patch_config("https://from-config.example")
        redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_public_url_with_path_prefix_baked_in(
        ...
        (``https://example.com/council``), the OAuth callback URL is
        the path appended verbatim. The operator is declaring the
        whole authority; we trust them."""
        patch_config(None)
        monkeypatch.setenv(
            "COUNCIL_DASHBOARD_PUBLIC_URL", "https://example.com/council",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://example.com/council/auth/callback"
    ...
    def test_public_url_ignores_x_forwarded_prefix(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "COUNCIL_DASHBOARD_PUBLIC_URL", "https://example.com/already-prefixed",
        )
    ...
    def test_public_url_strips_trailing_slash(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "COUNCIL_DASHBOARD_PUBLIC_URL", "https://example.com/",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_malformed_public_url_falls_through_to_reconstruction(
        ...
        for bad in [
            ...
            monkeypatch.setenv("COUNCIL_DASHBOARD_PUBLIC_URL", bad)
            redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_empty_public_url_env_treated_as_unset(
        ...
        entry."""
        monkeypatch.setenv("COUNCIL_DASHBOARD_PUBLIC_URL", "")
        patch_config("https://from-config.example")
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://from-config.example/auth/callback"
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_prefix_path(self, gated_app_proxied):
        ...
        pkce = next(c for c in cookies if "council_session_pkce" in c)
        # Browser only sends cookie back if the request path is under
        # the cookie's Path attribute, so we need /council here. Bare
        # /-rooted cookies would still be sent but would also be sent
        # to /billing/... etc.
        assert "Path=/council" in pkce, (
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Secure-council_session_pkce=")
        ]
    ...
    def test_pkce_cookie_uses_host_prefix_when_direct(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Host-council_session_pkce=")
        ]
        assert pkce_candidates, (
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from council_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("council_session_pkce=") for c in cookies), (
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        __Secure- prefix AND Path=/council, so the next request under
        ...
        (``/council/auth/login``, ``/council/auth/callback``). A cookie
        set with ``Path=/council`` would therefore NOT be sent back on
        the second request through TestClient even though it WOULD be
        sent by a real browser hitting ``/council/auth/callback``. To
        ...
        pkce_set = next(
            ...
            if "council_session_pkce" in c
        )
        # Parse "__Secure-council_session_pkce=...; HttpOnly; ...".
        pkce_kv = pkce_set.split(";", 1)[0]  # "__Secure-council_session_pkce=value"
        state = r1.headers["location"].split("state=")[1]

        # Round-trip the cookie by hand because TestClient's jar won't
        # automatically send a Path=/council cookie to a /auth/callback
        # request path.
        r2 = gated_app_proxied.get(
            f"/auth/callback?code=stub_code&state={state}",
            headers={
                "x-forwarded-prefix": "/council",
                "cookie": pkce_kv,
            },
            follow_redirects=False,
        )
        ...
        at_cookies = [
            c for c in cookies
            if c.startswith("__Secure-council_session_at=")
        ]
        assert at_cookies, (
            f"session_at missing __Secure- prefix: {cookies!r}"
        )
        assert "Path=/council" in at_cookies[0]
