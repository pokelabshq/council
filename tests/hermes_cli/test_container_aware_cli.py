...
def container_env(tmp_path, monkeypatch):
    """Set up a fake COUNCIL_HOME with .container-mode file."""
    council_home = tmp_path / ".council"
    council_home.mkdir()
    monkeypatch.setenv("COUNCIL_HOME", str(council_home))
    monkeypatch.delenv("COUNCIL_DEV", raising=False)

    container_mode = council_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=ai-council\n"
        "exec_user=council\n"
        "council_bin=/data/current-package/bin/council\n"
    )
    return council_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("council_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "ai-council"
    assert info["exec_user"] == "council"
    assert info["council_bin"] == "/data/current-package/bin/council"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("council_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_council_dev(container_env, monkeypatch):
    """Returns None when COUNCIL_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("COUNCIL_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_council_dev_zero(container_env, monkeypatch):
    """COUNCIL_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("COUNCIL_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        council_home = Path(tmpdir) / ".council"
        council_home.mkdir()
        (council_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("council_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_council_home": lambda: council_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("COUNCIL_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=council-custom\n"
        "exec_user=myuser\n"
        "council_bin=/opt/council/bin/council\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "council-custom"
    assert info["exec_user"] == "myuser"
    assert info["council_bin"] == "/opt/council/bin/council"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from council_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "council"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "ai-council" in cmd
    assert "/data/current-package/bin/council" in cmd
    assert "chat" in cmd
