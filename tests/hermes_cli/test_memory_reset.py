"""Tests for the `council memory reset` CLI command.
...
- Profile-scoped reset (uses COUNCIL_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake COUNCIL_HOME with memory files."""
    council_home = tmp_path / ".council"
    memories = council_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("COUNCIL_HOME", str(council_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nCouncil repo is at ~/.council/ai-council\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `council memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        council_home = tmp_path / ".council"
        (council_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("COUNCIL_HOME", str(council_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's COUNCIL_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_council_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
