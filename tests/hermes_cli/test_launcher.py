"""Tests for the top-level `./council` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./council` should use `council_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "council"
    called = []

    fake_main_module = types.ModuleType("council_cli.main")

    def fake_main():
        called.append("council_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "council_cli.main", fake_main_module)
    ...
    assert called == ["council_cli.main"]
