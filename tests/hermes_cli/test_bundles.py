"""Tests for council_cli/bundles.py — the `council bundles` CLI subcommand."""

import argparse

import pytest

from council_cli.bundles import (
    bundles_command,
    register_cli,
)
...
def bundles_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("COUNCIL_BUNDLES_DIR", str(bundles_dir))
