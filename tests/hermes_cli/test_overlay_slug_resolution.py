...
COUNCIL_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot") while
_PROVIDER_MODELS and config.yaml use Council IDs ("copilot").  The slug
...
def test_copilot_uses_council_slug():
...
def test_kimi_for_coding_overlay_uses_council_slug():
...
def test_kilo_overlay_uses_council_slug():
...
def test_mapped_provider_credential_pool_visibility(monkeypatch):
    ...
    monkeypatch.setattr(
        "council_cli.auth._load_auth_store",
