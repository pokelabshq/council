"""
Tests for timezone support (council_time module + integration points).
...
def _reset_council_time_cache():
    """Reset the council_time module cache (replacement for removed reset_cache)."""
    council_time._cached_tz = None
    council_time._cached_tz_name = None
    council_time._cache_resolved = False


# =========================================================================
# council_time.now() — core helper
# =========================================================================

class TestCouncilTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_council_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["COUNCIL_TIMEZONE"] = "Asia/Kolkata"
        result = council_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["COUNCIL_TIMEZONE"] = "UTC"
        result = council_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["COUNCIL_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="council_time"):
            result = council_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["COUNCIL_TIMEZONE"] = "Europe/London"
        tz = council_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["COUNCIL_TIMEZONE"] = "Not/A/Timezone"
        tz = council_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When COUNCIL_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) COUNCIL_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("COUNCIL_TIMEZONE=" + os.environ.get("COUNCIL_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "COUNCIL_TIMEZONE=NOT_SET" in result["output"], (
            "COUNCIL_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When COUNCIL_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("COUNCIL_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=council_tz) which shifted
        absolute time when system-local tz != Council tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_council_tz(self):
        """Already-aware datetimes should be normalized to Council tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Council tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Council tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Council is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Council tz
        ...
        # Use a Council timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _council_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["COUNCIL_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_council_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Council timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["COUNCIL_TIMEZONE"] = "US/Eastern"
        _reset_council_time_cache()
