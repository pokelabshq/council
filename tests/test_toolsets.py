...
class TestToolsetConsistency:
    ...
    def test_council_platforms_share_core_tools(self):
        """All council-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on council-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["council-cli", "council-telegram", "council-discord", "council-whatsapp", "council-slack", "council-signal", "council-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_council_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("council-whatsapp")

    def test_council_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("council-api-server")
