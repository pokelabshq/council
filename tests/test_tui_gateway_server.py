...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["COUNCIL_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("COUNCIL_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("COUNCIL_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("COUNCIL_VOICE", "1")
    monkeypatch.delenv("COUNCIL_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("COUNCIL_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("COUNCIL_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "council_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("COUNCIL_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("COUNCIL_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "council_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    import council_cli.config as config_mod
    ...
    # whose tools live in council-cli's universe (see toolsets.py).
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("COUNCIL_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "council_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("COUNCIL_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("COUNCIL_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("COUNCIL_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("COUNCIL_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "council_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown COUNCIL_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("COUNCIL_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("COUNCIL_MODEL", raising=False)
    monkeypatch.delenv("COUNCIL_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " poke/council-test "}}
    )

    assert server._resolve_model() == "poke/council-test"


def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("COUNCIL_MODEL", "poke/council-test")
    monkeypatch.setenv("COUNCIL_TUI_PROVIDER", "poke")
    monkeypatch.delenv("COUNCIL_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("poke/council-test", "poke")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("COUNCIL_MODEL", "poke/council-test")
    monkeypatch.delenv("COUNCIL_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("COUNCIL_INFERENCE_PROVIDER", "poke")
    monkeypatch.setattr(
        "council_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("poke/council-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("COUNCIL_MODEL", "sonnet")
    monkeypatch.delenv("COUNCIL_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("COUNCIL_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "council_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "council_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "council_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "council_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("COUNCIL_YOLO_MODE", "false")
    ...
    assert os.environ.get("COUNCIL_YOLO_MODE") == "1"
...
def test_config_set_statusbar_survives_non_dict_display(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_council_home", tmp_path)
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    ...
    assert server.os.environ["COUNCIL_GATEWAY_SESSION"] == "1"
    assert server.os.environ["COUNCIL_EXEC_ASK"] == "1"
    assert server.os.environ["COUNCIL_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("council_cli.main._has_any_provider_configured", lambda: False)
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("council_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("council_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_syncs_inference_provider_env(monkeypatch):
    """After an explicit provider switch, COUNCIL_INFERENCE_PROVIDER must
    ...
    server._sessions["sid"] = _session(agent=_Agent())
    monkeypatch.setenv("COUNCIL_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "council_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    assert os.environ["COUNCIL_INFERENCE_PROVIDER"] == "anthropic"


def test_config_set_model_syncs_tui_provider_unconditionally(monkeypatch):
    """Regression for #16857: /model must set COUNCIL_TUI_PROVIDER even when
    ...
    # Both env vars must reflect the user's choice. COUNCIL_TUI_PROVIDER is
    # the canonical explicit-this-process carrier consumed by
    # _resolve_startup_runtime() on /new.
    assert os.environ["COUNCIL_TUI_PROVIDER"] == "custom:xuanji"
    assert os.environ["COUNCIL_INFERENCE_PROVIDER"] == "custom:xuanji"
...
def test_config_set_model_syncs_tui_provider_env(monkeypatch):
    ...
    agent = Agent()
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setenv("COUNCIL_TUI_PROVIDER", "openai-codex")
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("council_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert resp["result"]["value"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["COUNCIL_TUI_PROVIDER"] == "anthropic"
        assert os.environ["COUNCIL_MODEL"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["COUNCIL_INFERENCE_MODEL"] == "anthropic/claude-sonnet-4.6"
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors CouncilCLI._manual_compress's
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Council TUI Status" in out
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("council_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "council_cli.commands.SlashCommandCompleter",
...
def test_session_create_close_race_does_not_orphan_worker(monkeypatch):
    ...
    # Start: session.create spawns _build thread, returns synchropokely
    resp = server.handle_request(
        {
            "id": "1",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("council_state")
    ...
    monkeypatch.setitem(sys.modules, "council_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_council_home
    # fixture pins COUNCIL_HOME to a temp dir; the handler should append
    # /sessions to it.
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `council model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `council model` and the classic CLI /model picker.
    ...
    for providers like Poke whose /models endpoint returns image/video
    ...
    with patch(
        "council_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("council_cli.models.provider_model_ids") as live_fetch:
            resp = server._methods["model.options"](99, {"session_id": ""})

    assert "result" in resp, resp
    providers = resp["result"]["providers"]
    poke = next((p for p in providers if p.get("slug") == "poke"), None)
    assert poke is not None
    assert poke["models"] == [
        "moonshotai/kimi-k2.5",
        "anthropic/claude-opus-4.7",
    ]
    assert poke["total_models"] == 30
...
class _ImmediateThread:
    """Runs the target callable synchropokely so assertions can follow."""
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"council_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "council_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
                "council_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "council_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_council_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.council/.env
    ...
    with patch.dict(sys.modules, {"council_cli.config": fake}):
