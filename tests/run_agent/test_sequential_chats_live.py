...
AlexKucera reported on Discord (2026-04-16) that after ``council update`` pulled
...
Opt-in — not part of default CI:
    COUNCIL_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.council/.env).
"""
...
pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set COUNCIL_LIVE_TESTS=1"),
