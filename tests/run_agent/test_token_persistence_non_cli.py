...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    council_state = ModuleType("council_state")
    council_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "council_state", council_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Council"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Council"
