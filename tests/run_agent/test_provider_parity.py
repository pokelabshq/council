...
class TestDeveloperRoleSwap:
    ...
    def test_developer_role_via_poke_portal(self, monkeypatch):
        agent = _make_agent(
            monkeypatch,
            "poke",
...
class TestBuildApiKwargsPokePortal:
    def test_includes_poke_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == poke_portal_tags()
...
# ── Chat completions response handling (OpenRouter/Poke) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_poke_when_no_openrouter(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_poke_auth", return_value=poke_auth), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("council_cli.models.get_poke_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == "google/gemini-3-flash-preview"

    def test_custom_endpoint_when_no_poke(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/Poke keys are available.
