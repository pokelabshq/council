"""Shared fixtures for the ai-council test suite.
...
2. **Isolated COUNCIL_HOME.** COUNCIL_HOME points to a per-test tempdir so
   code reading ``~/.council/*`` via ``get_council_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".council"`` instead of the canonical
   ``get_council_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No COUNCIL_SESSION_* inheritance** — the agent's current gateway
...
# COUNCIL_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_COUNCIL_BEHAVIORAL_VARS = frozenset({
    "COUNCIL_YOLO_MODE",
    "COUNCIL_INTERACTIVE",
    "COUNCIL_QUIET",
    ...
    "COUNCIL_SESSION_CHAT_ID",
    "COUNCIL_SESSION_CHAT_NAME",
    "COUNCIL_SESSION_THREAD_ID",
    "COUNCIL_SESSION_SOURCE",
    "COUNCIL_SESSION_KEY",
    "COUNCIL_GATEWAY_SESSION",
    "COUNCIL_PLATFORM",
    "COUNCIL_MODEL",
    "COUNCIL_INFERENCE_MODEL",
    "COUNCIL_INFERENCE_PROVIDER",
    "COUNCIL_TUI_PROVIDER",
    "COUNCIL_MANAGED",
    "COUNCIL_DEV",
    "COUNCIL_CONTAINER",
    "COUNCIL_EPHEMERAL_SYSTEM_PROMPT",
    "COUNCIL_TIMEZONE",
    "COUNCIL_REDACT_SECRETS",
    "COUNCIL_BACKGROUND_NOTIFICATIONS",
    "COUNCIL_EXEC_ASK",
    "COUNCIL_HOME_MODE",
    "AI_COUNCIL_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.council/kanban.db instead of the per-test COUNCIL_HOME.
    "COUNCIL_KANBAN_DB",
    "COUNCIL_KANBAN_BOARD",
    "COUNCIL_KANBAN_HOME",
    "COUNCIL_KANBAN_WORKSPACES_ROOT",
    "COUNCIL_KANBAN_LOGS_ROOT",
    "COUNCIL_KANBAN_TASK",
    "COUNCIL_KANBAN_WORKSPACE",
    "COUNCIL_KANBAN_RUN_ID",
    "COUNCIL_KANBAN_CLAIM_LOCK",
    "COUNCIL_KANBAN_DISPATCH_IN_GATEWAY",
    "COUNCIL_TENANT",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `poke` plugin auto-registers itself on plugin discovery,
    ...
    "COUNCIL_DASHBOARD_OAUTH_CLIENT_ID",
    "COUNCIL_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and COUNCIL_HOME to per-test tempdirs so code that
    reads ``~/.council/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral COUNCIL_* vars that could change test semantics.
    for name in _COUNCIL_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect COUNCIL_HOME to a per-test tempdir. Code that reads
    #    ``~/.council/*`` via ``get_council_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.council/*`` via
    #    ``Path.home() / ".council"`` instead of ``get_council_home()``
    #    is a bug to fix at the callsite.
    fake_council_home = tmp_path / "council_test"
    fake_council_home.mkdir()
    (fake_council_home / "sessions").mkdir()
    (fake_council_home / "cron").mkdir()
    (fake_council_home / "memories").mkdir()
    (fake_council_home / "skills").mkdir()
    monkeypatch.setenv("COUNCIL_HOME", str(fake_council_home))
    ...
    #    ~/.council/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import council_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_council_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal council config dict suitable for unit tests."""
    return {
...
def _ensure_current_event_loop(request):
    ...
    Python 3.11+ no longer guarantees a current loop for plain synchropoke tests.
...
# environment and finds the developer's live ``council-gateway`` process
...
# correlated with a ``tests/council_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> council-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart council-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting council/python patterns are also blocked.
    """
    ...
    def _matches_council_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _COUNCIL_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_council_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch council-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m council_cli.main".
                if (
                    "council" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live council-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting council/python could hit the live gateway. "
