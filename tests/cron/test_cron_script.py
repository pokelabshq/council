...
- Path resolution (absolute, relative to COUNCIL_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp COUNCIL_HOME."""
    council_home = tmp_path / ".council"
    council_home.mkdir()
    (council_home / "cron").mkdir()
    (council_home / "cron" / "output").mkdir()
    (council_home / "scripts").mkdir()
    monkeypatch.setenv("COUNCIL_HOME", str(council_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "COUNCIL_DIR", council_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", council_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", council_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", council_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("COUNCIL_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.council/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("COUNCIL_SESSION_PLATFORM") is None
        assert os.environ.get("COUNCIL_SESSION_CHAT_ID") is None
        assert os.environ.get("COUNCIL_SESSION_CHAT_NAME") is None
