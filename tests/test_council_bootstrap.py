"""Tests for council_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Council entry point
(council, ai-council, council-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Council entry point imports council_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported council_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("council_bootstrap", None)
    import council_bootstrap  # noqa: WPS433
    return council_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Council CLI
...
class TestEntryPointsImportBootstrap:
    """Every Council entry point must import council_bootstrap as its
    ...
    # Entry points that invoke Council as a process.  Each one must
    # import council_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "council_cli/main.py",   # council CLI (console_script)
        "run_agent.py",          # ai-council (console_script)
        "acp_adapter/entry.py",  # council-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import council_bootstrap' and that
        ...
        half-finished ``council update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``council_bootstrap`` as a top-level module) leaves council
        ...
        """
        # Resolve relative to the ai-council repo root.  Tests live
        # at tests/test_council_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # council start even when council_bootstrap hasn't been
        ...
        assert first_import_name == "council_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'council_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import council_bootstrap' line to be the first import."
        )
