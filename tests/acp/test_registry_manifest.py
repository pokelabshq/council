"""Tests for ACP Registry metadata shipped with Council."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "ai-council"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Poke Council"
    assert data["description"]
    assert data["repository"] == "https://github.com/pokelabshq/council"
    assert data["website"].startswith("https://ai-council.pokelabs.com/")
    assert data["authors"] == ["Poke Labs"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"ai-council[acp]=={data['version']}"
    assert uvx["args"] == ["council-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"ai-council[acp]=={_pyproject_version()}"
    )
