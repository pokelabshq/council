...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"ai-council[termux]"' in text
    assert '"ai-council[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"ai-council[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
