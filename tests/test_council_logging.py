"""Tests for council_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    council_logging._logging_initialized = False
    ...
    council_logging._install_session_record_factory()
    yield
    ...
    for h in list(root.handlers):
        if h not in pre_existing:
            ...
            h.close()
    council_logging._logging_initialized = False
    council_logging.clear_session_context()


@pytest.fixture
def council_home(tmp_path, monkeypatch):
    """Provide an isolated COUNCIL_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_council_home from conftest,
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, council_home):
        log_dir = council_logging.setup_logging(council_home=council_home)
        assert log_dir == council_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, council_home):
        council_logging.setup_logging(council_home=council_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, council_home):
        council_logging.setup_logging(council_home=council_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, council_home):
        council_logging.setup_logging(council_home=council_home)
        council_logging.setup_logging(council_home=council_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, council_home):
        council_logging.setup_logging(council_home=council_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        council_logging.setup_logging(council_home=council_home, force=True)
    ...
    def test_custom_log_level(self, council_home):
        council_logging.setup_logging(council_home=council_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, council_home):
        council_logging.setup_logging(
            council_home=council_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, council_home):
        council_logging.setup_logging(council_home=council_home)
    ...
    def test_writes_to_agent_log(self, council_home):
        council_logging.setup_logging(council_home=council_home)

        test_logger = logging.getLogger("test_council_logging.write_test")
        ...
        agent_log = council_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, council_home):
        council_logging.setup_logging(council_home=council_home)

        test_logger = logging.getLogger("test_council_logging.warning_test")
        ...
        agent_log = council_home / "logs" / "agent.log"
        errors_log = council_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, council_home):
        council_logging.setup_logging(council_home=council_home)

        test_logger = logging.getLogger("test_council_logging.info_test")
    ...
    def test_explicit_params_override_config(self, council_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (council_home / "config.yaml").write_text(yaml.dump(config))

        council_logging.setup_logging(council_home=council_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, council_home):
        """The custom record factory injects session_tag on all records."""
        council_logging.setup_logging(council_home=council_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_council_session_injector", False), (
            "Record factory should have _council_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, council_home):
        council_logging.setup_logging(council_home=council_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, council_home):
        council_logging.setup_logging(council_home=council_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, council_home):
        ...
        gw_log = council_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, council_home):
    ...
    def test_gateway_log_receives_gateway_records(self, council_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, council_home):
    ...
    def test_agent_log_still_receives_all(self, council_home):
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, council_home):
        """When session context is set, log lines include [session_id]."""
        council_logging.setup_logging(council_home=council_home)
        council_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, council_home):
    ...
    def test_clear_session_context(self, council_home):
        """After clearing, session tag disappears."""
        council_logging.setup_logging(council_home=council_home)
        council_logging.set_session_context("xyz789")
        council_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, council_home):
        ...
        def thread_a():
            council_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            council_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        council_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        council_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = council_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = council_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in council_logging.COMPONENT_PREFIXES
        # The gateway component captures both core gateway logs and the
        # council_plugins facility (plugin-installed gateway adapters log
        # under that prefix).
        assert ("gateway", "council_plugins") == council_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = council_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == council_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = council_logging.COMPONENT_PREFIXES["cli"]
        assert "council_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == council_logging.COMPONENT_PREFIXES["cron"]


class TestSetupVerboseLogging:
    """setup_verbose_logging() adds a DEBUG-level console handler."""

    def test_adds_stream_handler(self, council_home):
        council_logging.setup_logging(council_home=council_home)
        council_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_council_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, council_home):
        council_logging.setup_logging(council_home=council_home)
        council_logging.setup_verbose_logging()
        council_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        council_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = council_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        council_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("council_cli.config.is_managed", return_value=True):
                council_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, council_home):
        level, max_size, backup = council_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, council_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, council_home):
        import yaml
        config = {"model": "test"}
        (council_home / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> council_logging._ManagedRotatingFileHandler:
        handler = council_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that council_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        handler.flush()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        council_logging.setup_logging(council_home=council_home, mode="gateway")
        gw_path = council_home / "logs" / "gateway.log"
        rotated = council_home / "logs" / "gateway.log.1"
