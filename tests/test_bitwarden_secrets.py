...
binary download are exercised manually by `council secrets bitwarden
...
def council_home(tmp_path, monkeypatch):
    """Point Council at an isolated home directory."""
    home = tmp_path / ".council"
    home.mkdir()
    monkeypatch.setenv("COUNCIL_HOME", str(home))
    # Some modules cache get_council_home; clear if needed.
    import council_constants
    if hasattr(council_constants, "_COUNCIL_HOME_CACHE"):
        council_constants._COUNCIL_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(council_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(council_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(council_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from council_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under council_home/cache/."""
    home = tmp_path / ".council"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
