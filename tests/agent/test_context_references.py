...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Council Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_council_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / ".council"))

    council_env = tmp_path / ".council" / ".env"
    council_env.parent.mkdir(parents=True)
    council_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.council/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
