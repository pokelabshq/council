...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Council passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("COUNCIL_HOME", "/users/alice/.council/profiles/backend-worker")
        monkeypatch.setenv("COUNCIL_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "COUNCIL_KANBAN_DB",
            "/users/alice/.council/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.council/kanban/boards/smoke"]'
            in cmd
        )
