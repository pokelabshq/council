...
def isolated_home(monkeypatch):
    """Temp COUNCIL_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="council_test_31179_")
    council_home = os.path.join(test_home, ".council")
    os.makedirs(council_home)
...
def _fresh_modules():
    """Drop cached council modules so each test reloads against current env."""
