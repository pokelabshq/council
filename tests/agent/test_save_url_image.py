...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate COUNCIL_HOME under tmp_path."""
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / ".council"))
    (tmp_path / ".council").mkdir()

    # Force the constants/image cache helpers to re-read COUNCIL_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("council_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_council_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under COUNCIL_HOME — gateway cleanup
