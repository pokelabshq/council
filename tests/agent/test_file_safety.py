...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_council_env(self):
        """Council' own .env inside COUNCIL_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: council internal .env is in ~/.council/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.council/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.council/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Council cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        council_home = tmp_path / ".council"
        cache = council_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._council_home_path", return_value=council_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Council cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        council_home = tmp_path / ".council"
        hub = council_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_council_home(self, tmp_path):
        """The env basename guard does not depend on COUNCIL_HOME resolution."""
        council_home = tmp_path / ".council"
        council_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        council_home = tmp_path / ".council"
        cache = council_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
