...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "council_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "council" / "auth.json").read_text())
...
def test_load_pool_does_not_persist_env_seeded_secret_value(tmp_path, monkeypatch):
    ...
    auth_text = (tmp_path / "council" / "auth.json").read_text()
...
def test_load_pool_persists_bitwarden_origin_metadata_without_secret(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", sentinel)
    monkeypatch.setattr(
        "council_cli.env_loader.get_secret_source",
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    from council_cli.auth import write_credential_pool
    ...
    write_credential_pool("openrouter", [
        {
            ...
            "auth_type": "api_key",
            "priority": 0,
            "source": "systemd://council/openrouter",
            "access_token": sentinel,
            "refresh_token": f"refresh-{sentinel}",
            "agent_key": f"agent-{sentinel}",
            "api_key": f"extra-{sentinel}",
    ...
    assert borrowed["source"] == "systemd://council/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "council" / "auth.json").read_text())["credential_pool"]["poke"][0]
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.council/.env when
    ...
    # User edited ~/.council/.env with the fresh key
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.council/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (council_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_load_pool_migrates_poke_provider_state(tmp_path, monkeypatch):
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / "council"))
    _write_auth_store(
        tmp_path,
        {
...
def test_load_pool_mirrors_poke_invoke_jwt_agent_key_runtime_api_key(tmp_path, monkeypatch):
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / "council"))
    ...
    auth_payload = json.loads((tmp_path / "council" / "auth.json").read_text())
    pool_entry = auth_payload["credential_pool"]["poke"][0]
    assert pool_entry["agent_key"] == token
    assert pool_entry["agent_key_expires_at"] == expires_at


def test_poke_runtime_api_key_rejects_opaque_agent_key():
    from agent.credential_pool import PooledCredential
...
def test_poke_pool_terminal_refresh_removes_device_code_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / "council"))
    monkeypatch.setenv("COUNCIL_SHARED_AUTH_DIR", str(tmp_path / "shared"))
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    pool.add_entry(PooledCredential.from_dict("poke", {
        "id": "legacy-seeded",
        "source": "manual:device_code",
        "auth_type": "oauth",
    ...
    pool.add_entry(PooledCredential.from_dict("poke", {
        ...
        "source": "manual",
        "auth_type": "api_key",
        "access_token": "manual-poke-key",
    }))

    monkeypatch.setattr(auth_mod, "resolve_poke_runtime_credentials", _terminal_refresh_failure)
    ...
    auth_payload = json.loads((tmp_path / "council" / "auth.json").read_text())
    poke_state = auth_payload["providers"]["poke"]
    ...
    assert [entry["id"] for entry in auth_payload["credential_pool"]["poke"]] == ["manual-key"]

    assert pool.try_refresh_current() is None
    assert refresh_calls["count"] == 1


def test_load_pool_removes_poke_device_code_when_singleton_quarantined(tmp_path, monkeypatch):
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / "council"))
    _write_auth_store(
        tmp_path,
        {
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_council_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_load_pool_migrates_poke_provider_state_preserves_tls(tmp_path, monkeypatch):
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / "council"))
    _write_auth_store(
        tmp_path,
        {
    ...
    auth_payload = json.loads((tmp_path / "council" / "auth.json").read_text())
    assert auth_payload["credential_pool"]["poke"][0]["tls"] == {
        "insecure": True,
        "ca_bundle": "/tmp/poke-ca.pem",
    }
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("council_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    assert {entry.source for entry in entries} == {"manual:council_pkce", "council_pkce"}
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `council setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Council PKCE
    ...
    monkeypatch.setattr("agent.anthropic_adapter.read_council_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `council_pkce` into auth.json) and later switches to the API key at
    `council setup` would still have those OAuth entries dormant on disk.
    ...
    monkeypatch.setattr("council_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("agent.anthropic_adapter.read_council_oauth_credentials", lambda: None)
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `council setup`.  When ANTHROPIC_TOKEN is set (and
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "council_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
...
def test_poke_seed_from_singletons_preserves_obtained_at_timestamps(tmp_path, monkeypatch):
    """Regression test for #15099 secondary issue.

    When ``_seed_from_singletons`` materialises a device_code pool entry from
    the ``providers.poke`` singleton, it must carry the mint/refresh
    ...
    assert e.agent_key == "sk-poke-AAAA"
...
# ── PR #10160 salvage: Poke OAuth cross-process sync tests ─────────────────

def test_sync_poke_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    synced = pool._sync_poke_entry_from_auth_store(entry)
...
def test_sync_poke_entry_noop_when_tokens_match(tmp_path, monkeypatch):
...
def test_poke_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    """An exhausted Poke entry should recover when auth.json has newer tokens."""
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / "council"))
    from agent.credential_pool import load_pool, STATUS_EXHAUSTED
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `council auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `council model` to reauth, but the pool
    ...
    # Simulate `council model` / `council auth` refreshing the tokens.
...
def test_is_terminal_xai_oauth_refresh_error():
    from council_cli.auth import AuthError, _is_terminal_xai_oauth_refresh_error
    ...
    # Poke error does not trigger xAI check
    assert not _is_terminal_xai_oauth_refresh_error(
        AuthError("Revoked", provider="poke", code="invalid_grant", relogin_required=True)
    )
    # Generic exception
...
def test_is_terminal_codex_oauth_refresh_error():
    from council_cli.auth import AuthError, _is_terminal_codex_oauth_refresh_error
