...
- Warning respects AI_COUNCIL_TIMEOUT_WARNING env var
...
class TestStagedInactivityWarning:
    ...
    def test_warning_env_var_respected(self, monkeypatch):
        """AI_COUNCIL_TIMEOUT_WARNING env var is parsed correctly."""
        monkeypatch.setenv("AI_COUNCIL_TIMEOUT_WARNING", "600")
        _warning = float(os.getenv("AI_COUNCIL_TIMEOUT_WARNING", 900))
        assert _warning == 600.0

    def test_warning_zero_means_disabled(self, monkeypatch):
        """AI_COUNCIL_TIMEOUT_WARNING=0 disables the warning."""
        monkeypatch.setenv("AI_COUNCIL_TIMEOUT_WARNING", "0")
        _raw = float(os.getenv("AI_COUNCIL_TIMEOUT_WARNING", 900))
