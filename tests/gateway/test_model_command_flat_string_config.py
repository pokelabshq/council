...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "council_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_council_home() / config.yaml`` — point it here.
    monkeypatch.setattr("council_constants.get_council_home", lambda: council_home)
    monkeypatch.setattr("council_cli.config.get_council_home", lambda: council_home)
    return cfg_path
