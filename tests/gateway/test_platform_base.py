...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("COUNCIL_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("COUNCIL_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("COUNCIL_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("COUNCIL_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("COUNCIL_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("COUNCIL_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("COUNCIL_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("COUNCIL_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("COUNCIL_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.council/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.delenv("COUNCIL_MEDIA_DELIVERY_STRICT", raising=False)
        monkeypatch.delenv("COUNCIL_MEDIA_ALLOW_DIRS", raising=False)
    ...
    def test_denylist_blocks_council_credentials(self, tmp_path, monkeypatch):
        """~/.council/.env and ~/.council/auth.json stay blocked even in
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting COUNCIL_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``COUNCIL_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("COUNCIL_MEDIA_DELIVERY_STRICT", raw)
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"COUNCIL_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("COUNCIL_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"COUNCIL_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "COUNCIL_HUMAN_DELAY_MODE": "natural",
            "COUNCIL_HUMAN_DELAY_MIN_MS": "oops",
            "COUNCIL_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "COUNCIL_HUMAN_DELAY_MODE": "custom",
            "COUNCIL_HUMAN_DELAY_MIN_MS": "100",
            "COUNCIL_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"COUNCIL_MEDIA_DELIVERY_STRICT": "1",
                                     "COUNCIL_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("COUNCIL_MEDIA_DELIVERY_STRICT", "0")
