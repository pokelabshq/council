"""Tests for council_constants module."""
...
class TestGetDefaultCouncilRoot:
    """Tests for get_default_council_root() — Docker/custom deployment awareness."""

    def test_no_council_home_returns_native(self, tmp_path, monkeypatch):
        """When COUNCIL_HOME is not set, returns ~/.council."""
        monkeypatch.delenv("COUNCIL_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_council_root() == tmp_path / ".council"

    def test_council_home_is_native(self, tmp_path, monkeypatch):
        """When COUNCIL_HOME = ~/.council, returns ~/.council."""
        native = tmp_path / ".council"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("COUNCIL_HOME", str(native))
        assert get_default_council_root() == native

    def test_council_home_is_profile(self, tmp_path, monkeypatch):
        """When COUNCIL_HOME is a profile under ~/.council, returns ~/.council."""
        native = tmp_path / ".council"
    ...
    def test_council_home_is_docker(self, tmp_path, monkeypatch):
        """When COUNCIL_HOME points outside ~/.council (Docker), returns COUNCIL_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("COUNCIL_HOME", str(docker_home))
        assert get_default_council_root() == docker_home

    def test_council_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any COUNCIL_HOME outside ~/.council is treated as the root."""
        custom = tmp_path / "my-council-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("COUNCIL_HOME", str(custom))
        assert get_default_council_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (COUNCIL_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("COUNCIL_HOME", str(profile))
        assert get_default_council_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(council_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(council_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".council"
