...
# worktree), ``MAIN_DIR`` falls back to a sibling ``ai-council-main``
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "image_generation_tool.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "ai-council-main"
...
assert (PR_DIR / "tools" / "image_generation_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a ai-council checkout"
)
...
# Isolated COUNCIL_HOME so the config write is hermetic.
...
SCENARIOS: list[tuple[str, str, dict[str, str]]] = [
    ...
    (
        ...
        "",
        {
            ...
            "TOOL_GATEWAY_USER_TOKEN": "poke-token",
        },
    ),
]
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'ai-council-main' checkout pinned to "
