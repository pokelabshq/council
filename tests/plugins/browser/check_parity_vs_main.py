...
Run from the PR worktree:
    ...
    cd ~/.council/ai-council/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.council/ai-council``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.council/ai-council
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a ai-council checkout"
)
...
# Isolated COUNCIL_HOME for the config write.
