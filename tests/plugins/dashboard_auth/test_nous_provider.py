"""Tests for the bundled Poke dashboard-auth plugin.

Covers four shapes from Phase 4 of ``.council/plans/2026-05-21-dashboard-oauth-auth.md``:
...
import plugins.dashboard_auth.poke as poke_plugin
...
def _patched_jwks(provider: poke_plugin.PokeDashboardAuthProvider, rsa_keypair):
...
class TestConstruction:
    def test_protocol_compliance(self):
        assert_protocol_compliance(poke_plugin.PokeDashboardAuthProvider)

    def test_name_and_display(self):
        p = poke_plugin.PokeDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )
        assert p.name == "poke"
        assert p.display_name == "Poke Labs"
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            poke_plugin.PokeDashboardAuthProvider(
                client_id="council-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        monkeypatch.delenv("COUNCIL_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
        monkeypatch.delenv("COUNCIL_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        poke_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason is surfaced for the gate's fail-closed message.
        assert "COUNCIL_DASHBOARD_OAUTH_CLIENT_ID" in poke_plugin.LAST_SKIP_REASON

    def test_registers_with_default_portal_url_when_only_client_id_set(
        self, monkeypatch
    ):
        """Phase 7 follow-up: COUNCIL_DASHBOARD_PORTAL_URL is optional —
        defaults to the production Poke Portal. The user shouldn't have
        to set it for the common production deployment path."""
        monkeypatch.setenv("COUNCIL_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.delenv("COUNCIL_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        poke_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert isinstance(registered, poke_plugin.PokeDashboardAuthProvider)
        assert registered._portal_url == "https://portal.pokelabs.com"
        # Skip reason cleared on successful registration.
        assert poke_plugin.LAST_SKIP_REASON == ""

    def test_skips_when_client_id_malformed(self, monkeypatch):
        monkeypatch.setenv("COUNCIL_DASHBOARD_OAUTH_CLIENT_ID", "council-dashboard")
        monkeypatch.setenv("COUNCIL_DASHBOARD_PORTAL_URL", "https://p.example")
        ctx = MagicMock()
        poke_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason names the offending value + contract shape.
        assert "agent:" in poke_plugin.LAST_SKIP_REASON
        assert "council-dashboard" in poke_plugin.LAST_SKIP_REASON
    ...
    def test_strips_whitespace_from_env_vars(self, monkeypatch):
        monkeypatch.setenv("COUNCIL_DASHBOARD_OAUTH_CLIENT_ID", "  agent:x  ")
        monkeypatch.setenv("COUNCIL_DASHBOARD_PORTAL_URL", "  https://p.example  ")
        ctx = MagicMock()
        poke_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
    ...
    def test_empty_portal_url_env_uses_default(self, monkeypatch):
        ...
        monkeypatch.setenv("COUNCIL_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.setenv("COUNCIL_DASHBOARD_PORTAL_URL", "")
        ctx = MagicMock()
        poke_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert registered._portal_url == "https://portal.pokelabs.com"
...
class TestConfigYamlSource:
    ...
    canonical surface for these settings. ``COUNCIL_DASHBOARD_OAUTH_CLIENT_ID``
    and ``COUNCIL_DASHBOARD_PORTAL_URL`` are operator overrides that win when
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``council_cli.config.load_config``
        ...
        def _set(oauth_block: Dict[str, Any] | None) -> None:
            ...
            monkeypatch.setattr(
                "council_cli.config.load_config", lambda: cfg
            )
    ...
    def test_env_overrides_config_client_id(self, patch_config, monkeypatch):
        ...
        COUNCIL_DASHBOARD_OAUTH_CLIENT_ID at deploy time and we MUST
        honour it even if a stale config.yaml ships in the image."""
        monkeypatch.setenv("COUNCIL_DASHBOARD_OAUTH_CLIENT_ID", "agent:from-env")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        poke_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_env_overrides_config_portal_url(self, patch_config, monkeypatch):
        monkeypatch.setenv("COUNCIL_DASHBOARD_OAUTH_CLIENT_ID", "agent:x")
        monkeypatch.setenv(
            "COUNCIL_DASHBOARD_PORTAL_URL", "https://env.portal.example",
        )
        patch_config({
            "client_id": "agent:x",
    ...
    def test_empty_env_string_does_not_shadow_config(
        ...
        """``COUNCIL_DASHBOARD_OAUTH_CLIENT_ID=`` (set but empty) is
        ...
        monkeypatch.setenv("COUNCIL_DASHBOARD_OAUTH_CLIENT_ID", "")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        poke_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
    ...
    def test_neither_source_skips_with_helpful_reason(
        ...
        assert "dashboard.oauth.client_id" in poke_plugin.LAST_SKIP_REASON, (
            f"skip reason omits the config.yaml surface — operators "
            f"won't know it exists. got: {poke_plugin.LAST_SKIP_REASON!r}"
        )
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(
        ...
        monkeypatch.setattr(
            "council_cli.config.load_config", _broken_load
        )
        ctx = MagicMock()
        # Must not raise.
        poke_plugin.register(ctx)
...
class TestStartLogin:
    @pytest.fixture
    def provider(self):
        return poke_plugin.PokeDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )

    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://council.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://council.fly.dev/auth/callback"
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "council_session_pkce" in result.cookie_payload
        pkce = result.cookie_payload["council_session_pkce"]
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["council_session_pkce"] != b.cookie_payload[
            "council_session_pkce"
        ]
    ...
    def test_rejects_http_with_non_localhost(self, provider):
        with pytest.raises(ProviderError, match="localhost"):
            provider.start_login(
                redirect_uri="http://council.fly.dev/auth/callback"
            )
...
class TestCompleteLogin:
    ...
    def test_happy_path_returns_session(self, provider, rsa_keypair):
        ...
        with patch("plugins.dashboard_auth.poke.httpx.post", return_value=mock_resp):
    ...
    def test_network_error_raises_provider_error(self, provider):
        with patch(
            "plugins.dashboard_auth.poke.httpx.post",
...
class TestVerifySession:
    ...
    def test_verification_failure_message_surfaces_token_claims(
        ...
        config drift between COUNCIL_DASHBOARD_PORTAL_URL/CLIENT_ID and Portal."""
        token = _mint_token(rsa_keypair, iss="https://evil.example")
    ...
    def test_contract_version_missing_warns_but_succeeds(
        ...
        with caplog.at_level(logging.WARNING, logger="plugins.dashboard_auth.poke"):
            session = provider.verify_session(access_token=token)
        assert session is not None
        assert any(
