...
See: https://github.com/pokelabshq/council/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for council_constants.get_subprocess_home()."""

    def test_returns_none_when_council_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        council_home = tmp_path / ".council"
        council_home.mkdir()
        profile_home = council_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("COUNCIL_HOME", str(council_home))
        from council_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".council" / "profiles"
        ...
        monkeypatch.setenv("COUNCIL_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("COUNCIL_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from council_constants import (
            get_council_home,
            reset_council_home_override,
            set_council_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_council_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_council_home_override(profile)
        try:
            assert get_council_home() == profile
            release.set()
            thread.join(timeout=5)
        finally:
            reset_council_home_override(token)
            release.set()

        assert seen == [str(root)]
        assert get_council_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(council_home / "home")
    ...
    def test_no_injection_when_council_home_unset(self, monkeypatch):
        monkeypatch.delenv("COUNCIL_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["COUNCIL_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from council_cli.profiles import _PROFILE_DIRS
