...
def test_dashboard_run_resets_home_before_dropping_privileges() -> None:
    ...
    assert "exec s6-setuidgid council council dashboard" in text
...
def test_dashboard_run_does_not_derive_insecure_from_bind_host() -> None:
    ...
    ``COUNCIL_DASHBOARD_HOST``. Doing so disables the OAuth auth gate on
    ...
    The opt-in is now explicit: ``COUNCIL_DASHBOARD_INSECURE=1`` (truthy).
    ...
    assert '127.0.0.1|localhost' not in text, (
        ...
        "is the authority now — opt in via COUNCIL_DASHBOARD_INSECURE instead."
    )
    ...
    assert "COUNCIL_DASHBOARD_INSECURE" in text, (
        "Explicit COUNCIL_DASHBOARD_INSECURE opt-in is missing."
    )
    # Truthy values aligned with the rest of the s6 scripts
    # (COUNCIL_DASHBOARD, COUNCIL_DASHBOARD_TUI).
    for truthy in ("1", "true", "TRUE", "True", "yes", "YES", "Yes"):
        assert truthy in text, (
            f"COUNCIL_DASHBOARD_INSECURE should accept truthy value {truthy!r}"
        )
