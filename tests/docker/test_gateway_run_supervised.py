...
Before the s6 migration, ``docker run pokelabshq/council gateway
...
crash and a supervised dashboard alongside (when ``COUNCIL_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``COUNCIL_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``COUNCIL_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``council`` per the conftest module
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``council gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "COUNCIL_GATEWAY_NO_SUPERVISE=1",
    ...
    if inspect.stdout.strip() == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "COUNCIL_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The COUNCIL_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``council gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``council gateway
    ...
    # Count python processes running `council gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*council.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `council gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-council`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``COUNCIL_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${COUNCIL_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Council Gateway Starting…"
    ...
    assert "⚕" in combined or "Council Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Council Gateway Starting" in file_contents, (
