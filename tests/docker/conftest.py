...
Override the image with ``COUNCIL_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("COUNCIL_TEST_IMAGE", "ai-council-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``COUNCIL_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("COUNCIL_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged council user
# ---------------------------------------------------------------------------
#
# Background: every Council runtime path inside the container drops to UID
# 10000 (the ``council`` user) via ``s6-setuidgid council``. ``docker exec``
...
# fails (PermissionError swallowed) for council, so a test that ran as root
...
# helpers below run every probe under ``-u council`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: council).
