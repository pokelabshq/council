...
The shim (docker/council-exec-shim.sh, installed at /opt/council/bin/council)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> council login` would write /opt/data/auth.json as
...
failed: Council is not logged into Poke Portal" on every message.

These tests verify:

1. ``docker exec <c> council …`` (defaulting to root) gets dropped to the
   council user before the real binary runs.
2. ``docker exec --user council <c> council …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $COUNCIL_HOME from a ``docker exec`` session land
   as council:council — the actual user-visible invariant.
4. The COUNCIL_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def test_shim_drops_root_to_council_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `council` with a Python-style `-c` shim equivalent — there's no
    pure-council "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `council` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `council`
    do anything that writes to disk, then check the file's owner.

    Use `council config set` which writes config.yaml under COUNCIL_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "council", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by council, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "council:council", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected council:council. "
        "The shim did not drop privileges before invoking council."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user council already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "council", sleep_container,
         "council", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user council failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up council:council — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """COUNCIL_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "COUNCIL_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
         "council", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "root:root", (
        f"With COUNCIL_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``COUNCIL_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``COUNCIL_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"COUNCIL_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
         "council", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "council:council", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected council:council"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/council/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid council council <args>` which resolves `council`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> council login`` (root) wrote
    ...
    as a parse failure, and resolve_poke_runtime_credentials raised
    "Council is not logged into Poke Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `council config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is council-readable.

    Specifically: pretend the operator ran `council login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the council UID. We use `council auth list` since that touches the
    ...
    # `council auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `council config set` first to
    # provoke a write into COUNCIL_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "council", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # COUNCIL_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "council", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the council user "
