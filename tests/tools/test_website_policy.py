...
def test_check_website_access_uses_dynamic_council_home(monkeypatch, tmp_path):
    ...
    # Invalidate the module-level cache so the new COUNCIL_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old COUNCIL_HOME set by the autouse _isolate_council_home fixture.
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing COUNCIL_HOME to tmp_path
