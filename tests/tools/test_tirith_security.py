...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._council_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(council_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(council_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_sync_resolve_persists_failure(self, mock_which, mock_install,
                                            mock_disk_check, mock_mark):
        """Synchropoke _resolve_tirith_path persists failure to disk."""
        from tools.tirith_security import _resolve_tirith_path
        _tirith_mod._resolved_path = None
    ...
    def test_install_failed_recovers_from_council_bin(self):
        """After _INSTALL_FAILED, manual install in COUNCIL_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        council_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(council_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(council_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._council_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == council_bin
            assert _tirith_mod._resolved_path == council_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# COUNCIL_HOME isolation
# ---------------------------------------------------------------------------

class TestCouncilHomeIsolation:
    def test_council_bin_dir_respects_council_home(self):
        """_council_bin_dir must use COUNCIL_HOME, not hardcoded ~/.council."""
        from tools.tirith_security import _council_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"COUNCIL_HOME": tmpdir}):
            result = _council_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_council_home(self):
        """_failure_marker_path must use COUNCIL_HOME, not hardcoded ~/.council."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"COUNCIL_HOME": "/custom/council"}):
            result = _failure_marker_path()
        assert result == "/custom/council/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets COUNCIL_HOME; verify it's active."""
        council_home = os.getenv("COUNCIL_HOME")
        assert council_home is not None, "COUNCIL_HOME should be set by conftest"
        assert "council_test" in council_home, "Should point to test temp dir"

    def test_get_council_home_fallback(self):
        """Without COUNCIL_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_council_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove COUNCIL_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".council")
            result = _get_council_home()
