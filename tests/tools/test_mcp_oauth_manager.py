...
def test_council_provider_subclass_exists():
    """CouncilMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _COUNCIL_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _COUNCIL_PROVIDER_CLS is not None
    assert issubclass(_COUNCIL_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_council_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns CouncilMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _COUNCIL_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path))
    ...
    assert _COUNCIL_PROVIDER_CLS is not None
    assert isinstance(provider, _COUNCIL_PROVIDER_CLS)
    assert provider._council_server_name == "srv"
