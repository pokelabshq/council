...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        council_home.mkdir()
        (council_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(council_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.council/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        council_home = tmp_path / ".council"
        council_home.mkdir()
        (council_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        council_home = tmp_path / ".council"
        council_home.mkdir()
        (council_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        council_home = tmp_path / ".council"
        council_home.mkdir()
        (council_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        council_home = tmp_path / ".council"
        skills_dir = council_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.council/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        council_home = tmp_path / ".council"
        (council_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"COUNCIL_HOME": str(council_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.council")

        assert mounts[0]["container_path"] == "/home/user/.council/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.council/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.council/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape COUNCIL_HOME."""
        monkeypatch.setenv("COUNCIL_HOME", str(tmp_path / ".council"))
        (tmp_path / ".council").mkdir()

        # Create a sensitive file one level above council_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside council_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside COUNCIL_HOME must still be registered."""
        council_home = tmp_path / ".council"
        council_home.mkdir()
        monkeypatch.setenv("COUNCIL_HOME", str(council_home))
        (council_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_council_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of COUNCIL_HOME must be allowed."""
        council_home = tmp_path / ".council"
        council_home.mkdir()
        subdir = council_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside COUNCIL_HOME pointing outside must be rejected."""
        council_home = tmp_path / ".council"
        council_home.mkdir()
        monkeypatch.setenv("COUNCIL_HOME", str(council_home))

        # Create a sensitive file outside council_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside council_home pointing outside
        symlink = council_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes COUNCIL_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, council_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape COUNCIL_HOME."""
        council_home = tmp_path / ".council"
        council_home.mkdir()
        monkeypatch.setenv("COUNCIL_HOME", str(council_home))
        ...
        sensitive.write_text("{}")
        self._write_config(council_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(council_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside COUNCIL_HOME via config must still mount."""
        council_home = tmp_path / ".council"
        council_home.mkdir()
        monkeypatch.setenv("COUNCIL_HOME", str(council_home))

        (council_home / "oauth.json").write_text("{}")
        self._write_config(council_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        council_home = tmp_path / ".council"
        council_home.mkdir()
        (council_home / "cache" / "documents").mkdir(parents=True)
        (council_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("COUNCIL_HOME", str(council_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.council/cache/documents" in paths
        assert "/root/.council/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.council/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        council_home = tmp_path / ".council"
        council_home.mkdir()
        # Use legacy dir name — get_council_dir prefers old if it exists
        (council_home / "document_cache").mkdir()
        (council_home / "image_cache").mkdir()
        monkeypatch.setenv("COUNCIL_HOME", str(council_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(council_home / "document_cache") in host_paths
        assert str(council_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.council/cache/documents" in container_paths
        assert "/root/.council/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        council_home = tmp_path / ".council"
        doc_dir = council_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        council_home = tmp_path / ".council"
        ss_dir = council_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.council/cache/screenshots/session_abc/screen1.png"
