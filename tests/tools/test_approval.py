...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("council_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("council_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"COUNCIL_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_council_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.council/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_council_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $COUNCIL_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_council_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$COUNCIL_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_council_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $COUNCIL_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.council/ai-council && source venv/bin/activate && python -m council_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m council_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m council_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "council_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart council-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_council_detected(self):
        """pkill targeting council/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_council_detected(self):
        cmd = "killall council"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep council) bypasses the pkill/killall name-matching
    ...
    def test_pkill_council_still_detected(self):
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("COUNCIL_GATEWAY_SESSION", "COUNCIL_YOLO_MODE",
                      "COUNCIL_SESSION_KEY", "COUNCIL_INTERACTIVE")
        }
        os.environ.pop("COUNCIL_YOLO_MODE", None)
        os.environ.pop("COUNCIL_INTERACTIVE", None)
        os.environ["COUNCIL_GATEWAY_SESSION"] = "1"
        os.environ["COUNCIL_SESSION_KEY"] = self.SESSION_KEY
