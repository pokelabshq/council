...
class TestWriteDenyExactPaths:
    ...
    def test_council_env(self):
        # ``.env`` under the active COUNCIL_HOME (profile-aware, not just
        # ``~/.council``) must be write-denied. The hermetic test conftest
        # points COUNCIL_HOME at a tempdir — resolve via get_council_home()
        # to match the denylist.
        from council_constants import get_council_home
        path = str(get_council_home() / ".env")
        assert _is_write_denied(path) is True

    def test_council_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.council/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "council_root"
        ...
        # Sanity check: COUNCIL_HOME does point to the profile dir, not the root.
        from council_constants import get_council_home, get_default_council_root
        assert get_council_home() == profile_home
        assert get_default_council_root() == root
...
class TestWriteAllowed:
    ...
    def test_council_config_not_env(self):
        path = os.path.join(str(Path.home()), ".council", "config.yaml")
