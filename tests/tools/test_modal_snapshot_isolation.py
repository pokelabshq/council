...
def _restore_tool_modules():
    original_council_home = os.environ.get("COUNCIL_HOME")
    original_modules = {
        name: module
        ...
        or name == "council_cli"
        or name.startswith("council_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_council_home is None:
            os.environ.pop("COUNCIL_HOME", None)
        else:
            os.environ["COUNCIL_HOME"] = original_council_home
        _reset_modules(("tools", "council_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    council_cli = types.ModuleType("council_cli")
    council_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["council_cli"] = council_cli
    council_home = tmp_path / "council-home"
    os.environ["COUNCIL_HOME"] = str(council_home)
    sys.modules["council_cli.config"] = types.SimpleNamespace(
        get_council_home=lambda: council_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="ai-council", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": council_home / "modal_snapshots.json",
