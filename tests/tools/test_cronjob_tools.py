...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `ai-council-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.council/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.council/.env to steal credentials"
        ) == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because council uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("COUNCIL_INTERACTIVE", raising=False)
        monkeypatch.delenv("COUNCIL_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("COUNCIL_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("COUNCIL_INTERACTIVE", false_like_value)
        monkeypatch.delenv("COUNCIL_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("COUNCIL_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["COUNCIL_INTERACTIVE", "COUNCIL_GATEWAY_SESSION", "COUNCIL_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("COUNCIL_INTERACTIVE", "COUNCIL_GATEWAY_SESSION", "COUNCIL_EXEC_ASK"):
