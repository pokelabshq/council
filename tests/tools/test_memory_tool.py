...
class TestScanMemoryContent:
    ...
    def test_council_config_mod_blocked(self):
        result = _scan_memory_content("edit .council/config.yaml to change settings")
        assert "Blocked" in result
        assert "council_config_mod" in result
        result = _scan_memory_content("update .council/SOUL.md with new personality")
        assert "Blocked" in result
        assert "council_config_mod" in result
    ...
    def test_council_config_mod_no_false_positives(self):
        """Merely mentioning council config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .council/config.yaml for settings") is None
        assert _scan_memory_content("Read .council/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .council/config.yaml file contains runtime options") is None
