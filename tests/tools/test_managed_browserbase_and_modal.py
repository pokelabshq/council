...
def _enable_managed_poke_tools(monkeypatch):
    """Ensure managed_poke_tools_enabled() returns True even after module reloads.
    ...
    council_cli.poke_account — so the function body returns True.
    """
    monkeypatch.setattr(
...
def test_browser_use_availability_skips_refresh_for_expired_cached_gateway_token(tmp_path, monkeypatch):
    ...
    (tmp_path / "auth.json").write_text(
        '{"providers":{"poke":{"access_token":"expired-token","refresh_token":"refresh-token","expires_at":"%s"}}}'
        % expired_at,
        encoding="utf-8",
    )
    refresh_calls = []
...
def test_browser_use_managed_gateway_adds_idempotency_key_and_persists_external_call_id():
    ...
    assert sent_headers["X-Browser-Use-API-Key"] == "poke-token"
