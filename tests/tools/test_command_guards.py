...
def _clean_state():
    ...
    saved = {}
    for k in ("COUNCIL_INTERACTIVE", "COUNCIL_GATEWAY_SESSION", "COUNCIL_EXEC_ASK", "COUNCIL_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["COUNCIL_INTERACTIVE"] = "1"
        session_key = os.getenv("COUNCIL_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No COUNCIL_INTERACTIVE or COUNCIL_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
