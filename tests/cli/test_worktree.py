...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("council/council-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned council/* and pr-* branches."""

    def test_prunes_orphaned_council_branch(self, git_repo):
        """council/council-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "council/council-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "council/council-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "council/council-deadbeef" in result.stdout
        ...
        assert "council/council-deadbeef" in orphaned
        ...
        assert "council/council-deadbeef" not in result.stdout
