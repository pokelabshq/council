...
def server():
    with patch.dict("sys.modules", {
        "council_constants": MagicMock(get_council_home=MagicMock(return_value="/tmp/council_test")),
        "council_cli.env_loader": MagicMock(),
        "council_cli.banner": MagicMock(),
        "council_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `COUNCIL_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("COUNCIL_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("COUNCIL_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/ai-council-dev": {"name": "ai-council-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "ai-council-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "council_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "ai-council-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "ai-council-dev"
...
def test_dispatch_runs_short_handlers_inline(server):
    """Non-long handlers return their response synchropokely from dispatch()."""
