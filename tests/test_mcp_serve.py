"""
Tests for mcp_serve — Council MCP server.
...
def _isolate_council_home(tmp_path, monkeypatch):
    """Redirect COUNCIL_HOME to a temp directory."""
    monkeypatch.setenv("COUNCIL_HOME", str(tmp_path))
    try:
        import council_constants
        monkeypatch.setattr(council_constants, "get_council_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking council_state schema."""
