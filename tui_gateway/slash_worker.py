"""Persistent slash-command worker — one CouncilCLI per TUI session.
...
def _run(cli: CouncilCLI, command: str) -> str:
...
def main():
    ...
    os.environ["COUNCIL_SESSION_KEY"] = args.session_key
    os.environ["COUNCIL_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = CouncilCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
