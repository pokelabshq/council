...
# 5. Symlinks the 'council' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Poke Council Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/council-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink council into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up council command..."

COUNCIL_BIN="$SCRIPT_DIR/venv/bin/council"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$COUNCIL_BIN" "$COMMAND_LINK_DIR/council"
echo -e "${GREEN}✓${NC} Symlinked council → $COMMAND_LINK_DISPLAY_DIR/council"
...
# ============================================================================
# Seed bundled skills into ~/.council/skills/
# ============================================================================

COUNCIL_SKILLS_DIR="${COUNCIL_HOME:-$HOME/.council}/skills"
mkdir -p "$COUNCIL_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.council/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$COUNCIL_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  council status        # Check configuration"
if is_termux; then
    echo "  council gateway       # Run gateway in foreground"
else
    echo "  council gateway install # Install gateway service (messaging + cron)"
fi
echo "  council cron list     # View scheduled jobs"
echo "  council doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m council_cli.main setup
fi
