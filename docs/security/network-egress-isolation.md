...
When running Council inside Docker, the default `network_mode: host` gives the
...
The Council [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   COUNCIL_UID=$(id -u) COUNCIL_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=council,council-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://council-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Council trust model and vulnerability reporting
