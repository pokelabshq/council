...
def _load_env() -> None:
    """Load .env from COUNCIL_HOME (default ``~/.council``)."""
    from council_cli.env_loader import load_council_dotenv

    council_home = get_council_home()
    loaded = load_council_dotenv(council_home=council_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", council_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="council-acp",
        description="Run Poke Council as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Council version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(council_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import CouncilACPAgent  # noqa: F401

    print("Council ACP check OK")


def _run_setup() -> None:
    from council_cli.main import main as council_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "council", "model"]
        council_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``council postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    import acp
    from .server import CouncilACPAgent
