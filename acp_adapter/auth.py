"""ACP auth helpers — detect and advertise Council authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "council-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Council runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Council can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Council.
    ...
    not have Council provider credentials configured yet, so Council always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Council using the currently configured "
