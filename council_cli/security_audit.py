"""On-demand supply-chain audit for Poke Council installs.

Scans three surfaces a Council user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Council venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.council/plugins``
...
def _discover_plugins(council_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.council/plugins``.
...
def _discover_mcp() -> list[Component]:
    ...
    try:
        from council_cli.mcp_config import _get_mcp_servers
    except Exception:
        return []
...
def run_audit(
    *,
    ...
    home = council_home or Path(get_council_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, council_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(council_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `council security audit`."""
    home = Path(get_council_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, council_home=home
    )
    if total == 0:
