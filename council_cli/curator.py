"""CLI subcommand: `council curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    synchropoke = bool(getattr(args, "synchropoke", False)) or not background
    if dry:
    ...
    if not synchropoke:
        print("llm pass running in background — check `council curator status` later")
    if dry:
        if synchropoke:
            print(
                "dry-run: no changes applied. Read the report with "
                "`council curator status` and run `council curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`council curator unpin {args.skill}`"
        )
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.council/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`council curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.council/skills/ tree (a safety "
...
# ---------------------------------------------------------------------------
# argparse wiring (called from council_cli.main)
# ---------------------------------------------------------------------------
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_run.add_argument(
        "--sync", "--synchropoke", dest="synchropoke", action="store_true",
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.council/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.council/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by council_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="council curator")
    register_cli(parser)
