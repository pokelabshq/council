"""Shell completion script generation for council CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_council_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Poke Council bash completion
# Add to ~/.bashrc:
#   eval "$(council completion bash)"

_council_profiles() {{
    local profiles_dir="$HOME/.council/profiles"
...
_council_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_council_profiles)" -- "$cur"))
        return
    fi
...
complete -F _council_completion council
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef council
# Poke Council zsh completion
# Add to ~/.zshrc:
...
_council_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.council/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.council/profiles 2>/dev/null)}}")
    fi
...
_council() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_council_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'council command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Poke Council fish completion",
        "# Add to your config:",
        "#   council completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __council_profiles",
        "    echo default",
        "    if test -d $HOME/.council/profiles",
        "        ls $HOME/.council/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c council -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c council -f -s p -l profile"
        " -d 'Profile name' -xa '(__council_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__council_profiles)' -d 'Profile name'"
                )
