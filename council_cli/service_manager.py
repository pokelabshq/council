...
the existing module-level functions in council_cli.gateway and
council_cli.gateway_windows directly. This protocol is a thin facade
...
``council gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``council gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged council
    ...
    detection always fails. Since every Council runtime call inside the
    container drops to council via ``s6-setuidgid``, that silent failure
...
# in ``council_cli.gateway`` (systemd/launchd) and ``council_cli.gateway_windows``
...
# active (set via the ``council -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in council_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from council_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from council_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from council_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from council_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in council_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from council_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from council_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from council_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from council_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``council_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from council_cli import gateway_windows
        from council_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `council profile create`
# runs inside the container (Phase 4). Static services (main-council, dashboard)
...
# UID/GID of the in-image ``council`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the council user.
    ...
    (the ``council`` user — UID 10000 — running every Council runtime
    ...
    is dispatched as the council user.
    ...
    with council ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           council:council, 0755 (parent must already exist)
    ``svc_dir/event/``                     council:council, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 council:council, 0755
    ``svc_dir/supervise/event/``           council:council, 03730
    ``svc_dir/supervise/control``          council:council, 0660    (FIFO)
    ...
    slot's supervise/ was council-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _COUNCIL_UID, _COUNCIL_GID)
        except PermissionError:
            # Running as the council user already — directory is council-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _COUNCIL_UID, _COUNCIL_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is council-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _COUNCIL_UID, _COUNCIL_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``council -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`council profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-council, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources COUNCIL_HOME (and any extra env) via with-contenv —
             so e.g. ``-e COUNCIL_HOME=/data/council`` is honored at run
          ...
          4. Drops to the council user and exec's
             ``council -p <profile> gateway run`` (or just ``council
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``council gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root COUNCIL_HOME profile" (the implicit profile that exists at
        the top of $COUNCIL_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``council_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$COUNCIL_HOME/profiles/default/`` — a
        ...
        ``council_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        # guard.
        lines.append("export COUNCIL_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            lines.append("exec s6-setuidgid council council gateway run")
        else:
            lines.append(
                f"exec s6-setuidgid council council -p {shlex.quote(profile)} gateway run"
            )
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${COUNCIL_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the COUNCIL_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e COUNCIL_HOME=/data/council``
        gets its logs under /data/council/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``council logs`` reads and what
        ...
        return (
            ...
            f': "${{COUNCIL_HOME:=/opt/data}}"\n'
            f'log_dir="$COUNCIL_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R council:council "$log_dir" 2>/dev/null || true\n'
            f'exec s6-setuidgid council s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with council ownership
            ...
            # the council user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # Give s6-svscan a moment to reap. There's no synchropoke
        ...
        # is council-owned (see ``_seed_supervise_skeleton``), and on
