...
Pure display functions with no CouncilCLI state dependency.
"""
...
from council_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built council — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/pokelabshq/council.git"
...
def check_for_updates() -> Optional[int]:
    """Check whether a Council update is available.

    Two paths: if ``COUNCIL_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = council_home / ".update_check"
    embedded_rev = os.environ.get("COUNCIL_REVISION") or None
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``council_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from council_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/pokelabshq/council/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Council checkout. Cached per-process. Release URL always points at the
    canonical pokelabshq/council repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Poke Council v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else COUNCIL_CADUCEUS
    except Exception:
        _bskin = None
        _hero = COUNCIL_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Poke Labs[/]")

    if os.getenv("COUNCIL_YOLO_MODE"):
    ...
    try:
        from council_cli.codex_runtime_switch import get_current_runtime
        from council_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from council_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built council; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else AI_COUNCIL_LOGO
        console.print(_logo)
        console.print()
