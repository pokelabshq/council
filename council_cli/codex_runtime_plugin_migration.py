"""Migrate Council' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Council' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Council mcp_servers.<n>.command/args/env  → codex stdio transport
  Council mcp_servers.<n>.url/headers       → codex streamable_http transport
  Council mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Council mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Council-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Council' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Council config.")
...
# Council keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Council' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, council_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Council MCP server config to the codex inline-table dict
    ...
    has_command = bool(council_cfg.get("command"))
    has_url = bool(council_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (COUNCIL_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Council)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Council' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Council' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Council
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="council-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a COUNCIL_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed council-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.council`` symlink that happens to live under ``/private/var/folders``
...
def _build_council_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Council' own
    ...
    so a council installed under /opt/, /usr/local/, or a venv all work.
    COUNCIL_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # COUNCIL_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_council_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever COUNCIL_HOME its launcher sets
    ...
    # — that would override the launcher's COUNCIL_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("COUNCIL_HOME", tmp_path) would
    ...
    council_home = os.environ.get("COUNCIL_HOME") or ""
    if council_home and _looks_like_test_tempdir(council_home):
        council_home = ""
    if council_home:
        env["COUNCIL_HOME"] = council_home
    # PYTHONPATH passes through so a worktree-launched council finds the
    ...
    env["COUNCIL_QUIET"] = "1"
    env["COUNCIL_REDACT_SECRETS"] = env.get("COUNCIL_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_council_tools: bool = True,
) -> MigrationReport:
    """Translate Council mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        council_config: full ~/.council/config.yaml dict
        ...
        expose_council_tools: when True (default), register Council' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Council for tools
    ...
    council_servers = (council_config or {}).get("mcp_servers") or {}
    if not isinstance(council_servers, dict):
        report.errors.append(
            "mcp_servers in Council config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in council_servers.items():
    ...
    # Inject Council' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Council for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/council_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_council_tools:
        translated["council-tools"] = _build_council_tools_mcp_entry()
        if "council-tools" not in report.migrated:
            report.migrated.append("council-tools")
