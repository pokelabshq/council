"""CLI subcommand: ``council send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"council send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"council send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"council send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"council send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"council send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `council gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.council/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_council_env() -> None:
    """Populate ``os.environ`` from ``~/.council/.env`` AND bridge top-level
    ...
    startup that ``council send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.council/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.council/config.yaml`` into
       ...
       via ``council config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``council send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.council/.env and ~/.council/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "council send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  council send --to telegram \"hello\"\n"
            "  council send --to discord:#ops --file report.md\n"
            "  council send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "council send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `council send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Council "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.council/.env + ~/.council/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `council send --list telegram`.",
    )
