"""
Multi-provider authentication system for Poke Council.

Supports OAuth device code flows (Poke Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.council/auth.json with cross-process file locking.
...
from council_cli.config import get_council_home, get_config_path, read_raw_config
from council_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# Poke Portal defaults
DEFAULT_NOUS_PORTAL_URL = "https://portal.pokelabs.com"
DEFAULT_NOUS_INFERENCE_URL = "https://inference-api.pokelabs.com/v1"
DEFAULT_NOUS_CLIENT_ID = "council-cli"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from council_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.council/.env file
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``council auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `council model` to re-authenticate."

    if error.code == "subscription_required":
        if error.provider == "poke":
            return _format_poke_entitlement_auth_error(error)
...
def _format_poke_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        account_info = get_poke_portal_account_info(force_fresh=True)
        message = format_poke_portal_entitlement_message(
            account_info,
            capability="Poke model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in Poke Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("COUNCIL_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.council/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_council_home() / "auth.json"
    # Seat belt: if pytest is running and COUNCIL_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch COUNCIL_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".council" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set COUNCIL_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom COUNCIL_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_council_root()
    except Exception:
        return None
    profile_home = get_council_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.council/auth.json`` even when COUNCIL_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_council_root()`` for a profile-shaped COUNCIL_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".council" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared Poke store) don't share reentrancy
...
def _auth_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    ``_poke_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared Poke lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "poke_portal" in systems:
            providers["poke"] = systems["poke_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "poke" if providers else None}
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``poke``) that were only authenticated at
    global scope. Once the user runs ``council auth login <provider>`` inside
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``council auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Poke device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Council.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `council logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from council_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'council doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'council model' for available providers, or run 'council doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'council model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.council/.env.",
...
# Allowlist of hosts the Poke Portal proxy is willing to forward inference
...
_ALLOWED_NOUS_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.pokelabs.com",
})


def _validate_poke_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    if parsed.scheme != "https":
        logger.warning(
            "poke: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_NOUS_INFERENCE_HOSTS:
        logger.warning(
            "poke: refusing inference URL host %r from Portal response "
...
def _poke_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _assert_poke_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "Poke Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: council auth add poke",
        provider="poke",
        code=reason,
...
def _log_poke_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.info("Poke inference auth: using NAS invoke JWT")
    _oauth_trace(
        "poke_invoke_jwt_selected",
        sequence_id=sequence_id,
        access_token_fp=_token_fingerprint(access_token),
    )


def _poke_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_poke_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _poke_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_poke_invoke_jwt(
    ...
    *,
    ...
    _set_poke_agent_key_from_invoke_jwt(state)
    _log_poke_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )
...
def _poke_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("COUNCIL_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `council model` walks users into a broken Qwen setup flow.
...
#
# Tokens live in ~/.council/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `council auth list` / `council status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.council/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("COUNCIL_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set COUNCIL_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("COUNCIL_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("COUNCIL_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("COUNCIL_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but Council times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``COUNCIL_LOG_LEVEL=INFO`` (or check
            # ``~/.council/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>council auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `council auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `council auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `council auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `council auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.council/.env, and return it.
    ...
    # Persist so subsequent `council auth spotify` runs skip the wizard.
    save_env_value("COUNCIL_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("COUNCIL_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved COUNCIL_SPOTIFY_CLIENT_ID to ~/.council/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "COUNCIL_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Council:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Council is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.council/auth.json (not ~/.codex/)
#
# Council maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Council auth store (~/.council/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `council auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `council auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `council auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `council auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``council setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``council auth add openai-codex``
      ...
      ``council auth add`` workaround for #33000 would silently leave that
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Council auth store (~/.council/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Council auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run council auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `council auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Council auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Council's own Codex token store.
    ...
    try:
        ...
        if pool_token:
            base_url = (
                os.getenv("COUNCIL_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("COUNCIL_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Council processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.council/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `council model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `council model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `council model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `council model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.council/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `council model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``COUNCIL_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `council model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Council (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `council model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``council model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("COUNCIL_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("COUNCIL_XAI_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("COUNCIL_CA_BUNDLE")
...
# =============================================================================
# Poke Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared Poke token store — lets OAuth credentials persist across profiles
# so a new `council --profile <name> auth add poke --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${COUNCIL_SHARED_AUTH_DIR}/poke_auth.json, defaulting to
# ``<council-root>/shared/poke_auth.json`` where ``<council-root>`` is what
# ``get_default_council_root()`` returns — ``~/.council`` on Linux/macOS,
# ``%LOCALAPPDATA%\council`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's COUNCIL_HOME so named profiles (which
# typically live under ``<council-root>/profiles/<name>/``) all see the
# same file.
#
...
NOUS_SHARED_STORE_FILENAME = "poke_auth.json"
_poke_shared_lock_holder = threading.local()


def _poke_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared Poke token store.

    Honors ``COUNCIL_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<council-root>/shared/``, where ``<council-root>`` is what
    :func:`council_constants.get_default_council_root` returns — so
    Linux/macOS classic installs land at ``~/.council/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\council\\shared\\``, and
    Docker / custom ``COUNCIL_HOME`` deployments at
    ``<COUNCIL_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("COUNCIL_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from council_constants import get_default_council_root
    return get_default_council_root() / "shared"


def _poke_shared_store_path() -> Path:
    path = _poke_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Council root, refuse rather than silently corrupt cross-profile
    # state. Tests must set COUNCIL_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from council_constants import get_default_council_root
        real_home_shared = (
            get_default_council_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared Poke auth store during test run: "
                f"{path}. Set COUNCIL_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path


@contextmanager
def _poke_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared Poke OAuth store.
    ...
    ``_try_import_shared_poke_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _poke_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No COUNCIL_HOME yet (pre-setup): fall through without locking.
        yield
        return

    with _file_lock(
        lock_path,
        _poke_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared Poke auth lock",
    ):
        yield


def _merge_shared_poke_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local Poke state."""
    shared = _read_shared_poke_state()
    if not shared:
        return False
...
def _write_shared_poke_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the Poke OAuth state to the shared store.
    ...
    try:
        with _poke_shared_store_lock():
            path = _poke_shared_store_path()
            path.parent.mkdir(parents=True, exist_ok=True)
            # secure_parent_dir refuses to chmod / or top-level dirs (#25821).
            secure_parent_dir(path)
            tmp = path.with_name(f"{path.name}.tmp.{os.getpid()}.{uuid.uuid4().hex}")
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Poke
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "poke_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared Poke auth store: %s", exc)


def _read_shared_poke_state() -> Optional[Dict[str, Any]]:
    """Return the shared Poke OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared Poke auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_poke_state(reason: str) -> None:
    """Remove the shared Poke OAuth store after a terminal token failure."""
    try:
        with _poke_shared_store_lock():
            path = _poke_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("poke_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared Poke auth store: %s", exc)


def _is_terminal_poke_refresh_error(exc: Exception) -> bool:
    """True when retrying the same Poke refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "poke"
...
def _quarantine_poke_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    _clear_shared_poke_state(reason)
    invalidate_poke_auth_status_cache()


def _quarantine_poke_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded Poke pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("poke")
    if not isinstance(entries, list):
        return False
    ...
    if removed:
        pool["poke"] = retained
        _oauth_trace(
            "poke_pool_device_code_quarantined",
            reason=reason,
...
def _try_import_shared_poke_state(
    *,
    timeout_seconds: float = 15.0,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate Poke OAuth state from the shared store.
    ...
    """
    try:
        with _poke_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_poke_state()
            if not shared:
                return None

            # Build a full state dict so refresh_poke_oauth_from_state has every
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_poke_state(updated_state)

            refreshed = refresh_poke_oauth_from_state(
                state,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_poke_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "poke_shared_import_failed",
            error_type=type(exc).__name__,
            error_code=getattr(exc, "code", None),
        )
        if _is_terminal_poke_refresh_error(exc):
            _clear_shared_poke_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared Poke import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "poke_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-poke-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="poke", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="poke", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the Poke portal
    ...
    # called POST /api/oauth/token with Council's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Council's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "Poke Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Council install sharing "
            "~/.council/auth.json) called POST /api/oauth/token with Council's "
            "refresh token without persisting the rotated token back.\n"
            "Poke refresh tokens are single-use — only Council may call the "
            "refresh endpoint. For health checks, use `council auth status` "
            "instead.\n"
            "Re-authenticate with: council auth add poke"
        )
        relogin = True

    raise AuthError(description, provider="poke", code=code, relogin_required=relogin)


def fetch_poke_models(
    *,
    inference_base_url: str,
    api_key: str,
    timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the Poke inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="poke", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Council models — they're not reliable for agentic tool-calling
            if "council" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _poke_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
def resolve_poke_access_token(
    *,
    ...
    """Resolve a refresh-aware Poke Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "poke")

        if not state:
            raise AuthError(
                "Council is not logged into Poke Portal.",
                provider="poke",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("COUNCIL_PORTAL_BASE_URL")
        ...
        with _poke_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_poke_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Poke Portal login.",
                    provider="poke",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state(auth_store, "poke", state)
                    _save_auth_store(auth_store)
                return access_token
...
def refresh_poke_oauth_pure(
    ...
    *,
    ...
    """Refresh Poke OAuth state without mutating auth.json directly.
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _poke_invoke_jwt_status(
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for Poke Portal.",
                    provider="poke",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
            state["scope"] = refreshed.get("scope") or state.get("scope")
            refreshed_url = _validate_poke_inference_url_from_network(refreshed.get("inference_base_url"))
            if refreshed_url:
        ...
        _assert_poke_inference_jwt_usable(state)
        _select_poke_invoke_jwt(state)

    return state


def refresh_poke_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
    ...
    """Refresh Poke OAuth from a state dict. Thin wrapper around refresh_poke_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_poke_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "council-cli"),
...
def persist_poke_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist Poke OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    Poke credentials are read at runtime from two independent locations:

    - ``providers.poke``: singleton state read by
      ``resolve_poke_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.poke``: used by the runtime ``pool.select()`` path.

    Historically ``council auth add poke`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.poke``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.poke`` then calls ``load_pool("poke")`` so
    ...
    ``council auth add poke --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("poke")`` instead of the
    ...
    # these credentials via `council auth add poke --type oauth`. Best-
...
def _sync_poke_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.poke changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("poke")
    except Exception as exc:
        logger.debug("Failed to sync Poke credential pool from auth store: %s", exc)


def resolve_poke_runtime_credentials(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """
    Resolve Poke inference credentials for runtime use.
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Council is not logged into Poke Portal.",
                            provider="poke", relogin_required=True)
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed Poke auth-status cache warm during read paths.
            if (
                _poke_effective_provider_state(state)
                == _poke_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "poke_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
        ...
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "poke_runtime_credentials_start",
            sequence_id=sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        )
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Poke Portal login.",
                                provider="poke", relogin_required=True)

            invoke_jwt_status = _poke_invoke_jwt_status(
                access_token,
                scope=state.get("scope"),
                expires_at=state.get("expires_at"),
            )
            if force_refresh or invoke_jwt_status is not None:
                with _poke_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    if _merge_shared_poke_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        invoke_jwt_status = _poke_invoke_jwt_status(
                            access_token,
            ...
            _assert_poke_inference_jwt_usable(
                state,
                access_token=access_token,
            )
            _select_poke_invoke_jwt(
                state,
                access_token=access_token,
                sequence_id=sequence_id,
            )
        ...
        _persist_state("resolve_poke_runtime_credentials_final")

    if state_persisted:
        _sync_poke_pool_from_auth_store()

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Poke inference API key",
                        provider="poke", code="server_error")
...
def _empty_poke_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_poke_pool_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if not pool or not pool.has_credentials():
            return _empty_poke_auth_status()
...
# ── Process-level memo for get_poke_auth_status() ──
# get_poke_auth_status() validates state by calling resolve_poke_runtime_credentials(),
# which does a synchropoke OAuth refresh POST to portal.pokelabs.com. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`council tools`, status panels,
# subscription-feature checks) call it many times per render — `council tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that `council auth login/logout/add/remove` invalidate naturally on the next call.
_NOUS_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_poke_auth_status_cache: Optional[Tuple[float, Optional[float], Dict[str, Any]]] = None
...
def invalidate_poke_auth_status_cache() -> None:
    """Clear the get_poke_auth_status() process-level memo.

    Call this from any code path that mutates Poke auth state without going
    through resolve_poke_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _poke_auth_status_cache
    _poke_auth_status_cache = None


def get_poke_auth_status() -> Dict[str, Any]:
    """Status snapshot for Poke auth.
    ...
    ``invalidate_poke_auth_status_cache()`` explicitly.
    """
    global _poke_auth_status_cache
    now = time.monotonic()
    mtime = _auth_file_mtime()
    cached = _poke_auth_status_cache
    ...
    status = _compute_poke_auth_status()
    _poke_auth_status_cache = (now, mtime, dict(status))
    return status


def _compute_poke_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_poke_auth_status(). See that function."""
    ...
    if state:
        ...
        try:
            ...
            refreshed_state = get_provider_auth_state("poke") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_poke_pool_status()


def get_codex_auth_status() -> Dict[str, Any]:
    """Status snapshot for Codex auth.
    
    Checks the credential pool first (where `council auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `council auth` and
    # `council model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("COUNCIL_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("COUNCIL_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``council doctor`` runs the live
    ...
    try:
        from council_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Council' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `council doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set COUNCIL_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `council logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"poke", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _prompt_model_selection(
    ...
    """
    from council_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from council_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'council model' or 'council setup' instead."""
    print("The 'council login' command has been removed.")
    print("Use 'council auth' to manage credentials,")
    print("'council model' to select a provider, or 'council setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.council/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Council-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Council auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Council will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("COUNCIL_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Council will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Council gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Council creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Council auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    from council_constants import display_council_home as _dhh
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Council auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Council creates its own local OAuth session)")
    print()
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in Council — please report at "
            "https://github.com/pokelabshq/council/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Council with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Council auth store (~/.council/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Council login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the Poke / xAI-OAuth / Codex-OAuth quarantine pattern
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `council model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``council status`` that just want to know
...
def _poke_device_code_login(
    *,
    ...
    """Run the Poke device-code flow and return full OAuth state without persisting."""
    pconfig = PROVIDER_REGISTRY["poke"]
    portal_base_url = (
        portal_base_url
        or os.getenv("COUNCIL_PORTAL_BASE_URL")
        or os.getenv("NOUS_PORTAL_BASE_URL")
    ...
    print(f"Starting Council login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        return refresh_poke_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `council model` again to finish setup.")
            raise SystemExit(1)
        raise


def _login_poke(args, pconfig: ProviderConfig) -> None:
    """Poke Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing Poke credential
        # from any other profile. If present, offer to rehydrate it.
        shared = _read_shared_poke_state()
        if shared:
            try:
                shared_path = _poke_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
                print(f"Found existing Poke OAuth credentials at {shared_path}")
            else:
                print("Found existing shared Poke OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating Poke session from shared credentials...")
                auth_state = _try_import_shared_poke_state(
                    timeout_seconds=timeout_seconds,
                )
        ...
        if auth_state is None:
            auth_state = _poke_device_code_login(
        ...
        # overwrites it to "poke".  If the user picks "Skip (keep current)"
        ...
        with _auth_store_lock():
            auth_store = _load_auth_store()
            _save_provider_state(auth_store, "poke", auth_state)
            saved_to = _save_auth_store(auth_store)
        ...
        _write_shared_poke_state(auth_state)
        _sync_poke_pool_from_auth_store()
        ...
        # leave the config in a half-updated state (provider=poke but model
        ...
        try:
            ...
            from council_cli.models import (
                get_curated_poke_model_ids, get_pricing_for_provider,
                check_poke_free_tier, partition_poke_models_by_tier,
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Council builds still see
        ...
        # them to Poke with a mismatched model.  The Poke OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "poke".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. Poke credentials saved for future use.")
            print("  Run `council model` again to switch to Poke Portal.")
            return

        config_path = _update_config_for_provider(
            "poke", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=poke)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Council will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `council model` or configure an API key to use Council.")
        else:
