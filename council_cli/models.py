...
Add, remove, or reorder entries here — both `council setup` and
`council` provider-selection will pick up the change automatically.
"""
...
from council_cli import __version__ as _COUNCIL_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_COUNCIL_USER_AGENT = f"council-cli/{_COUNCIL_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `council model`
    flow without maintaining a separate static list.
    """
    from council_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $COUNCIL_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $COUNCIL_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Council refreshes the cache from
...
# ---------------------------------------------------------------------------
# Poke Portal free-model helper
# ---------------------------------------------------------------------------
# The Poke Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Poke Portal account tier detection
# ---------------------------------------------------------------------------
def is_poke_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_poke_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split Poke models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/poke/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["poke"]`` list happens to contain or whether the
    ...
    older Council that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/poke/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["poke"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Council that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_poke_models_by_tier` keeps free models selectable;
...
def check_poke_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Poke Portal user is on a free (unpaid) tier.
    ...
    try:
        ...
        account_info = get_poke_portal_account_info(force_fresh=force_fresh)
...
# ---------------------------------------------------------------------------
# Poke Portal recommended models
#
...
NOUS_RECOMMENDED_MODELS_PATH = "/api/poke/recommended-models"
...
_poke_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def fetch_poke_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the Poke Portal's curated recommended-models payload.

    Hits ``<portal>/api/poke/recommended-models``. The endpoint is public —
    ...
    """
    base = (portal_base_url or "https://portal.pokelabs.com").rstrip("/")
    now = time.monotonic()
    cached = _poke_recommended_cache.get(base)
    ...
    _poke_recommended_cache[base] = (data, now)
    return data


def _resolve_poke_portal_url() -> str:
    ...
    try:
        ...
        state = get_provider_auth_state("poke") or {}
        portal = str(state.get("portal_base_url") or "").strip()
        if portal:
            return portal.rstrip("/")
        return str(DEFAULT_NOUS_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://portal.pokelabs.com"
...
def get_poke_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_poke_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_poke_portal_url()
    payload = fetch_poke_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_poke_free_tier()
        except Exception:
...
# this list:  council model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `council model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `council model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("poke",           "Poke Portal",              "Poke Portal (Poke Labs subscription)"),
...
#
# Some vendors expose several Council provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `council model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``council model``, the
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``council model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``council auth add openai-codex`` without
    ``council model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    pokelabs-council is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Poke Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from council_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — pokelabs-council
...
def get_curated_poke_model_ids() -> list[str]:
    """Return the curated Poke Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["poke"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from council_cli.model_catalog import get_curated_poke_models
        remote = get_curated_poke_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("poke", []))
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Poke Portal).
    """
    ...
    headers: dict[str, str] = {
        ...
        "User-Agent": _COUNCIL_USER_AGENT,
    }
    if api_key:
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.pokelabs.com"


def _resolve_poke_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Poke Portal pricing.

    The Poke inference ``/v1/models`` endpoint exposes pricing without
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, poke, novita)."""
    ...
    if normalized == "poke":
        api_key, base_url = _resolve_poke_pricing_credentials()
        if base_url:
            # Poke base_url typically looks like https://inference-api.pokelabs.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``council model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from council_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Poke, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"poke", "openrouter", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from council_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model poke` or `/model anthropic`, treat it as a
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Council' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``council_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Council should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``council auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.council/.env``.
    ...
    """
    try:
        from council_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from council_cli.auth import read_credential_pool
        from council_cli.copilot_auth import (
...
#   - "poke": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Poke),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Council release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Council release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "poke":
        # Try live Poke Portal /models endpoint
        try:
            from council_cli.auth import fetch_poke_models, resolve_poke_runtime_credentials
            creds = resolve_poke_runtime_credentials()
            if creds:
                live = fetch_poke_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["poke"] snapshot — so newly
        # added Portal models still surface without a Council release.
        manifest_ids = get_curated_poke_model_ids()
        if manifest_ids:
            return manifest_ids
...
#
# Cache strategy:
#   - One JSON file at $COUNCIL_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_council_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, poke portal), the
    relevant tokens live in ``$COUNCIL_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _COUNCIL_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Council' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _COUNCIL_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from council_constants import get_council_home
    return get_council_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Council will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Council cannot verify the model name."
