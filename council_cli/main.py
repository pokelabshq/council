...
Usage:
    council                     # Interactive chat (default)
    council chat                # Interactive chat
    council gateway             # Run gateway in foreground
    council gateway start       # Start gateway as service
    council gateway stop        # Stop gateway service
    council gateway status      # Show gateway status
    council gateway install     # Install gateway service
    council gateway uninstall   # Uninstall gateway service
    council setup               # Interactive setup wizard
    council logout              # Clear stored authentication
    council status              # Show status of all components
    council cron                # Manage cron jobs
    council cron list           # List cron jobs
    council cron status         # Check if cron scheduler is running
    council doctor              # Check configuration and dependencies
    council honcho setup                    # Configure Honcho AI memory integration
    council honcho status                   # Show Honcho config and connection status
    council honcho sessions                 # List directory → session name mappings
    council honcho map <name>               # Map current directory to a session name
    council honcho peer                     # Show peer names and dialectic settings
    council honcho peer --user NAME         # Set user peer name
    council honcho peer --ai NAME           # Set AI peer name
    council honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    council honcho mode                     # Show current memory mode
    council honcho mode [hybrid|honcho|local]  # Set memory mode
    council honcho tokens                   # Show token budget settings
    council honcho tokens --context N       # Set session.context() token cap
    council honcho tokens --dialectic N     # Set dialectic result char cap
    council honcho identity                 # Show AI peer identity representation
    council honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    council honcho migrate                  # Step-by-step migration guide: OpenClaw native → Council + Honcho
    council version             Show version
    council update              Update to latest version
    council uninstall           Uninstall Poke Council
    council acp                 Run as an ACP server for editor integration
    council sessions browse     Interactive session picker with search

    council claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: council_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``council_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``council update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``council_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, council crashes on import and the user can't run
# ``council update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import council_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'council' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``council tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``council.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("council")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"council", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"council")
        # Windows: the .exe name is already ``council.exe`` — nothing to do.
    except Exception:
        pass
...
# `entry.tsx`; this is just the earlier cousin. ``COUNCIL_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("COUNCIL_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not (os.environ.get("COUNCIL_TUI") == "1" or "--tui" in sys.argv[1:]):
        return
    try:
        # Skip when stdout is redirected (`council --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from council_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Poke Council v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``council --version`` before config/logging imports on Termux."""
    if os.environ.get("COUNCIL_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to COUNCIL_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (council tools, council setup, council model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'council {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any council module import.
#
# Many modules cache COUNCIL_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("COUNCIL_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.council/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set COUNCIL_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors council_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If COUNCIL_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.council/profiles/coder or
    # /opt/data/profiles/coder).  If COUNCIL_HOME points to the council root
    # instead (e.g. systemd hardcodes COUNCIL_HOME=/root/.council), we must
    # still read active_profile — the user may have switched profiles via
    # `council profile use` and the gateway should honour that choice.
    # See issue #22502.
    council_home_env = os.environ.get("COUNCIL_HOME", "")
    if profile_name is None and council_home_env:
        if Path(council_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the council root
    if profile_name is None:
        try:
            from council_constants import get_default_council_root

            active_path = get_default_council_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set COUNCIL_HOME
    if profile_name is not None:
        try:
            from council_cli.profiles import resolve_profile_env

            council_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent council from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["COUNCIL_HOME"] = council_home
...
# Load .env from ~/.council/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from council_cli.config import get_council_home
from council_cli.env_loader import load_council_dotenv

load_council_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → COUNCIL_REDACT_SECRETS env
# var BEFORE council_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_council_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "COUNCIL_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["COUNCIL_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `council` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from council_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from council_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if council_constants not importable yet
...
from council_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_council_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("COUNCIL_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("COUNCIL_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from council_cli.config import get_env_path, get_council_home, load_config
    from council_cli.auth import get_auth_status

    # Determine whether Council itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_council_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from council_cli.auth import PROVIDER_REGISTRY
    ...
    # Check for Poke Portal OAuth credentials
    auth_file = get_council_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Council has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Council to use their tokens.
    if _has_council_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, council_bin
        cli_args: the original CLI arguments (everything after 'council')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    council_bin = container_info["council_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo council {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo council {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  council --tui --resume {target}")
    if title:
        print(f'  council --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @council/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@council" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/council-ink/package.json",
    "packages/council-ink/package-lock.json",
    "packages/council-ink/index.js",
    "packages/council-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``COUNCIL_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("COUNCIL_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``COUNCIL_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("COUNCIL_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    council_home = os.environ.get("COUNCIL_HOME") or str(Path.home() / ".council")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "COUNCIL_HOME": council_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(council_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(council_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if council_cli_dir is None:
        council_cli_dir = Path(__file__).parent
    bundled = council_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (COUNCIL_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("COUNCIL_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from council_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("COUNCIL_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with COUNCIL_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset COUNCIL_TUI_DIR (e.g. `unset COUNCIL_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("COUNCIL_QUIET"):
    ...
    if tui_dev:
        # Keep the local @council/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @council/ink resolves through
        # packages/council-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "council-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from council_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="council-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["COUNCIL_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["COUNCIL_PYTHON_SRC_ROOT"] = os.environ.get(
        "COUNCIL_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("COUNCIL_PYTHON", sys.executable)
    env.setdefault("COUNCIL_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["COUNCIL_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["COUNCIL_MODEL"] = model
        env["COUNCIL_INFERENCE_MODEL"] = model
    if provider:
        env["COUNCIL_TUI_PROVIDER"] = provider
        env["COUNCIL_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["COUNCIL_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["COUNCIL_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["COUNCIL_TUI_SKILLS"] = value
    if query:
        env["COUNCIL_TUI_QUERY"] = query
    if image:
        env["COUNCIL_TUI_IMAGE"] = image
    if checkpoints:
        env["COUNCIL_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["COUNCIL_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["COUNCIL_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["COUNCIL_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["COUNCIL_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["COUNCIL_ACCEPT_HOOKS"] = "1"
    ...
    # COUNCIL_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `council --tui` try to
    ...
    # still set COUNCIL_TUI_RESUME themselves.
    env.pop("COUNCIL_TUI_RESUME", None)
    if resume_session_id:
        env["COUNCIL_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `council update` so
    ...
    if code == 42:
        from council_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``COUNCIL_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``council kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``council kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("COUNCIL_KANBAN_BOARD"):
        return
    try:
        from council_cli.kanban_db import get_current_board

        os.environ["COUNCIL_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("COUNCIL_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'council sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from council_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from council_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'council doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Council isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  council setup")
        print()

        from council_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'council setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from council_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["COUNCIL_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.council/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["COUNCIL_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["COUNCIL_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["COUNCIL_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from council_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from council_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from council_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Council?")
        print()
    ...
    # has no creds.json.  Every subsequent `council gateway` then paid a 30s
    ...
    session_dir = get_council_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: council gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `council gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  council gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Poke Council'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  council gateway")
        ...
        print()
        print("  Or install as a service: council gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'council whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from council_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from council_cli.config import stamp_install_method
    from council_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Council post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from council_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``council model``) and the setup wizard
    ...
    from council_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from council_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("COUNCIL_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from council_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'council model' for "
                "available providers, or run 'council doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in council_cli/models.py). A multi-member group shows one
    ...
    elif selected_provider == "poke":
        _model_flow_poke(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.council/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.council/.env if the active provider is not 'custom'.
    ...
    """
    from council_cli.config import get_env_value, save_env_value, load_config
...
#
# Council uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `council model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `council model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`council_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from council_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from council_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Council only falls back to a lightweight backend (OpenRouter,")
        print("  Poke Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``council model`` flow, then route aux tasks to them here.
    """
    from council_cli.config import load_config
    from council_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from council_cli.auth import _prompt_model_selection
    from council_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from council_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from council_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from council_constants import OPENROUTER_BASE_URL
    from council_cli.auth import (
        ProviderConfig,
    ...
    from council_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.council/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `council setup` from scratch. OpenRouter
    ...
    from council_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_poke(config, current_model="", args=None):
    """Poke Portal provider: ensure logged in, then pick model."""
    from council_cli.auth import (
    ...
    from council_cli.poke_subscription import prompt_enable_tool_gateway

    state = get_provider_auth_state("poke")
    if not state or not state.get("access_token"):
        print("Not logged into Poke Portal. Starting login...")
        print()
        try:
            ...
            _login_poke(mock_args, PROVIDER_REGISTRY["poke"])
            ...
            return
        # login_poke already handles model selection + config update
        return
    ...
    from council_cli.models import (
        get_curated_poke_model_ids,
        get_pricing_for_provider,
        check_poke_free_tier,
        partition_poke_models_by_tier,
        union_with_portal_free_recommendations,
        union_with_portal_paid_recommendations,
    )

    model_ids = get_curated_poke_model_ids()
    if not model_ids:
        print("No curated models available for Poke Portal.")
        return

    # Verify credentials are still valid (catches expired sessions early)
    try:
        creds = resolve_poke_runtime_credentials()
    except Exception as exc:
        relogin = isinstance(exc, AuthError) and exc.relogin_required
        msg = format_auth_error(exc) if isinstance(exc, AuthError) else str(exc)
        if relogin:
            print(f"Session expired: {msg}")
            print("Re-authenticating with Poke Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None,
                    inference_url=None,
    ...
    # Fetch live pricing (non-blocking — returns empty dict on failure)
    pricing = get_pricing_for_provider("poke")

    # Force fresh account data for model selection so recent credit purchases
    # are reflected immediately.
    free_tier = check_poke_free_tier(force_fresh=True)
    if not free_tier:
        try:
            refreshed_creds = resolve_poke_runtime_credentials(
                force_refresh=True,
            )
            if refreshed_creds:
    ...
    _poke_portal_url = ""
    try:
        _poke_state = get_provider_auth_state("poke")
        if _poke_state:
            _poke_portal_url = _poke_state.get("portal_base_url", "")
    except Exception:
        pass
    ...
    if free_tier:
        try:
            from council_cli.poke_account import (
                format_poke_portal_entitlement_message,
                get_poke_portal_account_info,
            )

            _account_info = get_poke_portal_account_info(force_fresh=True)
            unavailable_message = (
                format_poke_portal_entitlement_message(
                    _account_info,
                    capability="paid Poke models",
                )
                or ""
            )
        except Exception:
        ...
        model_ids, pricing = union_with_portal_free_recommendations(
            model_ids, pricing, _poke_portal_url,
        )
        model_ids, unavailable_models = partition_poke_models_by_tier(
    ...
    if not model_ids and not unavailable_models:
        print("No models available for Poke Portal after filtering.")
        return

    if free_tier and not model_ids:
        print("No free models currently available.")
        if unavailable_models:
            from council_cli.auth import DEFAULT_NOUS_PORTAL_URL

            _url = (_poke_portal_url or DEFAULT_NOUS_PORTAL_URL).rstrip("/")
    ...
    selected = _prompt_model_selection(
        ...
        portal_url=_poke_portal_url,
        unavailable_message=unavailable_message,
    )
    if selected:
        _save_model_choice(selected)
        # Reactivate Poke as the provider and update config
        inference_url = creds.get("base_url", "")
        _update_config_for_provider("poke", inference_url)
        ...
        save_config(config)
        print(f"Default model set to: {selected} (via Poke Portal)")
        # Offer Tool Gateway enablement for paid subscribers
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from council_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `council auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from council_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from council_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``council model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``council model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``council auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from council_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from council_cli.auth import _save_model_choice, deactivate_provider
    from council_cli.config import get_env_value, load_config, save_config
    from council_cli.secret_prompt import masked_secret_prompt
    ...
    from council_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `council model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from council_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Council URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from council_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from council_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from council_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Council will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Council will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from council_cli.curses_ui import flush_stdin
...
# downstream call sites read `council_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `council` invocation — including
# fast paths like `council --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from council_cli.config import save_env_value, load_config, save_config
    from council_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from council_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from council_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from council_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Council turns to `copilot --acp`.")
    print("  Council currently starts its own ACP subprocess for each request.")
    print("  Council uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set COUNCIL_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``council setup`` / ``council model``.
    ...
    recover from a malformed paste without editing ``~/.council/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from council_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from council_cli.config import save_env_value
    from council_cli.secret_prompt import masked_secret_prompt
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `council setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from council_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so council knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Council typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Council, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, Poke."
                )
                print()
    ...
    if provider_id == "lmstudio":
        ...
        from council_cli.models import fetch_lmstudio_models
        ...
        from council_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from council_constants import display_council_home as _dhh_fn

            print(
                f"    Council will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from council_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'council model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Council CLI with a provider."""
    from council_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from council_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from council_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from council_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from council_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from council_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``council slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: council slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `council slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from council_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from council_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from council_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from council_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from council_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from council_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from council_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from council_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Council home directory to a zip file."""
    if getattr(args, "quick", False):
        from council_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from council_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Council backup from a zip file."""
    from council_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # Show update status (synchropoke — acceptable since user asked for version info)
    try:
        from council_cli.banner import check_for_updates
        from council_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Poke Council."""
    _require_tty("uninstall")
    from council_cli.uninstall import run_uninstall
...
# Critical files that every ``council`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``council update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "council_cli/main.py",
    "council_cli/config.py",
    "council_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "council_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``council`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="council-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``council update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``council_cli/web_dist/`` (per vite.config.ts
    outDir: "../council_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "council_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``council`` launcher present but ``council_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``council update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``council web``).
    ...
    # build path usable on a stock `py -m council_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (council web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``council dashboard`` processes other than ourselves.

    ``council dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``council update`` replaces files on disk, the running
    ...
    patterns = [
        "council dashboard",
        "council_cli.main dashboard",
        "council_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "council.*dashboard"`) keeps us consistent
            # with `council_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `council update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``council update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``council update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: council curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``council dashboard`` processes.

    Called at the end of ``council update`` (default ``reason``) and also
    from ``council dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    council dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Poke Council by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `council update "
            f"--branch {branch}`, or update against main with `council update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/pokelabshq/council/archive/refs/heads/{branch}.zip"
    )
    ...
    # optional — a failure here only affects ``council dashboard``. Make
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "council-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Council behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes council completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Council couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Council couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Council behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `council update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/pokelabshq/council.git",
    "git@github.com:pokelabshq/council.git",
    "https://github.com/pokelabshq/council",
    "git@github.com:pokelabshq/council",
}
OFFICIAL_REPO_URL = "https://github.com/pokelabshq/council.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_council_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_council_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Council repository.")
        print("  This means you may miss updates from pokelabshq/council.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/pokelabshq/council.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/pokelabshq/council.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``council update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from council_constants import get_default_council_root

    default_home = get_default_council_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``council update`` is still progressing even if pip/uv itself is silent.
    """
...
def _council_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "council.exe",
        scripts_dir / "council-gateway.exe",
    ]


def _detect_concurrent_council_instances(
    ...
    The Council Desktop Electron app spawns ``council.EXE`` as a backend child,
    so during ``council update`` the user-invoked process and the desktop's
    ...
    shims (``council.exe`` / ``council-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``council update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``council.exe``) is a separate process from the Python
    ...
    for shim in _council_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated council.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``council update``
    ...
    #      council.exe sitting *under* a non-Council parent (e.g. a Council
...
def _format_concurrent_instances_message(
    ...
    lines = ["✗ Another council.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Council Desktop, exit any open `council` REPLs, and")
    lines.append("  stop the gateway (`council gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `council update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_council_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``council.exe``.
    ...
    when ``council update`` runs, ``council.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``council.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next council invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Council Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Council Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _council_exe_shims(scripts_dir):
        ...
        print(
            "    Close Council Desktop, exit other `council` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `council update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_council_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``council.exe.old.*`` left by prior updates.

    Called early on every council invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``council update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `council update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``council.exe`` / ``council-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_council_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_council_exe(scripts_dir)
        try:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from council_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.council/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``council update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.council/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``council update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``council update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.council/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``council update --gateway``) the update is already
    ...
    try:
        ...
        # council_cli.config.get_council_home to simulate setup failure.
        from council_cli.config import get_council_home as _get_council_home

        logs_dir = _get_council_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``council update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from council_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``council update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from council_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from council_cli.config import recommended_update_command
        from council_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``council update`` without requiring a reinstall.  The
    ...
    ``council`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/council.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v council'`` already resolves.  Idempotent.
    """
    ...
    path_comment = (
        "# Poke Council — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of COUNCIL_HOME before running the update.
    ...
    COUNCIL_HOME directories.  The ``--backup`` flag on ``council update``
    ...
    try:
        from council_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_council_home so the user sees ~/.council/...
    try:
        from council_constants import get_council_home, display_council_home

        home = get_council_home()
        try:
            display_path = f"{display_council_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  council import {out_path}")
...
def cmd_update(args):
    """Update Poke Council to the latest version.
    ...
    if is_managed():
        managed_error("update Poke Council")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `council update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Council via pip (for PyPI installs)."""
    from council_cli import __version__
    from council_cli.config import is_uv_tool_install
    ...
    print("✓ Update complete! Restart council to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Poke Council...")
    print()

    # On Windows, abort early if another council.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_council_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $COUNCIL_HOME, this is cheap
        ...
        try:
            from council_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in council_cli/config.py bricked
        # every user who ran ``council update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``council
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``council update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_council_home added to council_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload council_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_council_home() added since the last release.
        try:
            import importlib
            import council_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit COUNCIL_HOME so
        # it is not affected by sync_skills()'s module-level COUNCIL_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's COUNCIL_HOME env var points at the default or a named profile.
        try:
            from council_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: council config migrate")
            else:
                print()
                print("Skipped. Run 'council config migrate' later to configure.")
        else:
        ...
        try:
            from council_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `council curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``council update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from council_cli.tools_config import install_cua_driver
        ...
        # When running as ``council update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_council_home() / ".update_exit_code"
            try:
        ...
        try:
            from council_cli.gateway import (
                is_macos,
            ...
            # Discover all council-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. council-gateway.service or council-gateway-coder.service
                            ...
                            # path in `council gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: council gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: council -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Council gateway unit files are still installed.
        # When both council.service (from a pre-rename install) and the
        # current council-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `council update` surfaces the issue until the user migrates.
        try:
            from council_cli.gateway import (
                has_legacy_council_units,
                _find_legacy_council_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_council_units():
                print()
                print("⚠ Legacy Council gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_council_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (council.service) fight the current")
                print("  council-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    council gateway migrate-legacy")
                print()
        ...
        print("  council model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``council -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `council profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_council_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → council -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.council)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  council profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     council -p {name} chat")
                else:
        ...
        from council_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from council_constants import get_council_home as _hh
        ...
        from council_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `council profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from council_cli.profiles import profile_exists
        ...
        from council_cli.profiles import rename_profile
        ...
        from council_cli.profiles import export_profile
        ...
        from council_cli.profiles import import_profile
        ...
        import tempfile
        from council_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="council_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  council -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      council -p {plan.manifest.name} chat")
        ...
        from council_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `council profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"council -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from council_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("council_requires"):
            print(f"Requires:     Council {data['council_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from council_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.council_requires:
        print(f"  Requires: Council {mf.council_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``council dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``council dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No council dashboard processes running.")
        return 0

    print(f"{len(pids)} council dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No council dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `council update`.
    ...
    if "COUNCIL_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["COUNCIL_WEB_DIST"])
            if "COUNCIL_WEB_DIST" in os.environ
            else PROJECT_ROOT / "council_cli" / "web_dist"
        )
    ...
    # (e.g. plugins/dashboard_auth/poke) registers BEFORE start_server's
    ...
    try:
        from council_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    from council_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("COUNCIL_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from council_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from council_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Council log files."""
    from council_cli.logs import tail_log, list_logs
...
# so that in ``council -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``council_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``council -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``council`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("COUNCIL_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from council_cli.mcp_startup import start_background_mcp_discovery
    ...
    if _run_inline_mcp_discovery:
        try:
            # MCP tool discovery remains synchropoke for entrypoints that do
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if "-h" in argv or "--help" in argv:
        return False
    if os.environ.get("COUNCIL_TUI") == "1" or "--tui" in argv:
        return False
    ...
    from council_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from council_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["COUNCIL_DEFER_AGENT_STARTUP"] = "1"
            os.environ["COUNCIL_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["COUNCIL_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `council --tui` is the hot path on phones. The full parser setup imports
    ...
    wants_tui = os.environ.get("COUNCIL_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
    ...
    if getattr(args, "command", None) not in {None, "chat"}:
        return False
    if not (getattr(args, "tui", False) or os.environ.get("COUNCIL_TUI") == "1"):
        return False
...
def main():
    """Main entry point for council CLI."""
    # Cosmetic: make the process show up as 'council' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from council_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``council.exe.old.*`` quarantine files left by previous
    # ``council update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_council_exe``.
    try:
    ...
    model_parser.add_argument(
        "--portal-url",
        help="Portal base URL for Poke login (default: production portal)",
    )
    model_parser.add_argument(
        "--inference-url",
        help="Inference API base URL for Poke login (default: production inference API)",
    )
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Poke login (default: council-cli)",
    )
    model_parser.add_argument(
        "--scope", default=None, help="OAuth scope to request for Poke login"
    )
    model_parser.add_argument(
        "--no-browser",
        action="store_true",
        help="Do not attempt to open the browser automatically during Poke login",
    )
    model_parser.add_argument(
        "--manual-paste",
        action="store_true",
    ...
    model_parser.add_argument(
        ...
        type=float,
        default=15.0,
        help="HTTP request timeout in seconds for Poke login (default: 15)",
    )
    model_parser.add_argument(
        "--ca-bundle", help="Path to CA bundle PEM file for Poke TLS verification"
    )
    model_parser.add_argument(
        "--insecure",
        action="store_true",
        help="Disable TLS verification for Poke login (testing only)",
    )
    ...
    # =========================================================================
    from council_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `council model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    from council_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from council_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if COUNCIL_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy council.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Council gateway unit files "
            "(e.g. council.service) left over from older installs. Profile "
            "units (council-gateway-<profile>.service) and unrelated "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. Poke Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="poke",
        help="Upstream provider: poke or xai (default: poke). See `council proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Poke Council with an interactive wizard. "
        "Run a specific section: council setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'council setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    setup_parser.add_argument(
        "--portal",
        action="store_true",
        help="One-shot Poke Portal setup: log in via OAuth, set Poke as the "
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Council.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $COUNCIL_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Council")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from council_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Council CLI",
    )
    login_parser.add_argument(
        "--provider",
        choices=["poke", "openai-codex", "xai-oauth"],
        default=None,
        help="Provider to authenticate with (default: poke)",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: council-cli)"
    )
    ...
    logout_parser.add_argument(
        "--provider",
        choices=["poke", "openai-codex", "xai-oauth", "spotify"],
        default=None,
    ...
    auth_add.add_argument("--portal-url", help="Poke portal base URL")
    auth_add.add_argument("--inference-url", help="Poke inference base URL")
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Council with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set COUNCIL_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Poke Council components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.council/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Council profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.council/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Council profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    # portal command — Poke Portal status + Tool Gateway routing
    # =========================================================================
    from council_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from council_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.council/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.council/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Poke Council setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `council "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Council "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.council/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Council Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Council setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Poke Council. Use 'council debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    council debug share              Upload debug report and print URL
    council debug share --lines 500  Include more log lines
    council debug share --expire 30  Keep paste for 30 days
    council debug share --local      Print report locally (no upload)
    council debug share --no-redact  Disable upload-time secret redaction
    council debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'council debug share'",
    )
    ...
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/council-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.council/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo council uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from council_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Council backup from a zip file",
        description="Extract a previously created Council backup into your "
        "Council home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Poke Council configuration",
    )
    ...
    def cmd_pairing(args):
        from council_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.council/skills/.bundled_manifest) "
            "so future 'council update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from council_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from council_cli.skills_hub import skills_command
    ...
    from council_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/council-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `council plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from council_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``council --help``, ``council version``, ``council logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from council_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from council_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_council_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_council_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_council_home()}/memories/\n")
        else:
            from council_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'council tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # council tools list [--platform cli]
    ...
    # council tools disable <name...> [--platform cli]
    ...
    # council tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from council_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from council_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `council computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `council tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: council computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: council computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Council as an MCP server",
        description=(
            "Manage MCP server connections and run Council as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'council mcp add' to connect to a new server, or\n"
            "'council mcp serve' to expose Council conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Council as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `council mcp add ...` to fall through to
    # interactive chat.
    ...
    # ── Catalog (Poke-approved MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `council mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List Poke-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `council mcp install n8n`)",
    )
    ...
    def cmd_mcp(args):
        from council_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_council_home() / "sessions"
            ...
            # Launch council --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Council",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Council",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.council/ (by default a "
        "single restore-point archive is written to ~/.council/backups/ "
        "before apply; restorable with 'council import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from council_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Poke Council to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'council config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, council will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another council.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Poke Council",
        description="Remove Poke Council from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Poke Council as an ACP (Agent Client Protocol) server",
        description="Start Poke Council in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Council ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Council provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.council/node/ "
    ...
    def cmd_acp(args):
        """Launch Poke Council as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Council instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `council update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `council profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Council profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Poke Council web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `council --tui` via PTY/WebSocket). "
            "Alternatively set COUNCIL_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `council dashboard` cmdlines and SIGTERM them directly — the same
    # path `council update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running council dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running council dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Council log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    council logs                    Show last 50 lines of agent.log
    council logs -f                 Follow agent.log in real time
    council logs errors             Show last 50 lines of errors.log
    council logs gateway -n 100     Show last 100 lines of gateway.log
    council logs --level WARNING    Only show WARNING and above
    council logs --session abc123   Filter by session ID
    council logs --component tools  Only show tool-related lines
    council logs --since 1h         Lines from the last hour
    council logs --since 30m -f     Follow, starting from 30 min ago
    council logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``council -c Pokemon Agent Dev`` → ``council -c 'Pokemon Agent Dev'``
    ...
    from council_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'council -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (council hooks list, cron
