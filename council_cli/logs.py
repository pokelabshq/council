"""``council logs`` — view and filter Council log files.
...
Usage examples::
    ...
    council logs                    # last 50 lines of agent.log
    council logs -f                 # follow agent.log in real time
    council logs errors             # last 50 lines of errors.log
    council logs gateway -n 100    # last 100 lines of gateway.log
    council logs --level WARNING    # only WARNING+ lines
    council logs --session abc123   # filter by session ID substring
    council logs --component tools  # only tool-related lines
    council logs --since 1h         # lines from the last hour
    council logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_council_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Council runs — try 'council chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_council_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_council_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_council_home()}/logs/")
        return

    print(f"Log files in {display_council_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'council chat' to generate logs)")
