"""CLI handlers for the ``council proxy`` subcommand."""
...
from council_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "council proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "poke"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"council auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Council proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Council proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: council proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``council proxy <subcommand>``."""
    ...
    print(
        "council proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  council proxy start [--provider poke|xai] [--host 127.0.0.1] [--port 8645]\n"
