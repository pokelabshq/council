...
# xAI's public API is OpenAI-compatible for the endpoints Council commonly
# uses. The Responses endpoint is included because Council' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Council-managed OAuth credentials."""

    auth_hint = "council auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`council auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`council auth reset xai-oauth` or re-authenticate with "
                    "`council auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `council auth add xai-oauth --type oauth`."
            )
