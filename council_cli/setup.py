"""
Interactive setup wizard for Poke Council.
...
from council_cli.poke_subscription import get_poke_subscription_features
...
_DOCS_BASE = "https://pokelabs-council.pokelabs.com/docs"
...
# display_council_home imported lazily at call sites (stale-module safety during council update)
...
from council_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from council_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Council Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Council using environment variables or config commands:")
    print_info("  council config set model.provider custom")
    print_info("  council config set model.base_url http://localhost:8080/v1")
    print_info("  council config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'council setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from council_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from council_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'council setup')")


def _print_setup_summary(config: dict, council_home):
    ...
    tool_status = []
    subscription_features = get_poke_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'council setup' to configure"))
    ...
    if subscription_features.web.managed_by_poke:
        tool_status.append(("Web Search & Extract (Poke subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_poke:
        tool_status.append(("Browser Automation (Poke Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Poke), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_poke:
        tool_status.append(("Image Generation (Poke subscription)", True, None))
        ...
        try:
            ...
            from council_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `council tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_poke:
        tool_status.append(("Video Generation (FAL via Poke subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from council_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_poke:
        tool_status.append(("Text-to-Speech (OpenAI via Poke subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'council setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'council setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_poke:
        tool_status.append(("Modal Execution (Poke subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'council setup terminal'"))
    elif managed_poke_tools_enabled() and subscription_features.poke_auth_present:
        tool_status.append(("Modal Execution (optional via Poke subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via council auth spotify — check auth.json, not env vars)
    try:
        from council_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'council setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {council_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('council setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('council setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('council setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('council setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('council setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('council config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('council config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('council config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('council', Colors.GREEN)}              Start chatting")
    print(f"   {color('council gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('council doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `council tools` and `council setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``council model``)
    ...
    provider added to ``council model`` is automatically available here.
    ...
    # Delegate to the shared council model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: council model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from council_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Council can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Council can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'council setup' or configure AUXILIARY_VISION_* settings")


    # Tool Gateway prompt is already shown by _model_flow_poke() above.
    save_config(config)

    if not quick and selected_provider != "poke":
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``council model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from council_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_poke_tools_enabled() and subscription_features.poke_auth_present:
        choices.append("Poke Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("poke-openai")
    choices.extend(
        [
    ...
    selected_via_poke = selected == "poke-openai"
    if selected == "poke-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Poke gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.council/.env."
            )
    ...
    if selected == "neutts":
    ...
    elif selected == "openai" and not selected_via_poke:
        ...
        # via the Council auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via council setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'council setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Council runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        managed_modal_available = bool(
            ...
            and
            get_poke_subscription_features(config).poke_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Poke gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into COUNCIL_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("COUNCIL_MAX_ITERATIONS")
    ...
    print_info("  Run `council setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # COUNCIL_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Council delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `council update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://pokelabs-council.pokelabs.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under COUNCIL_HOME, and print
    ...
    """
    try:
        from council_cli.slack_cli import _build_full_manifest
        from council_constants import get_council_home

        manifest = _build_full_manifest(
            bot_name="Council",
            bot_description="Your Council agent on Slack",
        )
        target = Path(get_council_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `council slack manifest --write` anytime to refresh after "
            "Council adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Council delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Council to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from council_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://pokelabs-council.pokelabs.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://pokelabs-council.pokelabs.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  council config edit")
    print_info("   Open config in your editor:  council config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from council_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Council from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'council setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     council config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_council_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  council gateway")
        else:
            from council_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   council gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   council gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `council setup tools` and `council tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from council_cli.config
    ...
    if section_key == "model":
        ...
        from council_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_council.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_council migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_council", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Council.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Council to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Council at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Council at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Council at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Council at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Council equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Council config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Council.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Council's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(council_home: Path) -> bool:
    ...
    print_info("Council can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: council claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=council_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: council claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Council configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Council config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Council (use council claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Poke Portal setup — OAuth + provider switch + Tool Gateway.

    Wired into ``council setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``council setup --portal``)
    that gets a brand-new user from zero to a fully working Council session
    ...
    from council_cli.auth import get_poke_auth_status
    ...
    print(color("│     ⚕ Council Setup — Poke Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Poke Portal sub.")
    print()
    print_info("  Sign up: https://portal.pokelabs.com/manage-subscription")
    print()

    # Skip OAuth if already logged in (don't re-prompt every time the user
    # runs `council setup --portal` after a successful first run).
    already_logged_in = False
    try:
        already_logged_in = bool((get_poke_auth_status() or {}).get("logged_in"))
    except Exception:
        already_logged_in = False

    if already_logged_in:
        print_success("  Already logged into Poke Portal.")
    else:
        # Hand off to the shared auth wiring so the device-code flow is
        # identical to `council auth add poke --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print()
            print_error(f"  Poke Portal login failed (exit {e.code}).")
            print_info("  You can retry later with `council auth add poke --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
            ...
            print()
            print_error(f"  Poke Portal login failed: {exc}")
            print_info("  You can retry later with `council auth add poke --type oauth`.")
            return

    # Set provider → poke so the model picker, status surfaces, and
    # managed-tool gating all light up. Leave model.model empty so the
    # runtime picks Poke's default model; the user can change it later
    # with `council model`.
    ...
    save_config(config)
    print()
    print_success("  Poke set as your inference provider.")

    # Offer the Tool Gateway opt-in (single Y/n) — same flow that fires
    # from `council model` after picking Poke.
    print()
    try:
    ...
    print_info("  Run `council portal status` to inspect routing.")
    print_info("  Run `council` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      council setup           — full or quick (auto-detected)
      council setup model     — just model/provider
      council setup tts       — just text-to-speech
      council setup terminal  — just terminal backend
      council setup gateway   — just messaging platforms
      council setup tools     — just tool configuration
      council setup agent     — just agent settings
    """
    from council_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot Poke Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Council Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Poke Council Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Poke Council installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, council_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Council configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'council setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(council_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Council?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, council_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {council_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'council config edit'")
    ...
    _print_setup_summary(config, council_home)


def _run_first_time_quick_setup(config: dict, council_home, is_existing: bool):
    ...
    the user can customize later via ``council setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'council setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    council setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  council setup gateway")
    print()

    _print_setup_summary(config, council_home)


def _run_quick_setup(config: dict, council_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'council setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Council to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'council setup gateway'.")
