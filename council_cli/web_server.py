"""
Poke Council — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m council_cli.main web          # Start on http://127.0.0.1:9119
    python -m council_cli.main web --port 8080
"""
...
try:
    ...
    # running `council dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["COUNCIL_WEB_DIST"]) if "COUNCIL_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Poke Council", version=__version__)
...
_SESSION_HEADER_NAME = "X-Council-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``council dashboard --tui``
# or COUNCIL_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
#
# This list is defined in ``council_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from council_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from council_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    # and which providers are registered so ``council status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Poke Labs" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from council_cli.dashboard_auth import list_providers as _list_providers
...
# streamed to a per-action log file under ``~/.council/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_council_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "council-update": "council-update.log",
}
...
def _spawn_council_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``council <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``council_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "council_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "COUNCIL_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``council gateway restart`` in the background."""
    try:
        proc = _spawn_council_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/council/update")
async def update_council():
    """Kick off ``council update`` in the background."""
    try:
        proc = _spawn_council_action(["update"], "council-update")
    except Exception as exc:
        _log.exception("Failed to spawn council update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Council supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in council_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.council/config.yaml`` — applies to **new** sessions only.
...
# Anthropic, device-code for Poke/Codex) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``council auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Council resolves Anthropic creds in this order at runtime:
    1. ``~/.council/.anthropic_oauth.json`` — Council-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_council_oauth_credentials,
            read_claude_code_credentials,
            _COUNCIL_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_council_oauth_credentials = None  # type: ignore
        _COUNCIL_OAUTH_FILE = None  # type: ignore

    council_creds = None
    if read_council_oauth_credentials:
        try:
            council_creds = read_council_oauth_credentials()
        except Exception:
            council_creds = None
    if council_creds and council_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "council_pkce",
            "source_label": f"Council PKCE ({_COUNCIL_OAUTH_FILE})",
            "token_preview": _truncate_token(council_creds.get("accessToken")),
            "expires_at": council_creds.get("expiresAt"),
            "has_refresh_token": bool(council_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Council even
    when they also have a separate Council-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "council auth add anthropic",
    ...
    },
    {
        ...
        "name": "Poke Portal",
        "flow": "device_code",
        "cli_command": "council auth add poke",
        "docs_url": "https://portal.pokelabs.com",
        "status_fn": None,  # dispatched via auth.get_poke_auth_status
    },
    {
        "id": "openai-codex",
        "name": "OpenAI Codex (ChatGPT)",
        "flow": "device_code",
        "cli_command": "council auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "council auth add qwen-oauth",
    ...
    },
    {
        ...
        # as Poke's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "council auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from council_cli import auth as hauth
        if provider_id == "poke":
            raw = hauth.get_poke_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "poke_portal",
                "source_label": raw.get("portal_base_url") or "Poke Portal",
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("council_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Council-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _COUNCIL_OAUTH_FILE
            if _COUNCIL_OAUTH_FILE.exists():
                _COUNCIL_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from council_cli.auth import clear_provider_auth
...
#          → persists to ~/.council/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (Poke, OpenAI Codex):
#     1. POST /api/providers/oauth/{poke|openai-codex}/start
...
# Guarded so council web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Council file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``council auth add anthropic``.
    """
    from agent.anthropic_adapter import _COUNCIL_OAUTH_FILE
    payload = {
    ...
    _COUNCIL_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _COUNCIL_OAUTH_FILE.with_name(
        f"{_COUNCIL_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _COUNCIL_OAUTH_FILE)
        try:
            _COUNCIL_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "council-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    """Initiate a device-code flow (Poke, OpenAI Codex, or MiniMax).
    ...
    if provider_id == "poke":
        ...
        portal_base_url = (
            os.getenv("COUNCIL_PORTAL_BASE_URL")
        ...
        device_data, effective_scope = await asyncio.get_running_loop().run_in_executor(
            None, _do_poke_device_request
        )
        sid, sess = _new_oauth_session("poke", "device_code")
        ...
        threading.Thread(
            target=_poke_poller, args=(sid,), daemon=True, name=f"oauth-poll-{sid[:6]}"
        ).start()
        return {
            "session_id": sid,
            "flow": "device_code",
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to Poke's device-code
...
def _poke_poller(session_id: str) -> None:
    """Background poller that drives a Poke device-code flow to completion."""
    from council_cli.auth import (
        _poll_for_token,
    ...
    try:
        ...
        # Same post-processing as _poke_device_code_login (validate/refresh JWT)
        ...
        full_state = refresh_poke_oauth_from_state(
            auth_state,
            timeout_seconds=15.0,
            force_refresh=False,
        )
        from council_cli.auth import persist_poke_credentials
        persist_poke_credentials(full_state)
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: poke login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("poke device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_poke_poller` but calls the MiniMax-specific token endpoint,
    which uses a PKCE-style ``code_verifier`` + ``user_code`` rather than
    the ``device_code`` field used by Poke. On success, builds the same
    auth_state dict that ``_minimax_oauth_login`` (the CLI flow) builds
    and persists via ``_minimax_save_auth_state`` — so the dashboard
    path leaves the system in the same state as
    ``council auth add minimax-oauth``.
    """
    from council_cli.auth import (
        _minimax_poll_token,
...
async def get_logs(
    ...
    from council_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_council_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from council_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from council_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, COUNCIL_HOME)."""
    from council_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["council_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process COUNCIL_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_council_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "council setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from council_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from council_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from council_cli.tools_config import (
...
#
# The endpoint spawns the same ``council --tui`` binary the CLI uses, behind
...
try:
    from council_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_ok(ws: "WebSocket") -> bool:
    ...
    if auth_required:
        ...
        from council_cli.dashboard_auth.audit import AuditEvent, audit_log
...
def _resolve_chat_argv(
    ...
    Default: whatever ``council --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``COUNCIL_TUI_RESUME`` env var —
    matching what ``council_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``COUNCIL_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from council_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("COUNCIL_TUI_DISABLE_MOUSE", "1")
    env.setdefault("COUNCIL_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["COUNCIL_TUI_RESUME"] = resume

    if sidecar_url:
        env["COUNCIL_TUI_SIDECAR_URL"] = sidecar_url
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        ...
        from council_cli.dashboard_auth.ws_tickets import mint_ticket
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Council inside WSL2 to use the dashboard's /chat "
...
# COUNCIL_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`council_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from council_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/council/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /council`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__COUNCIL_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/council``)
        ...
        ``__COUNCIL_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__COUNCIL_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__COUNCIL_BASE_PATH__="{prefix}";'
                f"window.__COUNCIL_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__COUNCIL_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__COUNCIL_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__COUNCIL_BASE_PATH__="{prefix}";'
                f"window.__COUNCIL_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/council`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Council backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Council Teal",         "description": "Classic dark teal — the canonical Council look"},
    {"name": "default-large", "label": "Council Teal (Large)", "description": "Council Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.council/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.council/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_council_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.council/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as council_cli.plugins):
    1. User plugins:    ~/.council/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.council/plugins/  (only if COUNCIL_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from council_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_council_home() / "plugins", "user"),
    ...
    # ``council_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("COUNCIL_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".council" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from council_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_council_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"council auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from council_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from council_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from council_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from council_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.council/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.council/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"council_dashboard_plugin_{plugin['name']}"
...
from council_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        from council_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing COUNCIL_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Poke Council exposes a
            # module-level ``LAST_SKIP_REASON`` string for this purpose;
            # without it the operator would only see "no providers" which
            # is misleading when the provider IS installed but unconfigured.
            skip_reasons: list[str] = []
            try:
                from plugins.dashboard_auth import poke as _poke_plugin

                if _poke_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • poke: {_poke_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
            ...
            raise SystemExit(
                ...
                f"(was the dashboard_auth/poke plugin removed?).\n"
    ...
    print(f"  Council Web UI → http://{host}:{port}")
