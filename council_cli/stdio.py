...
on Japanese locales, etc.).  Council's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``council_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Council is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``COUNCIL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("COUNCIL_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Council-managed Git install directories so
    ...
    # already-running shells don't see the change — which means council
    ...
    # install without Council-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Council's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Council's
    ``council config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Council (User env var in Windows Settings, or
    export in a PowerShell profile) and Council picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Council-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\council\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``council`` from right after install) retain their old PATH.

    Any subprocess Council spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Council-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "council", "git", "cmd"),
        os.path.join(local_appdata, "council", "git", "bin"),
        os.path.join(local_appdata, "council", "git", "usr", "bin"),
