"""
Configuration management for Poke Council.

Config files are stored in ~/.council/ for easy access:
- ~/.council/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.council/.env         - API keys and secrets

This module provides:
- council config          - Show current configuration
- council config edit     - Open config in editor
- council config set      - Set a specific value
- council config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.council/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``council logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  council config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Council makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Council itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``council update``, the TUI build.
...
#   on every plugin install / ``council update``.
...
# * ``COUNCIL_HOME`` / ``COUNCIL_PROFILE`` / ``COUNCIL_CONFIG`` /
#   ``COUNCIL_ENV`` — Council runtime location flags. Writing these into
...
#
# IMPORTANT: ``COUNCIL_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (COUNCIL_GEMINI_CLIENT_ID,
# COUNCIL_LANGFUSE_PUBLIC_KEY, COUNCIL_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Council runtime location — never via dashboard env writer.
    # NOT a COUNCIL_* blanket: integration credentials (COUNCIL_GEMINI_*,
    # COUNCIL_LANGFUSE_*, COUNCIL_SPOTIFY_*, ...) ARE allowed.
    "COUNCIL_HOME", "COUNCIL_PROFILE", "COUNCIL_CONFIG", "COUNCIL_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Council runtime location "
            "(COUNCIL_HOME, COUNCIL_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.council/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `council plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "COUNCIL_LANGFUSE_ENV",
    "COUNCIL_LANGFUSE_RELEASE",
    "COUNCIL_LANGFUSE_SAMPLE_RATE",
    "COUNCIL_LANGFUSE_MAX_CHARS",
    "COUNCIL_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from council_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("COUNCIL_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_council_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Council is running in package-manager-managed mode.

    Two signals: the COUNCIL_MANAGED env var (set by the systemd service),
    or a .managed marker file in COUNCIL_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Council was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.council/.install_method`` file (written by installers)
    2. COUNCIL_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``pokelabshq/council`` image stamps ``docker``
    ...
    so ``council update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_council_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.council/.install_method."""
    stamp = get_council_home() / ".install_method"
    try:
...
# Long-form text for ``council update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# council_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Council, it doesn't update
...
✗ ``council update`` doesn't apply inside the Docker container.

Poke Council runs as a published image (pokelabshq/council), not a
...
Verify the new version after restart:
  docker run --rm pokelabshq/council:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/pokelabshq/council/tags
  • Your config and session history live under ``$COUNCIL_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``council update`` inside Docker.
...
def format_managed_message(action: str = "modify this Council installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("COUNCIL_MANAGED", "").strip().lower()
    ...
    return (
        f"Cannot {action}: this Council installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Council."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from COUNCIL_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, council_bin
    or None if container mode is not active, we're already inside the
    container, or COUNCIL_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("COUNCIL_DEV") == "1":
        return None
    ...
    container_mode_file = get_council_home() / ".container-mode"
...
# Re-export from council_constants — canonical definition lives there.
from council_constants import get_council_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_council_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_council_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the council group can
    share state with the gateway service.

    The mode can be overridden via the COUNCIL_HOME_MODE environment variable
    (e.g. COUNCIL_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse COUNCIL_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("COUNCIL_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Council runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("COUNCIL_CONTAINER") or os.environ.get("COUNCIL_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set COUNCIL_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into COUNCIL_HOME if the user doesn't have one yet."""
...
def ensure_council_home():
    """Ensure ~/.council directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_council_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_council_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"COUNCIL_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Council-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Council
        ...
        # identity slot (SOUL.md). Empty by default. The COUNCIL_ENVIRONMENT_HINT
        ...
        # blocks indefinitely (input() is synchropoke) and ignores this.
        "clarify_timeout": 600,
    ...
    "terminal": {
        ...
        # skipped. When empty, Council auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Council sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Council
        ...
        # bundled Council image, which drops to the `council` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Council sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Council should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``council chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.council/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: council sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Council truncates the
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | poke | codex | custom
        ...
        # Invoked by ``council kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``council kanban decompose`` and the kanban
        ...
        # ``council profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `council model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `council --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `council -c` muscle memory.  Default off so existing
        # users aren't surprised.  COUNCIL_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `council --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/council`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled Poke Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``COUNCIL_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``COUNCIL_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.pokelabs.com).
        "oauth": {
            "client_id": "",  # agent:{instance_id} — Portal provisions this
            "portal_url": "",  # blank → use plugin default (production Portal)
        },
        # Public URL override (env: ``COUNCIL_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/council``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.council/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.council/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, Poke, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    # response. If not, Council feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Council auto-pauses the goal and
    ...
    # always goes to ~/.council/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${COUNCIL_SKILL_DIR} and ${COUNCIL_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `council curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.council/skills/ into
        # ~/.council/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `council curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for council-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Poke Council*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (COUNCIL_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.council/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or COUNCIL_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `council doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``council_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Council to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via COUNCIL_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `council -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `council` launches with
        ...
        # decomposition is manual via `council kanban decompose <id>` or
    ...
    #
    # Core Council tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.council/logs/.
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.council/.env,
        ...
        # behavior: files must live under the Council cache, under
        ...
        # user. Bridged to COUNCIL_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Council
        # cache (~/.council/cache/{documents,images,audio,video,screenshots})
        ...
        # to COUNCIL_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # COUNCIL_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to COUNCIL_MEDIA_TRUST_RECENT_SECONDS.
        # Only consulted when ``strict`` is true.
        "trust_recent_files_seconds": 600,
    },

    # Session storage — controls automatic cleanup of ~/.council/state.db.
    ...
    "sessions": {
        ...
        # default of ``council sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.council/sessions/session_{sid}.json`` on every turn
    ...
    # ``council update`` behaviour.
    "updates": {
        # Run a full ``council backup``-style zip of COUNCIL_HOME before every
        # ``council update``.  Backups land in ``<COUNCIL_HOME>/backups/`` and
        # can be restored with ``council import <path>``.  Off by default —
        # on large COUNCIL_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<COUNCIL_HOME>/lsp/bin/`` on first use.
    ...
    # `council tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.council/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.council/.env (or your shell) and never in config.yaml.
            ...
            # ~/.council/bin/ on first use.  When False you must install
            ...
            # `council secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
# selection step (Poke Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "Poke Portal base URL override",
        "prompt": "Poke Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'council model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Poke Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Poke Subscribers only, used to derive vendor hosts, e.g. pokelabs.com -> firecrawl-gateway.pokelabs.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Poke Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Poke Subscriber access token for tool-gateway requests (optional; otherwise read from the Council auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @council:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. COUNCIL_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #council)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: council-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Council API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Council API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Council API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # COUNCIL_TOOL_PROGRESS and COUNCIL_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "COUNCIL_TOOL_PROGRESS": {
    ...
    "COUNCIL_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.council.config`` entries, then checks
    ...
    try:
        ...
        # should never break `council update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Council versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``council_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'council setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Council won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'council doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("COUNCIL_HOME", "~/.council")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("COUNCIL_TOOL_PROGRESS")
            old_mode = get_env_value("COUNCIL_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from COUNCIL_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from COUNCIL_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("COUNCIL_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from COUNCIL_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$COUNCIL_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_council_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `council plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `council curator status` has no
    ...
    #   3. Creates `~/.council/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_council_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_council_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: council config set <key> <value>")
    ...
    # metadata.council.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _council_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.council/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.council/config.yaml.
    ...
    (which change ``COUNCIL_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — council auth) — OpenAI Codex
#   poke         (OAuth — council auth) — Poke Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.council/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.council/.env.
    ...
    interactive menu render (`council tools`, `council setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `council tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Council env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.council/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.council/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.council/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.council/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Council — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Council vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.council/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Council Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  council config edit     # Edit config file", Colors.DIM))
    print(color("  council config set <key> <value>", Colors.DIM))
    print(color("  council setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: council config set <key> <value>")
            print()
            print("Examples:")
            print("  council config set model anthropic/claude-sonnet-4")
            print("  council config set terminal.backend docker")
            print("  council config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'council config migrate' to add them")
        ...
        print("  council config           Show current configuration")
        print("  council config edit      Open config in editor")
        print("  council config set <key> <value>   Set a config value")
        print("  council config check     Check for missing/outdated config")
        print("  council config migrate   Update config with new options")
        print("  council config path      Show config file path")
        print("  council config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``council config`` UI so users
