"""Normalized Poke Portal account entitlement helpers."""
...
PokeAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]

_ACCOUNT_INFO_CACHE_TTL = 60
_account_info_cache: tuple[str, float, "PokePortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class PokePortalSubscriptionInfo:
...
@dataclass(frozen=True)
class PokePaidServiceAccessInfo:
...
class PokePortalAccountInfo:
    logged_in: bool
    source: PokeAccountInfoSource
    fresh: bool
    user_id: Optional[str] = None
    org_id: Optional[str] = None
    client_id: Optional[str] = None
    product_id: Optional[str] = None
    poke_client: Optional[str] = None
    ...
    subscription: Optional[PokePortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[PokePaidServiceAccessInfo] = None
...
def poke_portal_billing_url(account_info: Optional[PokePortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Poke account snapshot."""
    try:
...
def format_poke_portal_entitlement_message(
    account_info: Optional[PokePortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
) -> Optional[str]:
    """Return user-facing guidance for a missing Poke paid entitlement.
    ...
    credits does not.
    """
    billing_url = poke_portal_billing_url(account_info)
    ...
    if account_info is None:
        return (
            f"Council could not verify your Poke Portal entitlement, so {capability} "
            f"is unavailable. Run `council model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Poke inference credentials are configured, but Council cannot verify "
                f"your Poke Portal paid access for {capability}. Log in with "
                f"`council model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Poke Portal to use {capability}: run `council model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Council could not verify your Poke Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `council model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Council could not find a Poke Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `council model` to "
            f"authenticate again; if the problem persists, contact Poke support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `council model` to refresh Council."
        return message

    return (
        f"Your Poke Portal account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your Poke Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Poke Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Poke Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Poke Portal account has no usable paid credits{credit_detail}, so "
...
def reset_poke_portal_account_info_cache() -> None:
    """Clear the short-lived account-info cache used by tests."""
    global _account_info_cache
    _account_info_cache = None


def get_poke_portal_account_info(
    *,
    force_fresh: bool = False,
    min_jwt_ttl_seconds: int = 60,
) -> PokePortalAccountInfo:
    """Return normalized Poke Portal account entitlement information.
...
def _fresh_account_info(
    *,
    ...
    try:
        from council_cli.auth import get_provider_auth_state, resolve_poke_access_token

        access_token = resolve_poke_access_token()
        ...
        payload = _fetch_poke_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[PokePortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque Poke keys."""
    try:
        entry = _select_poke_pool_entry()
        if entry is None:
            return None
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_poke_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _select_poke_pool_entry() -> Optional[Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from council_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = PokePaidServiceAccessInfo(
        allowed=paid_access,
    ...
    return PokePortalAccountInfo(
        ...
        poke_client=_coerce_str(claims.get("poke_client")),
        portal_base_url=portal_base_url,
...
def _subscription_from_payload(value: Any) -> Optional[PokePortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return PokePortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[PokePaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return PokePaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
