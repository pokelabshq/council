"""MCP picker — interactive `council mcp picker` (also the default `council mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``council mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `council plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from council_cli.cli_output import prompt_yes_no
from council_cli.curses_ui import curses_single_select
from council_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Council session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from council_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `council mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: council mcp install <name>    Picker: council mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Council is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Council — run `council update` "
...
def show_catalog() -> None:
    """`council mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`council mcp picker` (and default `council mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`council mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `council mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
