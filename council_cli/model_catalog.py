...
The Council docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Poke Portal). This module fetches, validates, and caches that manifest,
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/pokelabshq/council/main/website/static/api/model-catalog.json",
)
...
def _cache_path() -> Path:
    ...
    return get_council_home() / "cache" / "model_catalog.json"
...
def get_curated_poke_models() -> list[str] | None:
    """Return Poke Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("poke")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``council model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
