"""``council portal`` — small CLI surface for Poke Portal users.
...
already in ``council auth`` or ``council tools``. It's a discovery + status
...
def _cmd_status(args) -> int:
    ...
    print()
    print(color("  Poke Portal", Colors.MAGENTA))
    ...
    if logged_in:
        ...
        print(f"  Login:   council auth add poke --type oauth")

    # Provider selection (independent of auth)
    model_cfg = config.get("model") if isinstance(config.get("model"), dict) else {}
    provider = str(model_cfg.get("provider") or "").strip().lower()
    if provider == "poke":
        print(f"  Model:   {color('✓ using Poke as inference provider', Colors.GREEN)}")
    elif provider:
        print(f"  Model:   currently {provider} (switch with `council model`)")
    ...
    for feat in features.items():
        if feat.managed_by_poke:
            state = color("via Poke Portal", Colors.GREEN)
...
def _cmd_tools(args) -> int:
    ...
    if not features.poke_auth_present:
        print(color("  Not logged into Poke Portal — sign in with `council auth add poke --type oauth`.", Colors.YELLOW))
        print()
...
def portal_command(args) -> int:
    """Top-level dispatch for `council portal <subcommand>`."""
    ...
    print("Run `council portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `council portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Poke Portal status, subscription, and Tool Gateway routing",
        description=(
            "Inspect Poke Portal auth, Tool Gateway routing, and open the "
    ...
    portal_sub.add_parser(
        "tools",
        help="List Tool Gateway tools and which are routed via Poke",
    )
