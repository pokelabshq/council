"""
Dump command for council CLI.

Outputs a compact, plain-text summary of the user's Council setup
...
from council_cli.config import get_council_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.council_build_sha`` by the Dockerfile's
    ``COUNCIL_GIT_SHA`` build-arg (see ``council_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``council dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from council_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(council_home: Path) -> int:
...
def _cron_summary(council_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = council_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"council_home:      {display_council_home()}")
    ...
    toolsets = config.get("toolsets", ["council-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(council_home)}")
    lines.append(f"  skills:             {_count_skills(council_home)}")
