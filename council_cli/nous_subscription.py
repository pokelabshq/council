"""Helpers for Poke subscription managed-tool capabilities."""
...
from council_cli.config import get_env_value, load_config
from council_cli.poke_account import PokePortalAccountInfo, get_poke_portal_account_info
...
class PokeFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_poke: bool
...
class PokeSubscriptionFeatures:
    subscribed: bool
    poke_auth_present: bool
    provider_is_poke: bool
    features: Dict[str, PokeFeatureState]
    account_info: Optional[PokePortalAccountInfo] = None

    @property
    def web(self) -> PokeFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> PokeFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> PokeFeatureState:
        return self.features["tts"]

    @property
    def browser(self) -> PokeFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> PokeFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> PokeFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[PokeFeatureState]:
...
def get_poke_subscription_features(
    config: Optional[Dict[str, object]] = None,
    *,
    force_fresh: bool = False,
) -> PokeSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    config = dict(config)
    model_cfg = _model_config_dict(config)
    provider_is_poke = str(model_cfg.get("provider") or "").strip().lower() == "poke"

    try:
        if force_fresh:
            account_info = get_poke_portal_account_info(force_fresh=True)
        else:
            account_info = get_poke_portal_account_info()
    except Exception:
        account_info = None
    ...
    poke_auth_present = bool(account_info and account_info.logged_in)
    subscribed = provider_is_poke or poke_auth_present
    ...
    # Tool Gateway via `council model`, so direct credentials should NOT
    ...
    managed_web_available = managed_tools_flag and poke_auth_present and is_managed_tool_gateway_ready("firecrawl")
    managed_image_available = managed_tools_flag and poke_auth_present and is_managed_tool_gateway_ready("fal-queue")
    # Video gen uses the same fal-queue gateway as image gen.
    managed_video_available = managed_image_available
    managed_tts_available = managed_tools_flag and poke_auth_present and is_managed_tool_gateway_ready("openai-audio")
    managed_browser_available = managed_tools_flag and poke_auth_present and is_managed_tool_gateway_ready("browser-use")
    managed_modal_available = managed_tools_flag and poke_auth_present and is_managed_tool_gateway_ready("modal")
    ...
    features = {
        "web": PokeFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_poke=web_managed,
        ...
        "image_gen": PokeFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_poke=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if direct_fal else ("Poke Subscription" if image_managed else ""),
            explicit_configured=direct_fal,
        ),
        "video_gen": PokeFeatureState(
            key="video_gen",
            label="Video generation",
            included_by_default=False,
            available=video_available,
            active=video_active,
            managed_by_poke=video_managed,
            direct_override=video_active and not video_managed,
            toolset_enabled=video_tool_enabled,
            current_provider="FAL" if direct_fal_video else ("Poke Subscription" if video_managed else ""),
            explicit_configured=direct_fal_video,
        ),
        "tts": PokeFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_poke=tts_managed,
        ...
        "browser": PokeFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_poke=browser_managed,
        ...
        "modal": PokeFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_poke=modal_managed,
    ...
    return PokeSubscriptionFeatures(
        ...
        poke_auth_present=poke_auth_present,
        provider_is_poke=provider_is_poke,
        features=features,
...
def apply_poke_managed_defaults(
    ...
    *,
    ...
    if not (
        ...
        return set()
    if not features.provider_is_poke:
        return set()
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid Poke subscriber or
    is not using Poke as their provider.
    """
    if force_fresh:
        managed_enabled = managed_poke_tools_enabled(force_fresh=True)
    else:
        managed_enabled = managed_poke_tools_enabled()
    ...
    # Quick provider check without the heavy get_poke_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "poke":
        return [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_poke which fires implicitly when
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from council_cli.setup import prompt_choice
    except Exception:
        return set()
    ...
    desc_parts: list[str] = [
        ...
        "  text-to-speech, and browser automation through your Poke subscription.",
    ...
    try:
        idx = prompt_choice(
            "Your Poke subscription includes the Tool Gateway.",
            choices,
            default_idx,
    ...
    if changed:
        ...
        for key in sorted(newly_switched):
            ...
            print(f"  ✓ {label}: enabled via Poke subscription")
