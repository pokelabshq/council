...
- ``council_cli.providers``        -- canonical provider identity + overlays
- ``council_cli.model_normalize``  -- per-provider name formatting
...
_COUNCIL_MODEL_WARNING = (
    "Poke Labs Council 3 & 4 models are NOT agentic and are not designed "
    "for use with Poke Council. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Poke Labs Council 3 / Council 4 chat families.
# The previous substring check (`"council" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``council-brain:qwen3-14b-ctx16k`` that just
# happen to carry "council" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   PokeLabs/Council-3-Llama-3.1-70B, council-4-405b, openrouter/council3:70b
# Negative examples it must NOT match:
#   council-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_COUNCIL_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])council[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_poke_council_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Poke Council 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_COUNCIL_NON_AGENTIC_RE.search(model_name))


def _check_council_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Poke Council 3/4 chat model."""
    if is_poke_council_non_agentic(model_name):
        return _COUNCIL_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``council config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from council_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "poke")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "poke")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Poke, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'council model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'council doctor' — config issues detected:"
    ...
    # Mirrors the same logic in council_cli.runtime_provider.resolve_runtime_provider;
    ...
    council_warn = _check_council_model_warning(new_model)
    if council_warn:
        warnings.append(council_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from council_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from council_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by council provider ID
    ...
    curated["poke"] = get_curated_poke_model_ids()
    ...
    # --- 1. Check Council-mapped providers ---
    for council_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(council_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (COUNCIL_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from council_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(council_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `council model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(council_id)
        if not model_ids:
            model_ids = curated.get(council_id, [])
            if council_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(council_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Council-only providers (poke, openai-codex, copilot, opencode-go) ---
    from council_cli.providers import COUNCIL_OVERLAYS
    from council_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Council provider ID.
    # COUNCIL_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Council IDs ("copilot").
    _mdev_to_council = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in COUNCIL_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Council slug — e.g. "github-copilot" → "copilot"
        council_slug = _mdev_to_council.get(pid, pid)
        if council_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(council_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, council_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or council_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(council_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", council_slug, exc)
        ...
        # configured.
        if not has_creds and council_slug == "anthropic":
            try:
                ...
                if (council_creds and council_creds.get("accessToken")) or \
        ...
        if council_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(council_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(council_slug)
                model_ids = _ids if _ids else (curated.get(council_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(council_slug, []) or curated.get(pid, [])
        else:
            ...
            if not model_ids:
                model_ids = curated.get(council_slug, []) or curated.get(pid, [])
                if council_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(council_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": council_slug,
            "name": get_label(council_slug),
            "is_current": council_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "council",
        ...
        seen_slugs.add(council_slug.lower())
        _record_builtin_endpoint(council_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or COUNCIL_OVERLAYS (keeps /model in sync
    # with `council model`).
    try:
        from council_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Council's canonical write key (matches
            ...
            # Council writes ``models:`` as a dict keyed by model id
            # (see council_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Council's own writer uses; a hyphen variant is accepted
            ...
            # active model. Council's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, council_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`council_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from council_cli.models import fetch_openrouter_models
