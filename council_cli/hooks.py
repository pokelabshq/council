"""council hooks — inspect and manage shell-script hooks.

Usage::

    council hooks list
    council hooks test <event> [--for-tool X] [--payload-file F]
    council hooks revoke <command>
    council hooks doctor

Consent records live under ``~/.council/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.council/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``council hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: council hooks {list|test|revoke|doctor}")
        print("Run 'council hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.council/config.yaml.")
        print("See `council hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `council hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `council hooks test` and `council hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Council wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `council hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `council hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `council hooks doctor`.")
