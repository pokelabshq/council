...
``$COUNCIL_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-council-setup
(the stage2 hook) has chowned the volume and seeded $COUNCIL_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$COUNCIL_HOME``,
    not under ``profiles/``). The dispatcher in ``council_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``council gateway start`` (no ``-p``) targets. Without it,
    bare ``council gateway start`` inside the container would land on
    ...
    ``$COUNCIL_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        council_home: The container's COUNCIL_HOME (typically /opt/data).
            Profiles live under ``<council_home>/profiles/<name>/``;
            the default profile lives at ``<council_home>`` itself.
    ...
    # named profiles).
    default_prior_state = _read_prior_state(council_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(council_home)
    ...
    profiles_root = council_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `council profile create` (config.yaml
            # is not — that comes later via `council setup`). Use it as the
            ...
            # via ``council -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(council_home, actions)
    return actions
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `council -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit council
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the council user) won't hit EACCES. See
...
def _write_reconcile_log(
    council_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $COUNCIL_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    council_home = Path(os.environ.get("COUNCIL_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        council_home=council_home, scandir=scandir,
    )
    for a in actions:
        print(
