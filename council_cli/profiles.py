"""
Profile management for multiple isolated Council instances.

Each profile is a fully independent COUNCIL_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.council/profiles/<name>/`` by default.

The "default" profile is ``~/.council`` itself — backward compatible,
zero migration needed.

Usage::

    council profile create coder          # fresh profile + bundled skills
    council profile create coder --clone  # also copy config, .env, SOUL.md, skills
    council profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    council -p coder chat                 # or via flag
    council profile use coder             # set as sticky default
    council profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See council_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.council``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   pokelabs-council  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `council profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `council
...
# `council skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Council infrastructure directories that only the default profile
         (``~/.council``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_council_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.council) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "pokelabs-council",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "council", "default", "test", "tmp", "root", "sudo",
})

# Council subcommands that cannot be used as profile names/aliases
_COUNCIL_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the council root, NOT to the current COUNCIL_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where COUNCIL_HOME points outside
    ``~/.council``, profiles live under ``COUNCIL_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_council_home() / "profiles"


def _get_default_council_home() -> Path:
    """Return the default (pre-profile) COUNCIL_HOME path.

    In standard deployments this is ``~/.council``.
    In Docker/custom deployments where COUNCIL_HOME is outside ``~/.council``
    (e.g. ``/opt/data``), returns COUNCIL_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_council_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Council installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its COUNCIL_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_council_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, council subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _COUNCIL_SUBCOMMANDS:
        return f"'{canon}' conflicts with a council subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``council profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Council config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_council_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``council update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.council)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.council/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `council update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`council profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `council update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `council profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `council -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches COUNCIL_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``council profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "COUNCIL_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.council).\n"
            "To remove everything, use: council uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from council_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set COUNCIL_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("COUNCIL_HOME")
    try:
        os.environ["COUNCIL_HOME"] = str(profile_dir)
        from council_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["COUNCIL_HOME"] = old_home
        elif "COUNCIL_HOME" in os.environ:
            del os.environ["COUNCIL_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.council/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: council profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from COUNCIL_HOME.

    Returns ``"default"`` if COUNCIL_HOME is not set or points to ``~/.council``.
    Returns the profile name if COUNCIL_HOME points into ``~/.council/profiles/<name>``.
    Returns ``"custom"`` if COUNCIL_HOME is set to an unrecognized path.
    """
    from council_constants import get_council_home
    council_home = get_council_home()
    resolved = council_home.resolve()

    default_resolved = _get_default_council_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.council itself — its parent is ~/ and its
        # directory name is ".council", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: council profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.council itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.council). "
            "Specify a different name: council profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="council_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"council_{old_name}"
    legacy_old_host = f"council.{old_name}"
    new_host = f"council_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_council_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("council_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a COUNCIL_HOME path string.

    Called early in the CLI entry point, before any council modules
    are imported, to set the COUNCIL_HOME environment variable.
    """
