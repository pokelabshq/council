...
Pure logic: walks a Council config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `council doctor` and a future `council migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Council config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Council config and return retirement issues.
