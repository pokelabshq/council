...
Used by ``council profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``council_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Poke Council kanban board.
...
Rules:
  ...
  - Never write "Poke Council profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_council_home())
        else:
