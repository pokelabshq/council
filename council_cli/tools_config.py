"""
Unified tool configuration for Poke Council.

`council tools` and `council setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.council/config.yaml under
...
from council_cli.poke_subscription import (
    apply_poke_managed_defaults,
    get_poke_subscription_features,
)
from council_cli.poke_account import format_poke_portal_entitlement_message
...
from council_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _COUNCIL_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `council tools` → Video Generation, which walks
...
# `council tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from council_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `council tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``council tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from council_cli.plugins import discover_plugins, get_plugin_toolsets
...
from council_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            {
                "name": "Poke Subscription",
                "badge": "subscription",
                "tag": "Managed OpenAI TTS billed to your subscription",
                "env_vars": [],
                "tts_provider": "openai",
                "requires_poke_auth": True,
                "managed_poke_feature": "tts",
    ...
    "web": {
        ...
        #   - "Poke Subscription" — managed Firecrawl billed via Poke
        #     subscription (requires_poke_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        #   - "Poke Subscription" — managed FAL billed via the Poke
        #     subscription (requires_poke_auth + override_env_vars).
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_poke_auth": True,
                "managed_poke_feature": "image_gen",
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "Poke Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Poke Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_poke_auth": True,
                "managed_poke_feature": "video_gen",
                "override_env_vars": ["FAL_KEY"],
                # The underlying plugin backend — when the user picks
                # "Poke Subscription" we set video_gen.provider = "fal"
    ...
    "x_search": {
        ...
        "setup_note": (
            "Council routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        #   - "Poke Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Poke subscription (requires_poke_auth +
        ...
        "providers": [
            {
                "name": "Poke Subscription (Browser Use cloud)",
                "badge": "subscription",
                "tag": "Managed Browser Use billed to your subscription",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_poke_auth": True,
                "managed_poke_feature": "browser",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. COUNCIL_CUA_DRIVER_VERSION is an
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("COUNCIL_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``council update`` and
      by ``council computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `council update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Council process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/pokelabshq/council:latest"
            )
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `council update` does not silently pull
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.council/config.yaml")
        ...
        # Run the full `council auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.council/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from council_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: council auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `council auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: council auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: council auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `council auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: council auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: council auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "council-cli" (which include all _COUNCIL_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[council-cli, spotify]`` after enabling Spotify via ``council
        ...
        # (e.g. "council-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``council tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `council tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"council-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("council-")}
    ...
    # shipped as a bundled plugin but user must opt in via `council tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `council tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by council tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (council-cli, council-telegram, etc.)
    ...
    # Opening `council tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "browser"}:
        features = get_poke_subscription_features(config, force_fresh=force_fresh)
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_poke):
            return True
...
# ("Poke Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Poke Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    for provider in cat.get("providers", []):
        if provider.get("managed_poke_feature") and not managed_available:
            continue
        if provider.get("requires_poke_auth") and not features.poke_auth_present:
            continue
        visible.append(provider)

    # Inject plugin-registered image_gen backends (OpenAI today, more
    # later) so the picker lists them alongside FAL / Poke Subscription.
    ...
    # remaining hardcoded rows ("Poke Subscription", "Firecrawl
    ...
    # the hardcoded "Poke Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_poke_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Return a reason when a category's Poke provider is hidden."""
    features = get_poke_subscription_features(config, force_fresh=force_fresh)
    managed_available = bool(
        features.account_info
        and features.account_info.logged_in
    ...
    if managed_available:
        return ""
    if not any(p.get("managed_poke_feature") for p in cat.get("providers", [])):
        return ""
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `council tools` gets a silent no-op
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_poke_message = _hidden_poke_gateway_message(
        cat,
        config,
        f"the Poke Subscription provider for {name}",
    ...
    if len(providers) == 1:
        ...
        if hidden_poke_message:
            for line in hidden_poke_message.splitlines():
        ...
        # When the user is logged into Poke, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Poke.
        try:
            _poke_logged_in = bool(
                get_poke_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).poke_auth_present
        ...
        for p in providers:
            ...
            # Highlight Poke-managed entries when the user has Portal auth.
            ...
            # picker unchanged.
            sub_marker = ""
            if _poke_logged_in and p.get("managed_poke_feature"):
                sub_marker = "  ★ Included with your Poke subscription"
...
def _is_provider_active(
    ...
    *,
    ...
    if video_plugin_name and not provider.get("managed_poke_feature"):
        video_cfg = config.get("video_gen", {})
        return isinstance(video_cfg, dict) and video_cfg.get("provider") == video_plugin_name

    managed_feature = provider.get("managed_poke_feature")
    if managed_feature:
        features = get_poke_subscription_features(config, force_fresh=force_fresh)
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if image_cfg.get("use_gateway") is not None and not is_truthy_value(image_cfg.get("use_gateway"), default=False):
                    return False
            return feature.managed_by_poke
        ...
        if provider.get("tts_provider"):
            return (
                feature.managed_by_poke
                and cfg_get(config, "tts", "provider") == provider["tts_provider"]
            )
        if "browser_provider" in provider:
            current = cfg_get(config, "browser", "cloud_provider")
            return feature.managed_by_poke and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return feature.managed_by_poke and current == provider["web_backend"]
        return feature.managed_by_poke
...
def _configure_provider(
    ...
    *,
    ...
    if provider.get("requires_poke_auth"):
        features = get_poke_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.poke_auth_present or not entitled:
            message = format_poke_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'Poke Subscription')}",
            )
            _print_warning(
                f"  {message or 'Poke Subscription is only available after logging into Poke Portal.'}"
            )
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Poke subscription.")
    ...
    # Poke-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Poke.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_poke_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_poke_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_poke_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.poke_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through Poke Portal subscription.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `council tools` and `council setup tools`.
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_poke_managed_defaults(
                config,
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Poke subscription defaults", Colors.GREEN))
    ...
    print()
    from council_constants import display_council_home
    print(color(f"  Tool configuration saved to {display_council_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'council' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, council mcp
