...
from ``council_constants`` or other council_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$COUNCIL_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``council_constants.get_council_home`` semantics: env var wins,
    else ``~/.council``. A local copy avoids an import cycle with the
    middleware which lives below ``council_cli``.
    """
    home = os.environ.get("COUNCIL_HOME") or str(Path.home() / ".council")
