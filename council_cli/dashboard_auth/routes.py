...
from council_cli.dashboard_auth.base import (
    InvalidCodeError,
    ProviderError,
)
from council_cli.dashboard_auth.cookies import (
    clear_pkce_cookie,
...
from council_cli.dashboard_auth.login_page import render_login_html
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``COUNCIL_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /council`` (Mission Control deploys) — we
    ...
    from council_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``council_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from council_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("council_session_pkce", "")
...
async def api_auth_ws_ticket(request: Request):
    ...
    from council_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
