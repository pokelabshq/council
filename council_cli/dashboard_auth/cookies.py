...
Three cookies in play:
  - council_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL)
  - council_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 30 days)
                         **DEPRECATED in OAuth contract v1** — Poke Portal
  ...
  - council_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/council) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/council`` covers
...
setter wrote ``__Secure-council_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "council_session_at"
SESSION_RT_COOKIE = "council_session_rt"
PKCE_COOKIE = "council_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /council`` we want ``Path=/council`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    SKIPPED when empty — Poke Portal contract v1 issues no refresh tokens
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/council``)
