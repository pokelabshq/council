"""`council checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.council/checkpoints/``.  Actions:

    council checkpoints               # same as `status`
    council checkpoints status        # total size, project count, breakdown
    council checkpoints list          # per-project checkpoint counts + workdir
    council checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    council checkpoints clear [-f]    # nuke the entire base (asks first)
    council checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    council checkpoints
    council checkpoints prune --retention-days 3 --max-size-mb 200
    council checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: council checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``council checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `council checkpoints` → status
