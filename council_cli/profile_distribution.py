"""Profile distributions — shareable, packaged Council profiles via git.

A distribution is a Council profile published as a git repository (or
...
* ``council profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``council skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``council profile``, not a parallel tree):

    council profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "council_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".council_history",
    # User data
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            council_requires=str(data.get("council_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.council_requires:
            out["council_requires"] = self.council_requires
        if self.author:
...
def check_council_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Council {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``council profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Council profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Council distribution."
        )

    # Version check up-front so we fail fast
    check_council_requires(manifest.council_requires, council_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.council).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="council_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `council profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`council profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="council_dist_update_") as tmp:
        plan = plan_install(
