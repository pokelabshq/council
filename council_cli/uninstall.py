"""
Poke Council Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.council/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Council PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                ...
                # Remove any PATH line containing council
                if 'council' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the council wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "council",
        Path("/usr/local/bin/council"),
    ]
...
def remove_node_symlinks(council_home: Path) -> list:
    ...
    creates::
        ...
        ~/.local/bin/node -> $COUNCIL_HOME/node/bin/node
        ~/.local/bin/npm  -> $COUNCIL_HOME/node/bin/npm
        ~/.local/bin/npx  -> $COUNCIL_HOME/node/bin/npx
    ...
    when the link still resolves into this Council home's ``node`` directory.
    A link the user has since repointed at nvm (or anything else outside
    Council) is left untouched so we never break unrelated tooling.
    """
    node_dir = (council_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``council gateway run`` processes
    ...
    try:
        from council_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from council_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``COUNCIL_HOME`` and ``COUNCIL_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\council\git\cmd``, ``%LOCALAPPDATA%\council\git\bin``,
#      ``%LOCALAPPDATA%\council\git\usr\bin``, ``%LOCALAPPDATA%\council\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\council\git\`` and Node to
#      ``%LOCALAPPDATA%\council\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``council dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\council\gateway-service\`` and sometimes
...
def _council_path_markers(council_home: Path) -> list[str]:
    ...
    # backslash-council as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(council_home: Path) -> list[str]:
    """Strip Council-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _council_path_markers(council_home)
            kept: list[str] = []
...
def remove_council_env_vars_windows() -> list[str]:
    """Delete COUNCIL_HOME and COUNCIL_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("COUNCIL_HOME", "COUNCIL_GIT_BASH_PATH"):
                try:
...
def _is_default_council_home(council_home: Path) -> bool:
    """Return True when ``council_home`` points at the default (non-profile) root."""
    try:
        from council_constants import get_default_council_root
        return council_home.resolve() == get_default_council_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its COUNCIL_HOME directory.

    We shell out to ``council -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current COUNCIL_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m council_cli.main` so we don't depend on a `council`
    #    wrapper that may be half-removed mid-uninstall.
    council_invocation = [_sys.executable, "-m", "council_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                council_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's COUNCIL_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.council/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.council/ for future reinstall
    """
    project_root = get_project_root()
    council_home = get_council_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie COUNCIL_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_council_home(council_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Poke Council Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {council_home / 'config.yaml'}")
    print(f"  Secrets: {council_home / '.env'}")
    print(f"  Data:    {council_home / 'cron/'}, {council_home / 'sessions/'}, {council_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their COUNCIL_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Council data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Council code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in council_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\council\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(council_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Council-owned PATH entries in User environment")

        log_info("Removing COUNCIL_HOME / COUNCIL_GIT_BASH_PATH User env vars...")
        removed_env = remove_council_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Council-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing council command...")
    ...
    #     (only when they still point into this Council home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Council-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(council_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Council-managed node/npm/npx symlinks found")
    ...
    #     under COUNCIL_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(council_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(council_home)
    ...
    # 5. Optionally remove ~/.council/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile COUNCIL_HOME dirs live under
        ...
        try:
            if council_home.exists():
                shutil.rmtree(council_home)
                log_success(f"Removed {council_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {council_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {council_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {council_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/pokelabshq/council/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/pokelabshq/council/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Poke Council! ⚕")
    print()
