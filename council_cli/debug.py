"""``council debug`` debug tools for Poke Council.

Currently supports:
    council debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.council/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[council debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.council/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``council debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    """Synchropokely DELETE any pending pastes whose ``expire_at`` has passed.
    ...
    intended to be called from every ``council debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Council version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `council debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``council debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.council/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``council debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----CouncilDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from council_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_council_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``council dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``council dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "council debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  council debug delete <url>")

    print(f"\nShare these links with the Council team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: council debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'council debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``council debug`` call.
    ...
    # best-effort — any failure is swallowed so ``council debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: council debug <command>")
        print()
        print("Commands:")
