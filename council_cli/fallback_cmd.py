"""
council fallback — manage the fallback provider chain.
...
Subcommands:
  council fallback [list]   Show the current fallback chain (default when no subcommand)
  council fallback add      Pick provider + model via the same picker as `council model`,
                           then append the selection to the chain
  council fallback remove   Pick an entry to delete from the chain
  council fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.council/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from council_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `council model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  council fallback add")
        print()
        return
    ...
    print("  Docs: https://pokelabs-council.pokelabs.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `council model`, then append the selection to the chain."""
    from council_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `council model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `council fallback list` to view, or `council fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``council fallback [subcommand]``."""
