"""Implementation of the ``council bundles`` CLI subcommand.

Mirrors the structure of ``council_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `council bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  council bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``council bundles`` argparse tree.

    Called from ``council_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `council bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``council bundles <subcommand>`` to the right handler."""
