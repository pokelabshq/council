"""council webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.council/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _council_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: council gateway run
"""
...
def webhook_command(args):
    """Entry point for 'council webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: council webhook {subscribe|list|remove|test}")
        print("Run 'council webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (council gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: council webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from council webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (council gateway run)")
