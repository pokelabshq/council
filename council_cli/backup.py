...
from council_constants import get_default_council_root, get_council_home, display_council_home
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to council root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside COUNCIL_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Council home directory."""
    council_root = get_default_council_root()

    if not council_root.is_dir():
        print(f"Error: Council home directory not found at {council_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"council-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"council-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_council_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(council_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(council_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(council_root)
    ...
    print(f"\nRestore with: council import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Council backup.
    ...
    # Look for telltale files that a council home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Council backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.council/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a council dir name
        if prefix in {".council", "council"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_council_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (council_root / "config.yaml").exists()
        has_env = (council_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Council configuration.")
        ...
        for member in members:
            ...
            target = council_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(council_root.resolve())
            except ValueError:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # council_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: council profile list  (after installing council)")
        ...
        if restored_profiles:
            ...
            for pname in gw_profiles:
                print(f"  council -p {pname} gateway install")

        print("Done. Your Council configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and council backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to COUNCIL_HOME).
...
# — `council update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(council_home: Optional[Path] = None) -> Path:
    home = council_home or get_council_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    council_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(council_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside COUNCIL_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``council update``.
    ...
    Args:
        ...
        council_home: Override for the Council home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(council_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for council backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_council_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, council_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``council_root`` to ``out_path``.
...
def _pre_update_backup_dir(council_home: Optional[Path] = None) -> Path:
    home = council_home or get_council_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of COUNCIL_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<COUNCIL_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``council update``) should continue even if the backup fails.
    """
    council_root = council_home or get_default_council_root()
    if not council_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(council_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, council_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `council claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of COUNCIL_HOME under ``backups/`` before a
    ``council claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``council import <archive>``.  Writes to
    ``<COUNCIL_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `council import` and the
