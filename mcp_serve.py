"""
Council MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Council-specific extra)

Usage:
    council mcp serve
    council mcp serve --verbose
...
logger = logging.getLogger("council.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using COUNCIL_HOME."""
    try:
        from council_constants import get_council_home
        return get_council_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("COUNCIL_HOME", Path.home() / ".council")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from council_constants import get_council_home
        directory_file = get_council_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("COUNCIL_HOME", Path.home() / ".council")
...
class EventBridge:
    ...
    This is the Council equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from council_constants import get_council_home
            db_file = get_council_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("COUNCIL_HOME", Path.home() / ".council")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Council MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "council",
        instructions=(
            "Poke Council messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Council MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
