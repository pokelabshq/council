...
Let the poke council join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where council runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `council meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `council meet` CLI |
...
| `cli.py` | `council meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$COUNCIL_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `council meet node {run,list,approve,remove,status,ping}` |
...
council meet install                                      # pip + Chromium
council meet setup                                        # preflight
council meet auth                                         # optional
council meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
council meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.council/.env
council meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
council meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
council meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.council/.env
council meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, council will **not** switch your system audio input automatically — the
...
council plugins enable google_meet
council meet node run --display-name my-mac --host 0.0.0.0 --port 18789
