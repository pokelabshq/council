"""`council meet node ...` subcommand tree.

Wired into the existing ``council meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``council meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="council-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``council meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             council meet node approve <name> ws://<host>:{args.port} {token}")
        try:
