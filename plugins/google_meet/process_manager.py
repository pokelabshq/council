...
session-scoped state file under ``$COUNCIL_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $COUNCIL_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_council_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this council install, leave it first —
    ...
    env = os.environ.copy()
    env["COUNCIL_MEET_URL"] = url
    env["COUNCIL_MEET_OUT_DIR"] = str(out)
    env["COUNCIL_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["COUNCIL_MEET_HEADED"] = "1"
    if auth_state:
        env["COUNCIL_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["COUNCIL_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if COUNCIL_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["COUNCIL_MEET_MODE"] = mode
    if realtime_model:
        env["COUNCIL_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["COUNCIL_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["COUNCIL_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["COUNCIL_MEET_REALTIME_KEY"] = realtime_api_key
