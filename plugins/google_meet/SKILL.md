...
council meet install                 # pip deps + Chromium (transcribe only)
council meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
council meet auth                    # optional; skips guest-lobby wait
council meet setup                   # preflight checks
```

`council meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or COUNCIL_MEET_REALTIME_KEY in ~/.council/.env
```
...
pip install playwright websockets && python -m playwright install chromium
council plugins enable google_meet
council meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
council meet node approve my-mac ws://<mac-ip>:18789 <token>
council meet node ping my-mac                   # confirm reachable
```

Run `council meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Council agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `council meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `COUNCIL_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `council_meet_src`). |
...
Local:
```
$COUNCIL_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$COUNCIL_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `council meet node approve`.
