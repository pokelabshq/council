...
``$COUNCIL_HOME/workspace/meetings/<meeting-id>/``. The main council process
...
Run standalone for debugging::
    ...
    COUNCIL_MEET_URL=https://meet.google.com/abc-defg-hij \\
    COUNCIL_MEET_OUT_DIR=/tmp/meet-debug \\
    COUNCIL_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``COUNCIL_MEET_OUT_DIR``.
...
# and exposes ``window.__councilMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__councilMeetInstalled) return;
  window.__councilMeetInstalled = true;
  window.__councilMeetQueue = [];
  ...
  function pushEntry(speaker, text) {
    ...
    window.__councilMeetQueue.push({
      ts: Date.now(),
  ...
  window.__councilMeetDrain = () => {
    const out = window.__councilMeetQueue.slice();
    window.__councilMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "council_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("COUNCIL_MEET_URL", "").strip()
    out_dir_env = os.environ.get("COUNCIL_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("COUNCIL_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("COUNCIL_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("COUNCIL_MEET_GUEST_NAME", "Poke Council")
    duration_s = _parse_duration(os.environ.get("COUNCIL_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when COUNCIL_MEET_MODE=realtime.
    mode = os.environ.get("COUNCIL_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("COUNCIL_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("COUNCIL_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("COUNCIL_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("COUNCIL_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")

    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — COUNCIL_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: COUNCIL_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in COUNCIL_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("COUNCIL_MEET_LOBBY_TIMEOUT", "300")
            )
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__councilMeetDrain && window.__councilMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__councilMeetInstalled) {
