"""ntfy platform adapter (Council plugin).
...
a Council dependency.

This adapter ships as a Council platform plugin under
``plugins/platforms/ntfy/``. The Council plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "council-in"              # subscribe topic (incoming)
          publish_topic: "council-out"     # optional — defaults to topic
...
_ECHO_TAG = "pokelabs-council"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``council cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Council dependency",
