"""SimpleX Chat platform adapter (Council plugin).
...
This adapter ships as a Council platform plugin under
``plugins/platforms/simplex/``. The Council plugin loader scans the
...
discoverable and `council setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Council — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"council-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``council setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.council/.env`` via ``council_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("council_cli.config not available; set SIMPLEX_* vars manually in ~/.council/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `council gateway status` without
