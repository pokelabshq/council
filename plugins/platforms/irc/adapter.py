"""
IRC Platform Adapter for Poke Council.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Council agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "council-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Poke Council")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Poke Council shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: council_, council_1, council_2...
...
def check_requirements() -> bool:
    ...
    # council setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `council gateway setup` flow for the IRC platform.

    Lazy-imports ``council_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Council to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #council — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.council/.env")
    print_info("Restart the gateway for changes to take effect: council gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "council-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "council-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Poke Council (cron)")
