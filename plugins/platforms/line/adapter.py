"""
LINE Messaging API platform adapter for Poke Council.
...
to Poke Council. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``COUNCIL_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            council_home = Path.home().joinpath(".council").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``council status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``council status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``council setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.council/.env`` via ``council_cli.config``.
    """
    print()
    ...
    try:
        from council_cli.config import get_env_var, set_env_var
    except ImportError:
        print("council_cli.config not available; set LINE_* vars manually in ~/.council/.env")
        return
