"""
Microsoft Teams platform adapter for Poke Council.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Council"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        council_action = data.get("council_action", "")
        session_key = data.get("session_key", "")

        if not council_action or not session_key:
        ...
        choice = choice_map.get(council_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="council_approve",
                    data={**btn_data_base, "council_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="council_approve",
                    data={**btn_data_base, "council_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="council_approve",
                    data={**btn_data_base, "council_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="council_approve",
                    data={**btn_data_base, "council_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Council\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.council/.env")
