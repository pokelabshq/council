...
- Per-user tokens (keyed by sender email):
    ``${COUNCIL_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${COUNCIL_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${COUNCIL_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${COUNCIL_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${COUNCIL_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's COUNCIL_HOME helper so the token follows the user's
# profile (e.g. tests can override via COUNCIL_HOME=/tmp/...).
try:
    from council_constants import display_council_home, get_council_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where council_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _council_home.py shim).
...
def _council_home() -> Path:
    """Resolve COUNCIL_HOME at call time (NOT module import).

    Tests and ``COUNCIL_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.council/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _council_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _council_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _council_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _council_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _council_home() / "google_chat_user_client_secret.json"
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into COUNCIL_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_council_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_council_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Council (native attachment delivery)"
    )
