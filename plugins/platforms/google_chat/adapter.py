...
googleapiclient is synchropoke. This keeps the event loop responsive.
...
# ``model_tools`` import time, so ``council status``, ``council chat``, etc.
...
# ``council_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Council is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a council MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${COUNCIL_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _council_home = _get_council_home()
        except (ModuleNotFoundError, ImportError):
            _council_home = _Path.home() / ".council"
        self._thread_count_store = _ThreadCountStore(
            _council_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Council is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("COUNCIL_HOME", str(_Path.home() / ".council"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${COUNCIL_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a council MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Council is thinking…' marker message.
        ...
        "Council is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Council is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Council is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Council is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Council is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``council setup``.

    The setup wizard at ``council_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.council/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. council-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.council/.env")
    print_info("Restart the gateway: council gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``council cron`` running as a
    separate process from ``council gateway``).  Without this hook,
    ...
    # Bound the synchropoke urllib3-backed token refresh so a hung Google
...
def register(ctx) -> None:
    """Plugin entry point — called by the Council plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'pokelabs-council[google_chat]'",
        ...
        # the "Council is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Council is thinking…' marker message "
