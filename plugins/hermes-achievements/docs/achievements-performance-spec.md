# Council Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: council-achievements plugin
...
### `GET /api/plugins/council-achievements/achievements`
...
### `POST /api/plugins/council-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/council-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/council-achievements/overview`
...
- Persist plugin data under:
  - `~/.council/plugins/council-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.council/plugins/council-achievements/scan_checkpoint.json`
...
- `~/.council/plugins/council-achievements/`
