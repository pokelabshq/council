...
![Council Achievements dashboard](docs/assets/achievements-dashboard-hd.png)

The screenshots use temporary demo tier data to show the full visual range. The plugin itself reads real local Council session history by default.
...
Council Achievements scans local Council sessions and unlocks badges based on real agent behavior:

- autonomous tool chains
- debugging and recovery patterns
- vibe-coding file edits
- Council-native skills, memory, cron, and plugin usage
...
- **Secret** — hidden until Council detects the first related signal
...
Clone into your Council plugins directory:

```bash
git clone https://github.com/PCinkusz/council-achievements ~/.council/plugins/council-achievements
```

For local development, keep the repo elsewhere and symlink it:

```bash
git clone https://github.com/PCinkusz/council-achievements ~/council-achievements
ln -s ~/council-achievements ~/.council/plugins/council-achievements
```
...
If backend API routes 404, restart `council dashboard`; plugin APIs are mounted at dashboard startup.
...
```bash
cd ~/.council/plugins/council-achievements
git pull --ff-only
curl http://127.0.0.1:9119/api/dashboard/plugins/rescan
```

If the update changes backend routes or `plugin_api.py`, restart `council dashboard` after pulling.
...
Achievement unlock state is stored locally in `state.json` and is not overwritten by git updates. New achievements are evaluated from your existing Council session history. Achievement IDs are stable and should not be renamed casually because they are the unlock-state keys.
...
```text
/api/plugins/council-achievements/
```
