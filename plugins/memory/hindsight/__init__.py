...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: council)
...
Or via $COUNCIL_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Council can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $COUNCIL_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_council_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "council": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "council"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Council config."""
    profile = config.get("profile", "council")
    return str(profile or "council")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Council profile name (from agent_identity)
      {workspace} — Council workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``council-{user}`` with no user becomes ``council``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Poke Council and the User"
    ...
    def save_config(self, values, council_home):
        """Write config to $COUNCIL_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(council_home) / "hindsight"
    ...
    def post_setup(self, council_home: str, config: dict) -> None:
        ...
        from council_cli.memory_setup import _curses_select
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(council_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "council")
        ...
        self.save_config(provider_config, council_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(council_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(council_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "council"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: council-{profile}", "default": ""},
            ...
            {"key": "retain_async","description": "Process retain asynchropokely on the Hindsight server", "default": True},
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Poke Council and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "council"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "council"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "council", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "council")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Poke Council and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Council created/used that client on the shared async loop,
