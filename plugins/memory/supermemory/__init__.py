...
_DEFAULT_CONTAINER_TAG = "council"
...
def _load_supermemory_config(council_home: str) -> dict:
    config = _default_config()
    config_path = Path(council_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, council_home: str) -> None:
    config_path = Path(council_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `council memory setup`.
        # All other options are documented for $COUNCIL_HOME/supermemory.json
    ...
    def save_config(self, values, council_home):
        ...
        _save_supermemory_config(sanitized, council_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from council_constants import get_council_home
        self._council_home = kwargs.get("council_home") or str(get_council_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._council_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "council", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "council_memory", "target": target, "type": "explicit_memory"},
