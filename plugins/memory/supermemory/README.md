...
Config file: `$COUNCIL_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `council` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `council-{identity}` → `council-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Council profile:

```json
{
  "container_tag": "council-{identity}"
}
```

For a profile named `coder`, this resolves to `council-coder`. The default profile resolves to `council-default`. Without `{identity}`, all profiles share the same container.
