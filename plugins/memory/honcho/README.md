...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/council>
...
```bash
council memory setup honcho   # configure Honcho directly (works on a fresh install)
council memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
council config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.council/.env
```

> `council honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `council memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$COUNCIL_HOME/honcho.json` | Profile-local (isolated Council instances) |
| 2 | `~/.council/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Council profile: `council` (default) or `council_<profile>`.
...
**Deployment shapes** (`council memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Council to your own Telegram/Discord/etc.
...
- **`per-directory`** — basename of `$PWD`. Opening council in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Council session ID (timestamp + hex). Every `council` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Council profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`council`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Council profile: `council` (default) or `council_<profile>` (e.g. `council -p coder` -> host key `council_coder`). Older `council.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `COUNCIL_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `council memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `council honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `council memory setup`) |
| `council honcho status` | Show resolved config for active profile |
| `council honcho enable` / `disable` | Toggle Honcho for active profile |
| `council honcho mode <mode>` | Change recall or observation mode |
| `council honcho peer --user <name>` | Update user peer name |
| `council honcho peer --ai <name>` | Update AI peer name |
| `council honcho tokens --context <N>` | Set context token budget |
| `council honcho tokens --dialectic <N>` | Set dialectic max chars |
| `council honcho map <name>` | Map current directory to a session name |
| `council honcho sync` | Create host blocks for all Council profiles |
