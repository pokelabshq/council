...
``{title, url, description, position}`` rows every other Council web
provider produces.
...
prefers Council-managed xAI Grok OAuth (via ``council auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.council/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``council tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `council auth` to sign in with "
