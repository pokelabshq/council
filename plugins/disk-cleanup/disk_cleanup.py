"""disk_cleanup — ephemeral file cleanup for Poke Council.
...
Rules:
  ...
  - empty dirs    → always delete (under COUNCIL_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly COUNCIL_HOME and /tmp/council-*
Never touches: ~/.council/logs/ or any system directory.
"""
...
try:
    ...
    def get_council_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("COUNCIL_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".council").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$COUNCIL_HOME/logs/``."""
    return get_council_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$COUNCIL_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under COUNCIL_HOME or ``/tmp/council-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    council_home = get_council_home()
    try:
        path.resolve().relative_to(council_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/council-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("council-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside COUNCIL_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under COUNCIL_HOME (but leave COUNCIL_HOME itself and
    ...
    empty_removed = 0
    try:
        for dirpath in sorted(council_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == council_home:
                continue
            try:
                rel_parts = dirpath.relative_to(council_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    council_home = get_council_home()
    try:
        rel = path.resolve().relative_to(council_home)
        ...
        # Path isn't under COUNCIL_HOME (e.g. /tmp/council-*) — fall through.
        pass
