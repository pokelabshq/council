...
Auto-tracks and cleans up ephemeral files created during Poke Council
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$COUNCIL_HOME` and `/tmp/council-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `COUNCIL_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `COUNCIL_HOME` or `/tmp/council-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$COUNCIL_HOME/disk-cleanup/` is itself excluded
- `$COUNCIL_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
