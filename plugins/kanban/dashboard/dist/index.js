/**
 * Council Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.council/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__COUNCIL_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "council-kanban-dot-triage",
    todo: "council-kanban-dot-todo",
    ready: "council-kanban-dot-ready",
    running: "council-kanban-dot-running",
    blocked: "council-kanban-dot-blocked",
    done: "council-kanban-dot-done",
    archived: "council-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-council-task";
  ...
  const DOCS_URL = "https://pokelabs-council.pokelabs.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://pokelabs-council.pokelabs.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "council.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="council-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "council-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "council-kanban-md",
  ...
  // standard `drop` event and our `council-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("council-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("council-kanban-column--drop");
          if (target) target.classList.add("council-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("council-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("council-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("council-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "council-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "council-kanban-attention",
        "council-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "council-kanban-attention-bar" },
        h("span", { className: "council-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "council-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "council-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "council-kanban-attention-row",
                  "council-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "council-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "council-kanban-attention-row-id" }, task.id),
                h("span", { className: "council-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "council-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "council-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "council-kanban-diag-action-btn",
      action.suggested ? "council-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " council-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".council-kanban-drawer-comment-row input, .council-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "council-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("council-kanban-diag", sevClass) },
      h("div", { className: "council-kanban-diag-header" },
        h("span", { className: "council-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "council-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "council-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "council-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "council-kanban-diag-data-row" },
                  h("span", { className: "council-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "council-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "council-kanban-diag-data-row" },
                h("span", { className: "council-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "council-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "council-kanban-diag-reassign-row" },
            h("span", { className: "council-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "council-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "council-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "council-kanban-diag-msg",
              msg.ok ? "council-kanban-diag-msg--ok" : "council-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "council-kanban-section" },
      h("div", { className: "council-kanban-section-head-row" },
        h("span", { className: "council-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "council-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "council-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "council-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "council-kanban-docs-link",
      title: "Open Council Kanban docs in a new tab",
      "aria-label": "Council Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `council kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "council-kanban-msg-ok" : "council-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "council-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "council-kanban-boardswitcher" },
      h("div", { className: "council-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "council-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "council-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "council-kanban-dialog-title" },
        ...
        h("div", { className: "council-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Council profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "council-kanban-bulk" },
      h("span", { className: "council-kanban-bulk-count" },
      ...
      h("div", { className: "council-kanban-bulk-priority",
      ...
      h("div", { className: "council-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Council profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "council-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("council-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("council-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "council-kanban-trash",
        dragOver ? "council-kanban-trash--drop" : "",
        props.draggingTaskId ? "council-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "council-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "council-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".council-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "council-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("council-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("council-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "council-kanban-column",
        dragOver ? "council-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "council-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "council-kanban-col-check",
        ...
        h("span", { className: cn("council-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "council-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "council-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "council-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "council-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "council-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "council-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "council-kanban-lane" },
                  h("div", { className: "council-kanban-lane-head" },
                    h("span", { className: "council-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "council-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "council-kanban-card--stale-red";
    if (age >= tier.amber) return "council-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".council-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "council-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "council-kanban-card",
        props.selected ? "council-kanban-card--selected" : "",
        props.failed ? "council-kanban-card--failed" : "",
        props.draggingSource ? "council-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "council-kanban-card-content" },
          h("div", { className: "council-kanban-card-row" },
            h("label", {
              className: "council-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "council-kanban-card-check",
            ...
            h("span", { className: "council-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or council kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "council-kanban-warning-badge",
                    "council-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "council-kanban-needs-assignee",
          ...
          h("div", { className: "council-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "council-kanban-card-row council-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "council-kanban-assignee",
                            title: `Assigned to Council profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "council-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "council-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "council-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "council-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Council profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Council profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "council-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "council-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "council-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "council-kanban-drawer-close",
        ...
        data ? h("div", { className: "council-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "council-kanban-section" },
      h("div", { className: "council-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "council-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "council-kanban-drawer-body" },
      h("div", { className: "council-kanban-drawer-title" },
        h("span", { className: cn("council-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "council-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "council-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "council-kanban-section" },
        h("div", { className: "council-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "council-kanban-section" },
        h("div", { className: "council-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "council-kanban-comment" },
                h("div", { className: "council-kanban-comment-head" },
                  h("span", { className: "council-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "council-kanban-comment-ago" },
      ...
      h("div", { className: "council-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "council-kanban-event",
              isDiag ? "council-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "council-kanban-event-header" },
                  h("span", { className: "council-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "council-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "council-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "council-kanban-event-header-plain" },
                  h("span", { className: "council-kanban-event-kind" }, e.kind),
                  h("span", { className: "council-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "council-kanban-event-phantom-row" },
                  h("span", { className: "council-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "council-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "council-kanban-event-payload" },
    ...
    return h("div", { className: "council-kanban-section" },
      h("div", { className: "council-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "council-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `council-kanban-run--${r.outcome || r.status || "ended"}`
          : "council-kanban-run--active";
        return h("div", { key: r.id, className: cn("council-kanban-run", outcomeClass) },
          h("div", { className: "council-kanban-run-head" },
            h("span", { className: "council-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "council-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "council-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "council-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "council-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "council-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "council-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "council-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "council-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "council-kanban-pre council-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "council-kanban-meta-row" },
      h("span", { className: "council-kanban-meta-label" }, props.label),
      h("span", { className: "council-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "council-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "council-kanban-meta-row" },
        h("span", { className: "council-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "council-kanban-meta-value council-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "council-kanban-meta-row" },
        h("span", { className: "council-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "council-kanban-meta-value council-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "council-kanban-section" },
      h("div", { className: "council-kanban-section-head-row" },
        h("span", { className: "council-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "council-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "council-kanban-section" },
      h("div", { className: "council-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "council-kanban-deps-row" },
        h("span", { className: "council-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "council-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "council-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "council-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "council-kanban-dep-chip-x",
      ...
      h("div", { className: "council-kanban-deps-row" },
        h("span", { className: "council-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "council-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "council-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "council-kanban-msg-ok"
          : "council-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "council-kanban-section" },
      ...
      h("div", { className: "council-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "council-kanban-home-sub council-kanban-home-sub--on"
              : "council-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__COUNCIL_PLUGINS__ && typeof window.__COUNCIL_PLUGINS__.register === "function") {
    window.__COUNCIL_PLUGINS__.register("kanban", KanbanPage);
  }
})();
