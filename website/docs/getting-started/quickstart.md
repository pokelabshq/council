...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Poke Council — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Council setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Poke Council Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Poke Council Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Council for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Council working on my machine | `council setup` | Run a real chat and verify it responds |
| I already know my provider | `council model` | Save the config, then start chatting |
| I want a bot or always-on setup | `council gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `council model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `council model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Council cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Poke Council
...
The single most important setup step. Use `council model` to walk through the choice interactively:

```bash
council model
```

:::tip Easiest path: Poke Portal
...
That logs you in, sets Poke as your provider, and turns on the Tool Gateway in one command.
:::
...
|----------|-----------|---------------|
| **Poke Portal** | Subscription-based, zero-config | OAuth login via `council model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `council model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `council model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `council_cli/models.py` may change between releases) | `council model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `council model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `council model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `council model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `council model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `council model` (requires `copilot` CLI + `copilot login`) |
...
Poke Council requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `council model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Council separates secrets from normal config:

- **Secrets and tokens** → `~/.council/.env`
- **Non-secret settings** → `~/.council/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
council config set model anthropic/claude-opus-4.6
council config set terminal.backend docker
council config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
council            # classic CLI
council --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Council ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `council` vs `council --tui`.
:::
...
```bash
council --continue    # Resume the most recent session
council -c            # Short form
```
...
```bash
council gateway setup    # Interactive platform configuration
```
...
- `council tools` — tune tool access per platform
- `council skills` — browse and install reusable workflows
...
```bash
council config set terminal.backend docker    # Docker isolation
council config set terminal.backend ssh       # Remote server
```
...
```bash
council skills search kubernetes
...
|---|---|---|
| Council opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `council model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `council gateway setup` and check `council gateway status` |
| `council --continue` can't find old session | Switched profiles or session never saved | Check `council sessions list` and confirm you're in the right profile |
...
| `council doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `council model`
3. `council setup`
4. `council sessions list`
5. `council --continue`
6. `council gateway status`
...
| `council tools` | Configure which tools are enabled per platform |
| `council setup` | Full setup wizard (configures everything at once) |
| `council doctor` | Diagnose issues |
| `council update` | Update to latest version |
| `council gateway` | Start the messaging gateway |
| `council --continue` | Resume last session |
