...
description: "Install and deploy Poke Council with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

Poke Council ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`council setup`, `council gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage council the same way you manage any other NixOS service.
:::
...
nix run github:pokelabshq/council -- setup
nix run github:pokelabshq/council -- chat

# Or install persistently
nix profile install github:pokelabshq/council
council setup
council chat
```
...
```bash
nix profile install github:pokelabshq/council#messaging
```

For all optional extras (voice, all providers, all platforms):

```bash
nix profile install github:pokelabshq/council#full
```
...
That's it. `nixos-rebuild switch` creates the `council` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o council /dev/stdin /var/lib/council/env
```
...
Setting `addToSystemPackages = true` does two things: puts the `council` CLI on your system PATH **and** sets `COUNCIL_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `council` in your shell creates a separate `~/.council/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `council` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `council chat`, `council sessions list`, `council version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the council codebase, set `COUNCIL_DEV=1` to bypass container routing and run the local checkout directly
...
Users listed in `hostUsers` are automatically added to the `council` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo council chat` manually.
:::
...
council version
council config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Council defaults to OpenRouter.
:::
...
{
  sops = {
    defaultSopsFile = ./secrets/council.yaml;
...
```yaml
# secrets/council.yaml (encrypted with sops)
council-env: |
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Council looks for specific filenames by convention:
...
:::tip
Environment variables in `env` values are resolved from `$COUNCIL_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Set `auth = "oauth"` for servers using OAuth 2.1. Council implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
Tokens are stored in `$COUNCIL_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.

<details>
<summary><strong>Initial OAuth authorization on headless servers</strong></summary>

The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Council prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u council` (native):
...
# Native mode
sudo -u council COUNCIL_HOME=/var/lib/council/.council \
  council mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
council mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.council/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/council/.council/mcp-tokens/
# Ensure: chown council:council, chmod 0600
```
...
When council runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `council setup` | Config is declarative — edit `settings` in your Nix config |
| `council config edit` | Config is generated from `settings` |
| `council config set <key> <value>` | Config is generated from `settings` |
| `council gateway install` | The systemd service is managed by NixOS |
| `council gateway uninstall` | The systemd service is managed by NixOS |
...
When container mode is enabled, council runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the council package itself do **not** trigger recreation.
...
If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/council-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current council package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `council plugins install` needed.
...
Plugins are symlinked into `$COUNCIL_HOME/plugins/` at activation time. Council discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
The package's `site-packages` is added to PYTHONPATH in the council wrapper. `importlib.metadata` discovers the entry point at session start.
...
:::note
A build-time collision check prevents plugin packages from shadowing core council dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
nix build .#checks.x86_64-linux.managed-guard       # COUNCIL_MANAGED blocks mutation
...
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `COUNCIL_BUNDLED_SKILLS` is set in wrapper |
...
| `stateDir` | `str` | `"/var/lib/council"` | State directory (`COUNCIL_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `council` CLI to system PATH and set `COUNCIL_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$COUNCIL_HOME/.env` at activation time |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `council gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the council user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$COUNCIL_HOME/plugins/`. Each must contain `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.council` symlink to the service stateDir and are auto-added to the `council` group |
...
```
/var/lib/council/                     # stateDir (owned by council:council, 0750)
├── .council/                         # COUNCIL_HOME
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Council binary + all Nix deps |
| `/data` | `/var/lib/council` | `rw` | All state, config, workspace |
| `/home/council` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
