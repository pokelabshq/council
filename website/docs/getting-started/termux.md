...
description: "Run Poke Council directly on an Android phone with Termux"
---

# Council on Android with Termux

This is the tested path for running Poke Council directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Council from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Council now ships a Termux-aware installer path:
...
- links `council` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `council` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/council" "$PREFIX/bin/council"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `council` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.council/.env`.
...
### `council doctor` says ripgrep or Node is missing
