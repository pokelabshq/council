...
sidebar_position: 2
title: "Installation"
description: "Install Poke Council on Linux, macOS, WSL2, native Windows (early beta), or Android via Termux"
---

# Installation

Get Poke Council up and running in under two minutes with the one-line installer.
...
:::warning Early BETA
Native Windows support is **early beta**. It installs and works for the common paths, but hasn't been road-tested as broadly as our POSIX installers. Please [file issues](https://github.com/pokelabshq/council/issues) when you hit rough edges. For the most battle-tested setup on Windows today, use the Linux/macOS one-liner above inside **WSL2** instead.
:::
...
2. Otherwise it downloads portable **PortableGit** (~50MB, from the official `git-for-windows` GitHub release) and unpacks it to `%LOCALAPPDATA%\council\git`.  No admin rights required.  Completely isolated — it won't interfere with any system Git install, broken or otherwise.  (On 32-bit Windows it falls back to MinGit because PortableGit ships only 64-bit and ARM64 assets; bash-dependent Council features won't work on 32-bit hosts.)

**Why not use winget?**  Earlier designs auto-installed Git via `winget install Git.Git`, but winget fails badly when a system Git install is in a partial or broken state (exactly when users need the installer to just work).  The portable Git approach sidesteps winget, the Windows installer registry, and any existing system Git entirely.  If the Council Git install itself ever breaks, `Remove-Item %LOCALAPPDATA%\council\git` and re-run the installer — no system impact, no uninstall drama.

The installer also sets `COUNCIL_GIT_BASH_PATH` to the located `bash.exe` so Council resolves it deterministically in fresh shells.

If you prefer WSL2, the Linux installer above works inside it; both native and WSL installs can coexist without conflict (native data lives under `%LOCALAPPDATA%\council`, WSL data lives under `~/.council`).

**Desktop installer (alternative):** A thin GUI installer is also available — download Council Desktop, run the `.exe`, and on first launch it calls `install.ps1` under the hood to provision Python (via `uv`), Node, PortableGit, and the rest of the dependencies. The desktop app and the PowerShell-installed CLI share the same install and data directories, so you can use either or both. See the [Windows (Native) guide](../user-guide/windows-native#desktop-installer-alternative) for details.

### Android / Termux

Council now ships a Termux-aware installer path too:
...
- **CLI (`council chat`, `council setup`, `council gateway`, …)** — native, uses your default terminal
...
Set `COUNCIL_DISABLE_WINDOWS_UTF8=1` in your environment if you hit an encoding-related bug and want to fall back to the legacy cp1252 stdio path (useful for bisecting).
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `council` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
council gateway setup  # Set up messaging platforms
council config set     # Set individual config values
council setup          # Or run the full setup wizard to configure everything at once
```
...
Running Council as a dedicated unprivileged user (e.g. a `council` systemd service account, or any user without `sudo` access) is supported. The only thing on the install path that genuinely needs root is Playwright's `--with-deps` step, which `apt`-installs shared libraries (`libnss3`, `libxkbcommon`, etc.) used by Chromium. The installer detects whether sudo is available and gracefully degrades when it isn't — it will install the Chromium binary into the service user's own Playwright cache and print the exact command an administrator needs to run separately.
...
2. **As the unprivileged service user**, run the regular installer. It will detect the missing sudo, skip `--with-deps`, and install Chromium into the user's local Playwright cache:
   ...
   ```bash
   curl -fsSL https://raw.githubusercontent.com/pokelabshq/council/main/scripts/install.sh | bash -s -- --skip-browser
   ```

3. **Make `council` available to the service user's shells.** The installer writes the launcher to `~/.local/bin/council`. System service accounts often have a minimal PATH that doesn't include `~/.local/bin`. Either add it to the user's environment, or symlink the launcher into a system location:
   ```bash
...
| Problem | Solution |
|---------|----------|
| `council: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `council model` to configure your provider, or `council config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `council config check` then `council config migrate` |

For more diagnostics, run `council doctor` — it will tell you exactly what's missing and how to fix it.
