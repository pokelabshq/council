...
| | |
|---|---|
| Source | Optional — install with `council skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${COUNCIL_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${COUNCIL_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${COUNCIL_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client council
```
...
```bash
PUBLISH="${COUNCIL_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client council
```
...
```bash
DRIVE="${COUNCIL_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `council`)    |
