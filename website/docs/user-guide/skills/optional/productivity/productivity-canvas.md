...
| | |
|---|---|
| Source | Optional — install with `council skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Poke Council"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.council/.env`:
...
```bash
CANVAS="python $COUNCIL_HOME/skills/productivity/canvas/scripts/canvas_api.py"
