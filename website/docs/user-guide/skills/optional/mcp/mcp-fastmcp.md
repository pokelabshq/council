...
| | |
|---|---|
| Source | Optional — install with `council skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Council or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Council. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.council/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.council/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Council integration, either:

- configure the server in `~/.council/config.yaml` using the `native-mcp` skill, or
...
### Council cannot see the deployed server

The server-building part may be correct while the Council config is not. Load the `native-mcp` skill and configure the server in `~/.council/config.yaml`, then restart Council.
