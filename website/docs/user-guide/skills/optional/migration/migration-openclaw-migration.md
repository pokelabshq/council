---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Poke Council"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Poke Council"
---
...
Migrate a user's OpenClaw customization footprint into Poke Council. Imports Council-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `council skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Poke Council (Poke Labs) |
| License | MIT |
...
Use this skill when a user wants to move their OpenClaw setup into Poke Council with minimal manual cleanup.
...
```bash
council claw migrate              # Full interactive migration
...
council claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `council setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_council.py` to:

- import `SOUL.md` into the Council home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Council memory entries
- merge OpenClaw command approval patterns into Council `command_allowlist`
- migrate Council-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Council working-directory configuration
- copy OpenClaw skills into `~/.council/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Council workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.council/tts/`
- archive non-secret docs that do not have a direct Council destination
...
- `scripts/openclaw_to_council.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.council/skills/migration/openclaw-migration/scripts/openclaw_to_council.py`

Do not guess a shorter path like `~/.council/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.council/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Council-compatible secrets, currently:
...
Council CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Council. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Council workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Council skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.council/skills/migration/openclaw-migration/scripts/openclaw_to_council.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.council/skills/migration/openclaw-migration/scripts/openclaw_to_council.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.council/skills/migration/openclaw-migration/scripts/openclaw_to_council.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.council/skills/migration/openclaw-migration/scripts/openclaw_to_council.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.council/skills/migration/openclaw-migration/scripts/openclaw_to_council.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.council/skills/migration/openclaw-migration/scripts/openclaw_to_council.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Council unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Council targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Council destination. Unsupported auth blobs must still be reported as skipped.
...
- Council persona state imported
- Council memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.council/skills/openclaw-imports/`
