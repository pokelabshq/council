...
| | |
|---|---|
| Source | Optional — install with `council skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Poke Council |
| License | MIT |
...
import upstream classes into Council itself.
...
```bash
mkdir -p ~/.council/cache/darwinian-evolver && cd ~/.council/cache/darwinian-evolver
...
```bash
cd ~/.council/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.council/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.council/skills/research/darwinian-evolver
DE_DIR=~/.council/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Council core.
   Custom driver scripts under `~/.council/skills/...` are user-side and fine.
