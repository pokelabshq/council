...
| | |
|---|---|
| Source | Optional — install with `council skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Poke Council |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Council terminal calls (desktop app flow only)
...
### Service Account (recommended for Council)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.council/.env` (the skill will prompt for this on first load).
...
## Council Execution Pattern (desktop app flow)

Council terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/council-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/council-op.sock"
