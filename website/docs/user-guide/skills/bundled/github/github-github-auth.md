...
- Give it a name like "pokelabs-council"
- Select scopes:
...
- Give it a title like "pokelabs-council-&lt;machine-name>"
...
elif [ -f ~/.council/.env ] && grep -q "^GITHUB_TOKEN=" ~/.council/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.council/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
