...
title: "Git Worktrees"
description: "Run multiple Council agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Poke Council is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Council so each session has a clean, isolated working directory.

## Why Use Worktrees with Council?

Council treats the **current working directory** as the project root:

- CLI: the directory where you run `council` or `council chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.council/config.yaml`
...
git worktree add ../repo-feature feature/council-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/council-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Council there:

```bash
cd ../repo-feature

# Start Council in the worktree
council
```
...
git worktree add ../repo-experiment-a feature/council-a
git worktree add ../repo-experiment-b feature/council-b
```
...
Each Council process:

- Works on its own branch (`feature/council-a` vs `feature/council-b`).
...
- Council checkpoint data under `~/.council/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Council experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/council-checkpoints-docs`, `feature/council-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Council from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `council -w` (Automatic Worktree Mode)

Council has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `council/council-<hash>`).
...
```bash
council -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `council -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Council session its own clean checkout.
