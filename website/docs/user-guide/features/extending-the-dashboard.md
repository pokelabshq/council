...
description: "Build themes and plugins for the Council web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Council web dashboard (`council dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.council/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.council/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.council/dashboard-themes
```

```yaml
# ~/.council/dashboard-themes/neon.yaml
name: neon
label: Neon
...
The CSS is injected as a single scoped `<style data-council-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**
...
|-------|---------|------------|--------|
| **Council Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Council Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Council Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
```yaml
# ~/.council/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Council plugins in `~/.council/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.

Plugins don't bundle React or UI components. They use the **Plugin SDK** exposed on `window.__COUNCIL_PLUGIN_SDK__`. This keeps plugin bundles tiny (typically a few KB) and avoids version conflicts.
...
```bash
mkdir -p ~/.council/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.council/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.council/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
  ...
  window.__COUNCIL_PLUGINS__.register("my-plugin", MyPage);
})();
```
...
Everything a plugin needs is on `window.__COUNCIL_PLUGIN_SDK__`. Plugins should never import React directly.
...
// Council API client + raw fetcher
...
#### Calling built-in Council endpoints
...
```javascript
window.__COUNCIL_PLUGINS__.registerSlot("my-plugin", "sidebar", MySidebar);
window.__COUNCIL_PLUGINS__.registerSlot("my-plugin", "header-left", MyCrest);
```
...
| `header-left` | Before the Council brand in the top bar. |
...
window.__COUNCIL_PLUGINS__.registerSlot("my-plugin", "sessions:top", PinnedSessionsBanner);
```
...
```json
// ~/.council/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.council/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__COUNCIL_PLUGIN_SDK__;
  const { React } = SDK;
  ...
  window.__COUNCIL_PLUGINS__.register("session-notes", function () { return null; });

  // The real work.
  window.__COUNCIL_PLUGINS__.registerSlot("session-notes", "sessions:top", Banner);
})();
```
...
A reference plugin (`example-dashboard` in [`council-example-plugins`](https://github.com/PokeLabs/council-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
```python
# ~/.council/plugins/my-plugin/dashboard/plugin_api.py
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.council/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.council/plugins/<name>/dashboard/` | `project` — only when `COUNCIL_ENABLE_PROJECT_PLUGINS` is set |
...
…or restart `council dashboard`.

#### Plugin load lifecycle

1. Dashboard loads. `main.tsx` exposes the SDK on `window.__COUNCIL_PLUGIN_SDK__` and the registry on `window.__COUNCIL_PLUGINS__`.
...
4. The plugin's IIFE runs and calls `window.__COUNCIL_PLUGINS__.register(name, Component)` — and optionally `.registerSlot(name, slot, Component)` for each slot.
...
The [`strike-freedom-cockpit`](https://github.com/PokeLabs/council-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `council-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/PokeLabs/council-example-plugins.git

# Theme
cp council-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.council/dashboard-themes/

# Plugin
cp -r council-example-plugins/strike-freedom-cockpit ~/.council/plugins/
```

Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Council Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
|--------|------|----------|
| `window.__COUNCIL_PLUGIN_SDK__` | object | `registry.ts` — React, hooks, UI components, API client, utils. |
| `window.__COUNCIL_PLUGINS__.register(name, Component)` | function | Register a plugin's main component. |
| `window.__COUNCIL_PLUGINS__.registerSlot(name, slot, Component)` | function | Register into a named shell slot. |
...
Check that the file is in `~/.council/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.council/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.council/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Open browser dev tools → Console — look for errors during the IIFE or `window.__COUNCIL_PLUGINS__ is undefined` (indicates the SDK didn't initialize, usually a React render crash earlier).
5. Verify your bundle calls `window.__COUNCIL_PLUGINS__.register(...)` with the **same name** as `manifest.json:name`.
...
2. Restart `council dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.council/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.council/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
