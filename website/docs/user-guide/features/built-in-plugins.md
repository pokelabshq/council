...
description: "Plugins shipped with Poke Council that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Council ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.council/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Council Plugin](/guides/build-a-council-plugin) to write your own.
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `council memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `council plugins list` and the interactive `council plugins` UI), but none load until you explicitly enable them:

```bash
council plugins enable disk-cleanup
```

Or via `~/.council/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Council. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
council plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `council plugins enable <name>`.
...
| `council-achievements` | dashboard tab | Steam-style collectible badges generated from your real Council session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `council memory` and `council plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `COUNCIL_HOME` or `/tmp/council-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under COUNCIL_HOME | always | Never |
...
**State** — everything lives at `$COUNCIL_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `COUNCIL_HOME` or `/tmp/council-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `council plugins enable disk-cleanup` (or check the box in `council plugins`).

**Disabling again:** `council plugins disable disk-cleanup`.
...
**Enabling:** `council plugins enable security-guidance` (or check the box in `council plugins`).

**Disabling again:** `council plugins disable security-guidance`.
...
Traces Council turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Council' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `council logs`.
...
council plugins enable observability/langfuse
```

Or check the box in the interactive `council plugins` UI. Then put the credentials in `~/.council/.env`:

```bash
COUNCIL_LANGFUSE_PUBLIC_KEY=pk-lf-...
COUNCIL_LANGFUSE_SECRET_KEY=sk-lf-...
COUNCIL_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Council turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `COUNCIL_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Council session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `council chat` session lives under one Langfuse session.

**Verify:**

```bash
council plugins list                 # observability/langfuse should show "enabled"
council chat -q "hello"              # check the Langfuse UI for a "Council turn" trace
```
...
| `COUNCIL_LANGFUSE_RELEASE` | — | Release/version tag |
| `COUNCIL_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `COUNCIL_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `COUNCIL_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Council-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Council-prefixed wins when both are set.
...
**Disabling:** `council plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.council/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
council plugins enable google_meet
...
**Disabling:** `council plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.council/cache/google_meet/` until you remove them.

### council-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Council session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Council feature changes.

**How it works:**

- Scans your entire `~/.council/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$COUNCIL_HOME/plugins/council-achievements/state.json`
...
| Secret | Hidden until Council detects the first related signal in your history |

**API** — routes mount under `/api/plugins/council-achievements/`:
...
| `POST /rescan` | Manual synchropoke rescan (blocks; use when the user clicks the rescan button) |
| `POST /reset-state` | Clear unlock history and cached snapshot |

**State files** — live under `$COUNCIL_HOME/plugins/council-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Council updates. |
...
**Enabling:** Nothing to enable — `council-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `council dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/council-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.council/plugins/council-achievements/` that ships no dashboard. The plugin's state files under `$COUNCIL_HOME/plugins/council-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Council plugin — see [Build a Council Plugin](/guides/build-a-council-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.council/plugins/<name>/`
- Manifest source is reported as `bundled` in `council plugins list`
