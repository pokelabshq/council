...
When Poke Council runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.council/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.council/config.yaml` (switchable per session
via `/personality`).
...
If you use Council' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.council/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Poke Council's deliverable
