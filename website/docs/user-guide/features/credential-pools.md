...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Council automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [Poke Portal](/integrations/poke-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
If you already have an API key set in `.env`, Council auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
council auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
council auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
council auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  council_pkce          oauth   council_pkce ←
...
Run `council auth` with no subcommand for an interactive wizard:
...
For providers that support both API keys and OAuth (Anthropic, Poke, Codex), the add flow asks which type:
...
| Command | Description |
|---------|-------------|
| `council auth` | Interactive pool management wizard |
| `council auth list` | Show all pools and credentials |
| `council auth list <provider>` | Show a specific provider's pool |
| `council auth add <provider>` | Add a credential (prompts for type and key) |
| `council auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `council auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `council auth remove <provider> <index>` | Remove credential by 1-based index |
| `council auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `council auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `council model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via council model:
council auth list
# Shows:
...
council auth add Together.ai --api-key sk-together-second-key
```
...
Council automatically discovers credentials from multiple sources and seeds the pool on startup:

| Source | Example | Auto-seeded? |
|--------|---------|-------------|
| Environment variables | `OPENROUTER_API_KEY`, `ANTHROPIC_API_KEY` | Yes |
| OAuth tokens (auth.json) | Codex device code, Poke device code | Yes |
| Claude Code credentials | `~/.claude/.credentials.json` | Yes (Anthropic) |
| Council PKCE OAuth | `~/.council/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `council auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `council auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Council can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Council-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`council_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`council_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.council/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Council' credential store, so its token remains persistable.
