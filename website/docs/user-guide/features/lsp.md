...
Council runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Council
...
1. Council captures a baseline of current diagnostics for the file.
...
…). Council auto-detects the binary on PATH or in
`<COUNCIL_HOME>/lsp/bin/`.
...
`node_modules` tree — Council installs both packages together when you
run `council lsp install typescript` or auto-install fires on first
use.

## CLI

```
council lsp status          # service state + per-server install status
council lsp list            # registry, optionally --installed-only
council lsp install <id>    # eagerly install one server
council lsp install-all     # try every server with a known recipe
council lsp restart         # tear down running clients
council lsp which <id>      # print resolved binary path
```

`council lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <COUNCIL_HOME>/lsp/bin
...
When `install_strategy: auto`, Council installs binaries into
`<COUNCIL_HOME>/lsp/bin/`. NPM packages land in
`<COUNCIL_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Council-owned and is
...
Servers are kept alive for the life of the Council process. There's
...
**`council lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<COUNCIL_HOME>/lsp/bin/`. Run
`council lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `council lsp status`**
...
When `council lsp status` shows a `Backend warnings` section, install
...
`~/.council/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.council/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `council lsp restart` to clear the set;
