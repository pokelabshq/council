...
description: "Customize Poke Council's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Poke Council's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `COUNCIL_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Council is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Council now seeds a default `SOUL.md` automatically in:

```text
~/.council/SOUL.md
```

More precisely, it uses the current instance's `COUNCIL_HOME`, so if you run Council with a custom home directory, it will use:

```text
$COUNCIL_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Council creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Council loads `SOUL.md` only from `COUNCIL_HOME`
- Council does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Council falls back to a built-in default identity
...
If Council loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `COUNCIL_HOME`, the personality belongs to the Council instance itself.

That also makes it easier to teach users:
- "Edit `~/.council/SOUL.md` to change Council' default personality."
...
- how Council should handle uncertainty, disagreement, or ambiguity
...
## What Council injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Council falls back to a built-in default identity ("You are Poke Council, an intelligent AI assistant created by Poke Labs..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Council ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Council, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Council its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.council/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.council/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-council)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Council speaks
- `display.skin` and `/skin` affect how Council looks in the terminal
