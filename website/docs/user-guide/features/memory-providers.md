...
Poke Council ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
council memory setup      # interactive picker + configuration
council memory status     # check what's active
council memory off        # disable external provider
```

You can also select the active memory provider via `council plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.council/config.yaml`:
...
When a memory provider is active, Council automatically:
...
**Setup Wizard:**
```bash
council memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

On a fresh install, configure Honcho directly with `council memory setup honcho`. The legacy `council honcho setup` command still works (it now redirects to `council memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$COUNCIL_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$COUNCIL_HOME/honcho.json` > `~/.council/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/pokelabshq/council/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/council).
...
:::tip Migrating from `council honcho`
If you previously used `council honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Council profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Council profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Council profile. Host key `council` → default; `council.<profile>` for others. |
...
```bash
council profile create coder --clone
```

`--clone` creates a `council.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Council profile, creates host blocks for any profile without one, inherits settings from the default `council` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/pokelabshq/council/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/council).
...
council memory setup    # select "openviking"
# Or manually:
council config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.council/.env
```
...
**Setup:**
```bash
council memory setup    # select "mem0"
# Or manually:
council config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.council/.env
```

**Config:** `$COUNCIL_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `council-user` | User identifier |
| `agent_id` | `council` | Agent identifier |
...
**Setup:**
```bash
council memory setup    # select "hindsight"
# Or manually:
council config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.council/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p council ui start`

**Config:** `$COUNCIL_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `council` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchropokely on the server |
| `retain_context` | `conversation between Poke Council and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/pokelabshq/council/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
council memory setup    # select "holographic"
# Or manually:
council config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.council-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$COUNCIL_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
council memory setup    # select "retaindb"
# Or manually:
council config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.council/.env
```
...
# Then configure Council
council memory setup    # select "byterover"
# Or manually:
council config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$COUNCIL_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
council memory setup    # select "supermemory"
# Or manually:
council config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.council/.env
```

**Config:** `$COUNCIL_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `council` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `council-{identity}` → `council-coder`) to isolate memories per Council profile
...
| **Requires** | `pip install council-memori` + `council-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install council-memori
council-memori install
council config set memory.provider memori
council memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `council-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$COUNCIL_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$COUNCIL_HOME/` so each profile has its own credentials
