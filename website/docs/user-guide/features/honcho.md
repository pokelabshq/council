...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Council's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Council instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
council memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.council/.env
```
...
- `per-session` — each `council` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Council syncs them back at session init.
...
The `council honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `council memory setup honcho` (or run `council memory setup` and pick it from the list); the `council honcho` subcommand then appears on the next invocation.

```bash
council memory setup honcho    # Configure Honcho directly (works before activation)
council honcho status          # Connection status, config, and key settings
council honcho setup           # Redirects to `council memory setup` (post-activation alias)
council honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
council honcho peer            # Show or update peer names + dialectic reasoning level
council honcho mode            # Show or set recall mode (hybrid/context/tools)
council honcho tokens          # Show or set token budget for context and dialectic
council honcho identity        # Seed or show the AI peer's Honcho identity
council honcho sync            # Sync Honcho config to all existing profiles
council honcho peers           # Show peer identities across all profiles
council honcho sessions        # List known Honcho session mappings
council honcho map             # Map current directory to a Honcho session name
council honcho enable          # Enable Honcho for the active profile
council honcho disable         # Disable Honcho for the active profile
council honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `council honcho`

If you previously used the standalone `council honcho setup`:
...
No re-login or re-setup needed. Run `council memory setup` and select "honcho" — the wizard detects your existing config.
