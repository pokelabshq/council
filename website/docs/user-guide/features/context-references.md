...
Type `@` followed by a reference to inject content directly into your message. Council expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Council env: `$COUNCIL_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$COUNCIL_HOME/skills/.hub/`
