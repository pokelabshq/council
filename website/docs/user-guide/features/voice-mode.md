...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Poke Council — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Poke Council supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Council](/guides/use-voice-mode-with-council).
...
:::tip
The `~/.council/` directory and default `config.yaml` are created automatically the first time you run `council`. You only need to create `~/.council/.env` manually for API keys.
:::

:::tip Poke Portal covers both
A paid [Poke Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `council setup --portal` wires both up at once.
:::
...
Voice mode is available in both the **classic CLI** (`council chat`) and the **TUI** (`council --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.council/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
council                # Start the interactive CLI
```
...
1. Start the CLI with `council` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.council/config.yaml` (default: `ctrl+b`).
:::
...
```bash
council gateway        # Start the gateway (connects to configured platforms)
council gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@councilbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.council/.env`:
```bash
...
```bash
council gateway        # Start with existing configuration
```
...
If you are running Council inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.council/.env`
...
- Review gateway logs: `tail -f ~/.council/logs/gateway.log`
