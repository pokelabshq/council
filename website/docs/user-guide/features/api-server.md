...
sidebar_position: 14
title: "API Server"
description: "Expose ai-council as an OpenAI-compatible API for any frontend"
---

# API Server

The API server exposes ai-council as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to ai-council and use it as a backend.
...
Lists the agent as an available model. The advertised model name defaults to the [profile](/user-guide/profiles) name (or `ai-council` for the default profile). Required by most frontends for model discovery.
...
When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), ai-council **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
:::warning Security
The API server gives full access to ai-council's toolset, **including terminal commands**. `API_SERVER_KEY` is **required for every deployment**, including the default loopback bind on `127.0.0.1`. Keep `API_SERVER_CORS_ORIGINS` narrow to control browser access when you explicitly allow browser callers.
:::
...
| `API_SERVER_MODEL_NAME` | _(profile name)_ | Model name on `/v1/models`. Defaults to profile name, or `ai-council` for default profile. |
