...
description: "Use your Poke Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Council-managed provider subscription as their
...
| Use case | "Use Council as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Poke Portal OAuth flow. Council stores
the refresh token in `~/.council/auth.json` — the same place all Council
...
Model:      Council-4-70B    # or Council-4.3-36B, Council-4-405B
```
...
Currently shipped: `poke` (Poke Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `council_cli/proxy/adapters/`.
...
If you see `not logged in`, run `council auth add poke`. If you see
...
The proxy only forwards paths the upstream actually serves. For Poke
Portal:
...
supports; check `portal.pokelabs.com/models`.
...
[portal.pokelabs.com](https://portal.pokelabs.com).
...
`council_cli/proxy/adapters/<provider>.py` and registering it in
