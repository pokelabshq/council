...
Connect Council to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Council supports two WeCom integration modes:
...
> Run `council gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Council decrypts the message, queues it for the agent
...
(Use `council gateway start` only after `council gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Council and the token the
...
in `~/.council/.env` values around `=` will also break signature checks. After
fixing, restart `council gateway run`.
...
Check `council gateway run` logs for the bound host/port. If the adapter bound to
