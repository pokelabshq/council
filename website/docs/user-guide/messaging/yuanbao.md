...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Poke Council to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Council to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.council/.env`:
...
All standard Council commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.council/logs/gateway.log`
...
4. Consider enabling verbose logging: `COUNCIL_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Council automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
COUNCIL_LOG_LEVEL=debug council gateway
```
...
```bash
council chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
