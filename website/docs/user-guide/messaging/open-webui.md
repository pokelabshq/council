...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to Poke Council via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With Poke Council's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
Open WebUI connects to Poke Council's API server just like it would connect to OpenAI. Council handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.

:::important Runtime location
The API server is a **Council agent runtime**, not a pure LLM proxy. For each request, Council creates a server-side `AIAgent` on the API-server host. Tool calls run where that API server is running.

For example, if a laptop points Open WebUI or another OpenAI-compatible client at a Council API server on a remote machine, `pwd`, file tools, browser tools, local MCP tools, and other workspace tools run on the remote API-server host, not on the laptop.
:::

Open WebUI talks to Council server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.
...
- ensures `~/.council/.env` contains `API_SERVER_ENABLED`, `API_SERVER_HOST`, `API_SERVER_KEY`, `API_SERVER_PORT`, and `API_SERVER_MODEL_NAME`
- restarts the Council gateway so the API server comes up
- installs Open WebUI into `~/.local/open-webui-venv`
- writes a launcher at `~/.local/bin/start-open-webui-council.sh`
- on macOS, installs a `launchd` user service; on Linux with `systemd --user`, installs a user service there

Defaults:

- Council API: `http://127.0.0.1:8642/v1`
- Open WebUI: `http://127.0.0.1:8080`
- model name advertised to Open WebUI: `Poke Council`

Useful overrides:

```bash
OPEN_WEBUI_NAME='My Council UI' \
OPEN_WEBUI_ENABLE_SIGNUP=true \
COUNCIL_API_MODEL_NAME='My Poke Council' \
bash scripts/setup_open_webui.sh
```
...
```bash
council config set API_SERVER_ENABLED true
council config set API_SERVER_KEY your-secret-key
```

`council config set` auto-routes the flag to `config.yaml` and the secret to `~/.council/.env`. If the gateway is already running, restart it so the change takes effect:

```bash
council gateway stop && council gateway
```

### 2. Start Poke Council gateway
...
6. Enter:
   ...
   - **API Key**: the exact same value as `API_SERVER_KEY` in Council
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and Poke Council responds accordingly. Each request includes the full conversation history.
...
With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and Poke Council can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, Council also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. Poke Council creates a server-side `AIAgent` instance using the API server's profile, model/provider config, memory, skills, and configured API-server toolsets
...
Your agent has access to the same tools and capabilities as that API-server Council instance. If the API server is remote, those tools are remote too.

If you need tools to run against your **local** workspace today, run Council locally and point it at a pure LLM provider or pure OpenAI-compatible model proxy (for example vLLM, LiteLLM, Ollama, llama.cpp, OpenAI, OpenRouter, etc.). A future split-runtime mode for "remote brain, local hands" is being tracked in [#18715](https://github.com/pokelabshq/council/issues/18715); it is not the behavior of the current API server.
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | Poke Council's API URL (include `/v1`) |
...
Poke Council may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in Poke Council.
...
To run separate Council instances per user — each with their own config, memory, and skills — use [profiles](/user-guide/profiles). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.
...
The model dropdown will show `alice` and `bob` as distinct models. You can assign models to Open WebUI users via the admin panel, giving each user their own isolated Council agent.
...
```bash
council -p alice config set API_SERVER_MODEL_NAME "Alice's Agent"
```
:::
