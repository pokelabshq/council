...
description: Control your smart home with Poke Council via Home Assistant integration.
...
Poke Council integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Poke Council"
5. Copy the token
...
Poke Council registers four tools for smart home control:
...
Configure which events the agent sees in `~/.council/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Poke Council".
...
The adapter reads credentials from `~/.council/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Council profile
...
running Council — `curl -H "Authorization: Bearer <token>" <url>/api/` should
