...
description: "Chat with Council from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Council from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Council](/guides/use-voice-mode-with-council).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [Poke Portal](/integrations/poke-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["Council Gateway"]
...
```bash
council gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
council gateway              # Run in foreground
council gateway setup        # Configure messaging platforms interactively
council gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo council gateway install --system   # Linux only: install a boot-time system service
council gateway start        # Start the default service
council gateway stop         # Stop the default service
council gateway status       # Check default service status
council gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Poke Council to the latest version |
...
Configure per-platform overrides in `~/.council/gateway.json`:
...
council pairing approve telegram XKGH5N7P

# Other pairing commands:
council pairing list          # View pending + approved users
council pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Council appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.council/config.yaml`:
...
Each `/background` prompt spawns a **separate agent instance** that runs asynchropokely:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.council/config.yaml`:
...
```bash
COUNCIL_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
council gateway install               # Install as user service
council gateway start                 # Start the service
council gateway stop                  # Stop the service
council gateway status                # Check status
journalctl --user -u council-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo council gateway install --system
sudo council gateway start --system
sudo council gateway status --system
journalctl -u council-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Council will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Council installations on the same machine (with different `COUNCIL_HOME` directories), each gets its own systemd service name. The default `~/.council` uses `council-gateway`; other installations use `council-gateway-<hash>`. The `council gateway` commands automatically target the correct service for your current `COUNCIL_HOME`.
:::

### macOS (launchd)

```bash
council gateway install               # Install as launchd agent
council gateway start                 # Start the service
council gateway stop                  # Stop the service
council gateway status                # Check status
tail -f ~/.council/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.council.gateway.plist`. It includes three environment variables:
...
- **COUNCIL_HOME** — scopes the gateway to your Council installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `council gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `COUNCIL_HOME` directory gets its own launchd label. The default `~/.council` uses `ai.council.gateway`; other installations use `ai.council.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `council-cli` | Full access |
| Telegram | `council-telegram` | Full tools including terminal |
| Discord | `council-discord` | Full tools including terminal |
| WhatsApp | `council-whatsapp` | Full tools including terminal |
| Slack | `council-slack` | Full tools including terminal |
| Google Chat | `council-google_chat` | Full tools including terminal |
| Signal | `council-signal` | Full tools including terminal |
| SMS | `council-sms` | Full tools including terminal |
| Email | `council-email` | Full tools including terminal |
| Home Assistant | `council-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `council-mattermost` | Full tools including terminal |
| Matrix | `council-matrix` | Full tools including terminal |
| DingTalk | `council-dingtalk` | Full tools including terminal |
| Feishu/Lark | `council-feishu` | Full tools including terminal |
| WeCom | `council-wecom` | Full tools including terminal |
| WeCom Callback | `council-wecom-callback` | Full tools including terminal |
| Weixin | `council-weixin` | Full tools including terminal |
| BlueBubbles | `council-bluebubbles` | Full tools including terminal |
| QQBot | `council-qqbot` | Full tools including terminal |
| Yuanbao | `council-yuanbao` | Full tools including terminal |
| Microsoft Teams | `council-teams` | Full tools including terminal |
| API Server | `council-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `council-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.council/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
