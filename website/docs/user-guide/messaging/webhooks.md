...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Poke Council — Webhooks Tutorial"
...
1. Enable via `council gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `council webhook subscribe`
...
Add the `github-pr` route to your `~/.council/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Council processes the event, and posts a review comment on the PR.
...
- **Synchropoke response** — the POST returns `200 OK` once delivery succeeds, or `502` if the target rejects it, so your upstream service can retry intelligently
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `council webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
council webhook remove github-issues
```

### Test a subscription

```bash
council webhook test github-issues
council webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.council/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `council webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `council gateway run`
