...
If you only run one Council agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Council agents that should all
...
(`ai.council.gateway-<name>.plist`) or systemd user service
(`council-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
council profile create coder
council profile create personal-bot
council profile create research
...
`~/.local/bin/council-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: council-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    council gateway "$action"
  else
    council -p "$profile" gateway "$action"
  fi
}
...
```bash
council-gateways start      # start every configured profile
council-gateways stop       # stop every configured profile
council-gateways restart    # restart all
council-gateways status     # status across all
council-gateways list       # delegates to `council gateway list`
```

:::tip
The `default` profile is targeted with `council gateway <action>` (no `-p`),
not `council -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `council -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.council.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/council-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.council.gateway.plist` /
`council-gateway.service`.
...
```bash
# Default profile
tail -f ~/.council/logs/gateway.log
tail -f ~/.council/logs/gateway.error.log

# Named profile
tail -f ~/.council/profiles/<name>/logs/gateway.log
tail -f ~/.council/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.council/logs/gateway.log ~/.council/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
council logs --tail              # follow default profile
council -p coder logs --tail     # follow one profile
council logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
council profile list             # profiles + model + gateway state
council-gateways status          # full status across every profile
launchctl list | grep council    # macOS — PIDs and labels
systemctl --user list-units 'council-gateway-*'   # Linux — units
```
...
```
~/.council/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.council/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
council config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.council/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=council --why="gateways running" \
  sleep infinity &
...
`council-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.council/.env ~/.council/profiles/*/.env
```

## Updating the code

`council update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `council gateway start` after a previous `council gateway stop`. The
...
```bash
ps -ef | grep "council_cli.*-p <profile>"
cat ~/.council/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.council.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.council.gateway-<profile>.plist

# Linux
systemctl --user restart council-gateway-<profile>.service
```

### Health check

```bash
council doctor                  # default profile
council -p <profile> doctor     # one profile
```
