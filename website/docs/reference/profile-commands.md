...
This page covers all commands related to [Council profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `council profile` without a subcommand shows help.
...
```bash
council profile use <name>
```

Sets `<name>` as the active profile. All subsequent `council` commands (without `-p`) will use this profile.
...
council profile use default
```

## `council profile create`

```bash
council profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `council profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `council update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
council profile create mybot

# Clone config only from current profile
council profile create work --clone

# Clone everything from current profile
council profile create backup --clone-all

# Clone config from a specific profile
council profile create work2 --clone --clone-from work
```

## `council profile describe`

```bash
council profile describe [<name>] [options]
```
...
council profile describe researcher

# Set it explicitly
council profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
council profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
council profile describe --all --auto
```

## `council profile delete`

```bash
council profile delete <name> [options]
```
...
```bash
council profile delete mybot
council profile delete mybot --yes
```
...
```bash
council profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Council home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ council profile show work
Profile: work
Path:    ~/.council/profiles/work
...
## `council profile alias`

```bash
council profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Council installation.
...
```bash
council profile alias work
# Creates/updates ~/.local/bin/work

council profile alias work --name mywork
# Creates ~/.local/bin/mywork

council profile alias work --remove
# Removes the wrapper script
```

## `council profile rename`

```bash
council profile rename <old-name> <new-name>
```
...
```bash
council profile rename mybot assistant
# ~/.council/profiles/mybot → ~/.council/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `council profile export`

```bash
council profile export <name> [options]
```
...
```bash
council profile export work
# Creates work.tar.gz in the current directory

council profile export work -o ./work-2026-03-29.tar.gz
```

## `council profile import`

```bash
council profile import <archive> [options]
```
...
```bash
council profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

council profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`council profile export` / `import` are still the right commands for
...
### `council profile install`

```bash
council profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `council -p telemetry`). |
...
council profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
council profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
council profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
council profile install ./telemetry/
```

### `council profile update`

```bash
council profile update <name> [--force-config] [--yes]
```
...
### `council profile info`

```bash
council profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Council version, author, env var requirements, the source URL/path, and
...
`council profile list` also shows the distribution name and version in a
`Distribution` column, and `council profile show <name>` / `delete <name>`
...
council profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
council profile install https://github.com/your-org/internal-assistant.git
```
...
`council_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Council version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Council can clone from.
3. Tell recipients to run `council profile install <your-repo-url>`.
...
## `council -p` / `council --profile`

```bash
council -p <name> <command> [options]
council --profile <name> <command> [options]
```

Global flag to run any Council command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
council -p work chat -q "Check the server status"
council --profile dev gateway start
council -p personal skills list
council -p work config edit
```

## `council completion`

```bash
council completion <shell>
```
...
- `council profile <TAB>` — subcommands (list, use, create, etc.)
- `council profile use <TAB>` — profile names
- `council -p <TAB>` — profile names
