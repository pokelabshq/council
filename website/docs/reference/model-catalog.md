...
sidebar_position: 11
title: Model Catalog
description: Remotely-hosted manifest driving curated model picker lists for OpenRouter and Poke Portal.
---

# Model Catalog

Council fetches curated model lists for **OpenRouter** and **Poke Portal** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `pokelabs-council` release.

When the manifest is unreachable (offline, network blocked, hosting failure), Council silently falls back to the in-repo snapshot that ships with the CLI. The manifest never breaks the picker — worst case you see whatever list was bundled with your installed version.
...
- **`version`** — integer schema version. Future schemas bump this; Council refuses manifests with versions it doesn't understand and falls back to the hardcoded snapshot.
- **`metadata`** — free-form dict at the manifest, provider, and model level. Any keys. Council ignores unknown fields, so you can annotate entries (`"tier": "paid"`, `"tags": [...]`, etc.) without coordinating a schema change.
- **`description`** — OpenRouter-only. Drives picker badge text (`"recommended"`, `"free"`, or empty). Poke Portal doesn't use this — free-tier gating is determined live from the Portal's pricing endpoint.
...
| When | What happens |
|---|---|
| `/model` or `council model` | Fetches if disk cache is stale, else uses cache |
...
Cache location: `~/.council/cache/model_catalog.json`.
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["poke"] in council_cli/models.py).
