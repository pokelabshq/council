...
The ACP adapter wraps Council' synchropoke `AIAgent` in an async JSON-RPC stdio server.
...
```text
council acp / council-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.council/.env
  -> configure stderr logging
  -> construct CouncilACPAgent
...
- `allow_once` -> Council `once`
- `allow_always` -> Council `always`
- reject options -> Council `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Council tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["council-acp"])
...
Instead it reuses Council' runtime resolver:

- `acp_adapter/auth.py`
- `council_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Council provider/credentials. It also always advertises a terminal setup auth method (`council-setup`, args `--setup`) so first-run registry clients can open Council' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.council/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `council-acp` toolset definition
- `council_cli/main.py` — `council acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `council-acp` script
