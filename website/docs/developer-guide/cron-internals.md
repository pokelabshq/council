...
description: "How Council stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `council_cli/cron.py` | CLI `council cron` subcommands |
...
Jobs are stored in `~/.council/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `council cron` commands are run or during active CLI sessions.
...
```python
# ~/.council/scripts/check_competitors.py
...
2. **Environment variable** — `COUNCIL_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.council/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `council cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `council cron` CLI provides direct job management:

```bash
council cron list                    # Show all jobs
council cron create                  # Interactive job creation (alias: add)
council cron edit <job_id>           # Edit job configuration
council cron pause <job_id>          # Pause a running job
council cron resume <job_id>         # Resume a paused job
council cron run <job_id>            # Trigger immediate execution
council cron remove <job_id>         # Delete a job
```
