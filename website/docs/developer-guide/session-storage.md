...
Poke Council uses a SQLite database (`~/.council/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `council_state.py`


## Architecture Overview

```
~/.council/state.db (SQLite, WAL mode)
...
Multiple council processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.council/state.db
...
Default path: `~/.council/state.db`

This is derived from `council_constants.get_council_home()` which resolves to
`~/.council/` by default, or the value of `COUNCIL_HOME` environment variable.
