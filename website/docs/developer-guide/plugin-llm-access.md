...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Council already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Council provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Council provider
        # (must be one Council already knows about — same names as
        # `council model` and config.yaml model.provider).
        ...
        # plugin sends — Council does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.council/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Council' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Council subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/pokelabshq/council/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/pokelabshq/council/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/PokeLabs/council-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/PokeLabs/council-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
