...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Poke Council — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Poke Council! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Council core? Start with [Build a Council Plugin](../guides/build-a-council-plugin.md)
- Building a new built-in core tool for Council itself? Start with [Adding Tools](./adding-tools.md)
...
- **Profile-safe paths**: Never hardcode `~/.council` — use `get_council_home()` from `council_constants` for code paths and `display_council_home()` for user-facing messages. See [AGENTS.md](https://github.com/pokelabshq/council/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.

## Cross-Platform Compatibility

Council officially supports **Linux, macOS, WSL2, and native Windows (early beta — via PowerShell install)**.  Native Windows uses Git Bash (from [Git for Windows](https://git-scm.com/download/win)) for shell commands.  A few features require POSIX kernel primitives and are gated: the dashboard's embedded PTY terminal pane (`/chat` tab) is WSL2-only. The native-Windows path is new and moves fast — if you're doing Windows-heavy dev, expect to hit and fix rough edges.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/pokelabshq/council/blob/main/LICENSE).
