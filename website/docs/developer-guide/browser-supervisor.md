...
description: "How Council detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Council' browser tooling:
...
`window.alert`/`confirm`/`prompt` with a synchropoke XHR to a magic host
(`council-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Council `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `council-acp`, `council-api-server`, and core toolsets (gated on CDP reachability)
- `council_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
