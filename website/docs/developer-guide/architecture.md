...
sidebar_position: 1
title: "Architecture"
description: "Poke Council internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Poke Council internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ council_state.py   │              │ Browser (5 backends)  │
...
├── council_state.py           # SQLite session/state database with FTS5
├── council_constants.py       # COUNCIL_HOME, profile-aware paths
...
│
├── council_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `council` subcommands (large file)
...
│   ├── skills_config.py      # council skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # council tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # council gateway start/stop
│
...
```text
User input → CouncilCLI.process_input()
...
The synchropoke orchestration engine (`AIAgent` in `run_agent.py`). Handles provider selection, prompt construction, tool execution, retries, fallback, callbacks, compression, and persistence. Supports three API modes for different provider backends.
...
Three discovery sources: `~/.council/plugins/` (user), `.council/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `council plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-council-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Council as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`council -p <name>`) gets its own COUNCIL_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
