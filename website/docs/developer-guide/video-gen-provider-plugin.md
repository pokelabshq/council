...
description: "How to build a video-generation backend plugin for Poke Council"
---
...
Council scans for video-gen backends in three places:
...
class MyVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "My Backend",
            "badge": "paid",
            "tag": "Short description shown in `council tools`",
            "env_vars": [
                {
                    "key": "MY_API_KEY",
...
The provider's `capabilities()` advertises which of these are honored. The agent sees the active backend's capabilities in the tool description, dynamically rebuilt when the user changes backend via `council tools`.
...
The user picks `veo3.1` once in `council tools`. The agent never thinks about endpoints — it just passes (or doesn't pass) `image_url`.
...
If your backend returns base64, use `save_b64_video()` to write under `$COUNCIL_HOME/cache/videos/`. For raw bytes from a follow-up HTTP fetch, use `save_bytes_video()`. Otherwise return the upstream URL directly — the gateway resolves remote URLs on delivery.
