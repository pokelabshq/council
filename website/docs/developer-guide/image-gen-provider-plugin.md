...
description: "How to build an image-generation backend plugin for Poke Council"
---
...
The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `council tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.council/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `council plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `council tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `council tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `council tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $COUNCIL_HOME/cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `council plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `council tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `council tools` model picker |
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` — it writes to `$COUNCIL_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` and returns the absolute `Path`. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.council/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `council plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export COUNCIL_HOME=/tmp/council-imggen-test
mkdir -p $COUNCIL_HOME/plugins/image_gen/my-backend
# …copy __init__.py + plugin.yaml into that dir…

export MY_BACKEND_API_KEY=your-test-key
council plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $COUNCIL_HOME/config.yaml
echo "  provider: my-backend" >> $COUNCIL_HOME/config.yaml

# Exercise it
council -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `council tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
`my_backend_imggen_package` must expose a top-level `register` function. See [Distribute via pip](/guides/build-a-council-plugin#distribute-via-pip) in the general plugin guide for the full setup.
