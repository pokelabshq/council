---
sidebar_position: 6
title: "Use MCP with Council"
description: "A practical guide to connecting MCP servers to Poke Council, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Council

This guide shows how to actually use MCP with Poke Council in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Council tool
- you want Council to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Council to internal APIs, databases, or company systems without modifying Council core

Do not use MCP when:
- a built-in Council tool already solves the job well
...
- Council remains the agent
- MCP servers contribute tools
- Council discovers those tools at startup or reload time
...
If you installed Council with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Council banner/status should show MCP integration when configured
- ask Council what tools it has available
...
## WSL2: bridge Council in WSL to Windows Chrome

This is the practical setup when:

- Council runs inside WSL2
...
In this setup, Council does **not** connect to Chrome directly. Instead:

- Council runs in WSL
- Council starts a local stdio MCP server
...
```text
Council (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Council stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Council core browser transport
...
```bash
council mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
council mcp test chrome-devtools-win
```

Then start a fresh Council session or run:
...
Once loaded, Council can use the MCP-prefixed browser tools directly. For example:
...
If Council runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Council from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Council from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Council:
...
2. Council-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Council will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Council to reason over a bounded workspace.
...
Now Council can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Council core.
...
Because Council now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
