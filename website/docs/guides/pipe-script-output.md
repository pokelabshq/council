...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `council send`."
---

# Pipe Script Output to Messaging Platforms

`council send` is a small, scriptable CLI that pushes a message to any
messaging platform Council is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | council send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `council
...
council send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
council send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
council send --list

# Filter by platform
council send --list telegram
```
...
Any platform Council ships adapters for works as a target:
...
`council send` resolves the message body in this order:

1. **Positional argument** — `council send --to telegram "hi"`
2. **`--file PATH`** — `council send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | council send --to telegram`

When stdin is a TTY (no pipe), Council does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  council send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `council send` reuses your Council config, the same script works on
any host where Council is installed — no need to export bot tokens into
...
`council send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  council send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | council send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.council/bin/council send \
...
```bash
./train.py --epochs 200 && \
  council send --to telegram "training done" || \
  council send --to telegram "training failed (exit $?)"
```
...
council send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(council send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `council send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `council send` calls
the platform's REST endpoint directly using credentials from
`~/.council/.env` and `~/.council/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `council gateway start` and retry.
...
# Machine-readable
council send --list --json
```

The listing is built from `~/.council/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`council gateway
...
| Approach | Multi-platform | Reuses Council creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `council send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`council send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `council send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `council send` shares with cron delivery.
