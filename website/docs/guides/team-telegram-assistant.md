...
This tutorial walks you through setting up a Telegram bot powered by Poke Council that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Poke Council installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.council/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Council itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Council Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_council_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Poke Council. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.council/.env`:
...
```
[Gateway] Starting Council Gateway...
...
# View live logs
journalctl --user -u council-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `council gateway install` to update the plist.
:::
...
```bash
council gateway stop && council gateway start
```
...
council pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.council/.env`:
...
Control how much detail the bot shows when using tools. In `~/.council/config.yaml`:
...
Customize how the bot communicates by editing `~/.council/SOUL.md`:

For a full guide, see [Use SOUL.md with Council](/guides/use-soul-with-council).
...
```bash
# From the CLI
council cron list          # View all scheduled jobs
council cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u council-gateway` (Linux) or `~/.council/logs/gateway.log` (macOS) |
| Cron job output | `~/.council/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.council/cron/jobs.json` |
| Pairing data | `~/.council/pairing/` |
| Session history | `~/.council/sessions/` |
