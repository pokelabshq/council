...
description: "Diagnose and fix common Council cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`council gateway` for foreground, or `council gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `council cron tick`.
...
```bash
date
council cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.council/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.council/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.council/.env` |
...
| `local` | Write access to `~/.council/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `council cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `council skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `council skills list`.
...
2. `~/.council/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `council cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Council config directory). Verify:
```bash
ls ~/.council/scripts/your-script.py   # Must exist
council cron edit <job_id> --script ~/.council/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `council skills install <skill-name>`.
...
Jobs are stored in `~/.council/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.council/cron/jobs.json
chmod 600 ~/.council/cron/jobs.json   # Your user should own it
```
...
```bash
council cron list                    # Show all jobs, states, next_run times
council cron run <job_id>            # Schedule for next tick (for testing)
council cron edit <job_id>           # Fix configuration issues
council logs                         # View recent Council logs
council skills list                  # Verify installed skills
```
...
1. Run the job with `council cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.council/logs/agent.log` for scheduler messages and `~/.council/logs/errors.log` for warnings
3. Open an issue at [github.com/pokelabshq/council](https://github.com/pokelabshq/council) with:
