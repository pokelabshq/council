---
sidebar_position: 8
title: "Use Voice Mode with Council"
description: "A practical guide to setting up and using Council voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Council
...
:::tip
[Poke Portal](/integrations/poke-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Council sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Council.
...
## Step 1: make sure normal Council works first
...
Council supports both local and cloud speech stacks.
...
### If you use `council setup`

If you choose NeuTTS in the setup wizard, Council checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Council transcribes and responds
...
- asking Council to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Council loop.

## Tuning CLI behavior

### Silence threshold

If Council starts/stops too aggressively, tune:
...
Council stays a normal chat bot, but can speak replies.
...
- you want Council to function like a portable research or ops assistant
...
Council joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Council detects speech boundaries
- transcripts are posted in the associated text channel
- Council responds in text and audio
...
1. get text Council working
2. install `pokelabs-council[voice]`
