...
```bash
council teams-pipeline validate
```
...
```bash
council teams-pipeline token-health
council teams-pipeline token-health --force-refresh
```
...
```bash
council teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
council teams-pipeline maintain-subscriptions
council teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Council cron (recommended if you already run the Council gateway)

Council ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.council/scripts/`. First create the script:

```bash
mkdir -p ~/.council/scripts
cat > ~/.council/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec council teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.council/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
council cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/council-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Council Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/council teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/council-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Council Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now council-teams-pipeline-maintain.timer
systemctl list-timers council-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/council teams-pipeline maintain-subscriptions >> /var/log/council/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.council/.env` at the top of a wrapper script that crontab calls.
...
```bash
council teams-pipeline subscriptions   # should show expirationDateTime advanced
council teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
council teams-pipeline list
council teams-pipeline list --status failed
council teams-pipeline show <job-id>
```

### Replay a stored job

```bash
council teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
council teams-pipeline fetch --meeting-id <meeting-id>
council teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `council teams-pipeline maintain-subscriptions --dry-run`
- inspect `council teams-pipeline list --status failed`
...
- run `council teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `council teams-pipeline run`
...
- [ ] `council teams-pipeline validate` returns an OK snapshot
- [ ] `council teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Council cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
