...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Poke Council — no API key required"
---

# MiniMax OAuth

Poke Council supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Council automatically refreshes your session.
...
# → Council opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.council/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.council/.env
```
...
```bash
council auth add minimax-oauth --no-browser
```

Council will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Council implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Council generates a PKCE verifier / challenge pair and a random state value.
...
4. Council polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.council/auth.json` under the `minimax-oauth` key.
...
```bash
council config set model.default MiniMax-M2.7
council config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.council/config.yaml` will contain entries similar to:
...
```bash
council --provider minimax-oauth    # canonical
council --provider minimax-portal   # alias
council --provider minimax-global   # alias
council --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `council setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
council --provider minimax-oauth
```
...
Council refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Council marks the session as requiring re-login.
...
**Fix:** run `council auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Council raises a timeout error.

**Fix:** re-run `council auth add minimax-oauth` (or `council model`). The flow starts fresh.

### State mismatch (possible CSRF)

Council detected that the `state` value returned by the authorization server does not match what it sent.
...
If `council` cannot open a browser window, use `--no-browser`:

```bash
council auth add minimax-oauth --no-browser
```

Council prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `council model` and select MiniMax (OAuth), or run `council auth add minimax-oauth`.
...
```bash
council auth remove minimax-oauth
```
...
- [council doctor](../reference/cli-commands.md)
