...
title: "Build a Council Plugin"
description: "Step-by-step guide to building a complete Council plugin with tools, hooks, data files, and skills"
---

# Build a Council Plugin

This guide walks through building a complete Council plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Council has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `COUNCIL_LOCAL_STT_COMMAND` to a shell template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.council/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `council skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.council/plugins/calculator
cd ~/.council/plugins/calculator
```
...
This tells Council: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Council may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `council my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Council continues fine
...
If your plugin doesn't show up — or shows up but isn't loading — set `COUNCIL_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
COUNCIL_PLUGINS_DEBUG=1 council plugins list
```
...
The same logs are always written to `~/.council/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
council logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `council plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.council/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.council/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.council/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.council/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `council plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Council into installing arbitrary packages — only specs Council itself ships are eligible |
...
auditable in `~/.council/logs/agent.log`. Plugins load after built-in
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Council injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.
...
- **The system prompt is Council's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `council <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for council my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: council my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for council my-plugin."""
...
After registration, users can run `council my-plugin status`, `council my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `council name` in a terminal |
...
Council has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.council/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `council gateway status` without SDK instantiation.
...
Council also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Council without any Python plugin. Declare them in `~/.council/config.yaml`:
...
Council connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.council/hooks/<name>/`:

```yaml
# ~/.council/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.council/hooks/long-task-alert/handler.py
...
council skills search my-workflow --source myorg/skills-repo
council skills install myorg/skills-repo/my-workflow
```
...
For STT, point `COUNCIL_LOCAL_STT_COMMAND` at a shell template. Supported placeholders: `{input_path}`, `{output_path}`, `{format}`, `{voice}`, `{model}`, `{speed}` (TTS); `{input_path}`, `{output_dir}`, `{language}`, `{model}` (STT). Any path-interacting CLI is automatically a plugin.
...
```bash
pip install council-plugin-calculator
# Plugin auto-discovered on next council startup
```
...
```python
# Wrong — will break if Council passes extra context
def handler(args):
    ...
