...
sidebar_position: 2
title: "安装"
description: "在 Linux、macOS、WSL2、原生 Windows（早期 Beta）或通过 Termux 在 Android 上安装 Poke Council"
---

# 安装

使用一行安装命令，两分钟内即可启动并运行 Poke Council。
...
:::warning 早期 BETA
原生 Windows 支持处于**早期 beta** 阶段。常见路径下可正常安装和运行，但尚未像我们的 POSIX 安装程序那样经过广泛测试。遇到问题请[提交 issue](https://github.com/pokelabshq/council/issues)。目前在 Windows 上最稳定的方案是在 **WSL2** 内使用上方的 Linux/macOS 一行命令。
:::
...
2. 否则，它会下载便携式 **PortableGit**（约 50MB，来自官方 `git-for-windows` GitHub 发布页）并解压到 `%LOCALAPPDATA%\council\git`。无需管理员权限，完全隔离——不会干扰任何系统 Git 安装，无论其状态如何。（在 32 位 Windows 上会回退到 MinGit，因为 PortableGit 仅提供 64 位和 ARM64 资产；依赖 bash 的 Council 功能在 32 位主机上无法使用。）

**为什么不使用 winget？** 早期设计通过 `winget install Git.Git` 自动安装 Git，但当系统 Git 安装处于部分损坏状态时，winget 会严重失败（而这恰恰是用户最需要安装程序正常工作的时候）。便携式 Git 方案绕过了 winget、Windows 安装程序注册表以及任何现有系统 Git。如果 Council 的 Git 安装本身出现问题，执行 `Remove-Item %LOCALAPPDATA%\council\git` 并重新运行安装程序即可——对系统无影响，无需卸载操作。

安装程序还会将 `COUNCIL_GIT_BASH_PATH` 设置为找到的 `bash.exe` 路径，以便 Council 在新 shell 中确定性地解析它。

如果你偏好 WSL2，上方的 Linux 安装程序可在其中运行；原生安装和 WSL 安装可以共存而不冲突（原生数据位于 `%LOCALAPPDATA%\council`，WSL 数据位于 `~/.council`）。

**桌面安装程序（替代方案）：** 也提供一个轻量 GUI 安装程序——下载 Council Desktop，运行 `.exe`，首次启动时它会在后台调用 `install.ps1` 来配置 Python（通过 `uv`）、Node、PortableGit 及其余依赖。桌面应用和 PowerShell 安装的 CLI 共享相同的安装目录和数据目录，可以单独或同时使用。详见 [Windows（原生）指南](../user-guide/windows-native#desktop-installer-alternative)。

### Android / Termux

Council 现在也提供 Termux 感知的安装路径：
...
- **CLI（`council chat`、`council setup`、`council gateway` 等）** — 原生，使用默认终端
...
如果遇到编码相关的 bug 并希望回退到旧版 cp1252 stdio 路径（用于问题定位），请在环境中设置 `COUNCIL_DISABLE_WINDOWS_UTF8=1`。
:::

### 安装程序做了什么

安装程序自动处理一切——所有依赖（Python、Node.js、ripgrep、ffmpeg）、仓库克隆、虚拟环境、全局 `council` 命令配置以及 LLM 提供商配置。完成后即可开始聊天。
...
council gateway setup  # 配置消息平台
council config set     # 设置单个配置项
council setup          # 或运行完整的设置向导一次性配置所有内容
```

:::tip 最快路径：Poke Portal
...
该命令一次性完成登录、设置 Poke 为提供商并开启 Tool Gateway。
:::
...
支持以专用非特权用户身份运行 Council（例如 `council` systemd 服务账户，或任何没有 `sudo` 权限的用户）。安装路径中真正需要 root 权限的只有 Playwright 的 `--with-deps` 步骤，该步骤通过 `apt` 安装 Chromium 所需的共享库（`libnss3`、`libxkbcommon` 等）。安装程序会检测 sudo 是否可用，并在不可用时优雅降级——它会将 Chromium 二进制安装到服务用户自己的 Playwright 缓存中，并打印管理员需要单独运行的确切命令。
...
3. **使 `council` 对服务用户的 shell 可用。** 安装程序将启动器写入 `~/.local/bin/council`。系统服务账户通常具有不包含 `~/.local/bin` 的最小 PATH。可以将其添加到用户环境，或将启动器符号链接到系统位置：
   ```bash
...
| 问题 | 解决方案 |
|---------|----------|
| `council: command not found` | 重新加载 shell（`source ~/.bashrc`）或检查 PATH |
| `API key not set` | 运行 `council model` 配置提供商，或 `council config set OPENROUTER_API_KEY your_key` |
| 更新后配置丢失 | 运行 `council config check`，然后运行 `council config migrate` |

如需更多诊断信息，运行 `council doctor`——它会告诉你确切缺少什么以及如何修复。
