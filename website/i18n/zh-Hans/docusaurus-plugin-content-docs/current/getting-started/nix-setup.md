...
description: "使用 Nix 安装和部署 Poke Council——从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 安装配置

Poke Council 提供了一个 Nix flake，支持三个层级的集成：
...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`council setup`、`council gateway install`、编辑配置）与标准安装完全相同。

**对于 NixOS 模块用户**，整个生命周期有所不同：配置存放在 `configuration.nix` 中，密钥通过 sops-nix/agenix 管理，服务是一个 systemd 单元，CLI 配置命令被屏蔽。管理 council 的方式与管理其他 NixOS 服务相同。
:::
...
就这些。`nixos-rebuild switch` 会创建 `council` 用户、生成 `config.yaml`、连接密钥并启动 gateway——这是一个长期运行的服务，将 Agent 连接到消息平台（Telegram、Discord 等）并监听传入消息。
...
设置 `addToSystemPackages = true` 有两个作用：将 `council` CLI 添加到系统 PATH，**并**在系统范围内设置 `COUNCIL_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `council` 会创建独立的 `~/.council/` 目录。
:::

### 容器感知 CLI

:::info
当 `container.enable = true` 且 `addToSystemPackages = true` 时，主机上的**所有** `council` 命令都会自动路由到托管容器中执行。这意味着你的交互式 CLI 会话在与 gateway 服务相同的环境中运行——可以访问所有容器内安装的包和工具。

- 路由是透明的：`council chat`、`council sessions list`、`council version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
- 如果容器未运行，CLI 会短暂重试（交互式使用时显示 5 秒 spinner，脚本中静默等待 10 秒），然后以明确的错误退出——不会静默回退
- 对于在 council 代码库上工作的开发者，设置 `COUNCIL_DEV=1` 可绕过容器路由，直接运行本地检出版本
...
`hostUsers` 中列出的用户会自动加入 `council` 组以获得文件权限访问。
...
CLI 会自动检测何时需要 sudo 并透明地使用它。没有此配置，你需要手动运行 `sudo council chat`。
:::
...
:::note 模型命名
`settings.model.default` 使用你的提供商所期望的模型标识符。使用 [OpenRouter](https://openrouter.ai)（默认）时，格式如 `"anthropic/claude-sonnet-4"` 或 `"google/gemini-3-flash"`。如果直接使用提供商（Anthropic、OpenAI），请将 `settings.model.base_url` 指向其 API，并使用其原生模型 ID（例如 `"claude-sonnet-4-20250514"`）。未设置 `base_url` 时，Council 默认使用 OpenRouter。
:::
...
```yaml
# secrets/council.yaml（使用 sops 加密）
council-env: |
...
`documents` 选项将文件安装到 Agent 的工作目录（即 `workingDirectory`，Agent 将其作为工作区读取）。Council 按约定查找特定文件名：
...
:::tip
`env` 值中的环境变量在运行时从 `$COUNCIL_HOME/.env` 解析。使用 `environmentFiles` 注入密钥——切勿将 token 直接放入 Nix 配置。
:::
...
对于使用 OAuth 2.1 的服务器，设置 `auth = "oauth"`。Council 实现了完整的 PKCE 流程——元数据发现、动态客户端注册、token 交换和自动刷新。
...
Token 存储在 `$COUNCIL_HOME/mcp-tokens/<server-name>.json` 中，在重启和重建后持久保留。

<details>
<summary><strong>无头服务器上的初始 OAuth 授权</strong></summary>

首次 OAuth 授权需要基于浏览器的同意流程。在无头部署中，Council 将授权 URL 打印到 stdout/日志，而不是打开浏览器。

**方案 A：交互式引导** — 通过 `docker exec`（容器）或 `sudo -u council`（原生）运行一次流程：
...
# 确保：chown council:council，chmod 0600
```
...
当 council 通过 NixOS 模块运行时，以下 CLI 命令会被**屏蔽**，并显示指向 `configuration.nix` 的描述性错误：

| 被屏蔽的命令 | 原因 |
|---|---|
| `council setup` | 配置是声明式的——请在 Nix 配置中编辑 `settings` |
| `council config edit` | 配置由 `settings` 生成 |
| `council config set <key> <value>` | 配置由 `settings` 生成 |
| `council gateway install` | systemd 服务由 NixOS 管理 |
| `council gateway uninstall` | systemd 服务由 NixOS 管理 |
...
启用容器模式后，council 在持久化 Ubuntu 容器内运行，Nix 构建的二进制文件以只读方式从主机绑定挂载：
...
Nix 构建的二进制文件能在 Ubuntu 容器内运行，是因为 `/nix/store` 被绑定挂载——它携带自己的解释器和所有依赖，不依赖容器的系统库。容器入口点通过 `current-package` 符号链接解析：`/data/current-package/bin/council gateway run --replace`。执行 `nixos-rebuild switch` 时，只更新符号链接——容器继续运行。
...
仅当容器的**身份哈希**发生变化时才会重建容器。哈希涵盖：schema 版本、镜像、`extraVolumes`、`extraOptions` 和入口点脚本。环境变量、settings、文档或 council 包本身的变更**不会**触发重建。

:::warning 可写层丢失
当身份哈希发生变化（镜像升级、新卷、新容器选项）时，容器会被销毁并从 `container.image` 的全新拉取重建。可写层中通过 `apt install`、`pip install` 或 `npm install` 安装的包将丢失。`/data` 和 `/home/council` 中的状态会保留（这些是绑定挂载）。

如果 Agent 依赖特定包，考虑将其烘焙到自定义镜像中（`container.image = "my-registry/council-base:latest"`），或在 Agent 的 SOUL.md 中编写安装脚本。
:::

### GC Root 保护

`preStart` 脚本在 `${stateDir}/.gc-root` 创建一个指向当前 council 包的 GC root。这可以防止 `nix-collect-garbage` 删除正在运行的二进制文件。如果 GC root 损坏，重启服务会重新创建它。

---

## 插件

NixOS 模块支持声明式插件安装——无需命令式的 `council plugins install`。
...
插件在激活时以符号链接方式安装到 `$COUNCIL_HOME/plugins/`。Council 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。
...
该包的 `site-packages` 会添加到 council wrapper 的 PYTHONPATH 中。`importlib.metadata` 在会话启动时发现入口点。
...
:::note
构建时冲突检查可防止插件包覆盖核心 council 依赖。如果插件提供了封闭 venv 中已有的包，`nixos-rebuild` 会以明确的错误失败。
:::
...
nix build .#checks.x86_64-linux.managed-guard       # COUNCIL_MANAGED 屏蔽变更操作
...
| `bundled-skills` | skills 目录存在，包含 SKILL.md 文件，wrapper 中设置了 `COUNCIL_BUNDLED_SKILLS` |
...
| `stateDir` | `str` | `"/var/lib/council"` | 状态目录（`COUNCIL_HOME` 的父目录） |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent 工作目录（`MESSAGING_CWD`） |
| `addToSystemPackages` | `bool` | `false` | 将 `council` CLI 添加到系统 PATH 并在系统范围内设置 `COUNCIL_HOME` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | 包含密钥的 env 文件路径。激活时合并到 `$COUNCIL_HOME/.env` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | `council gateway` 的额外参数 |
| `extraPackages` | `listOf package` | `[]` | Agent 可用的额外包。添加到 council 用户的每用户 profile，终端命令、skills 和 cron 任务均可见 |
| `extraPlugins` | `listOf package` | `[]` | 以符号链接方式安装到 `$COUNCIL_HOME/plugins/` 的目录插件包。每个包必须包含 `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.council` 符号链接（指向服务 stateDir）的交互式用户，自动加入 `council` 组 |
...
```
/var/lib/council/                     # stateDir（归 council:council 所有，权限 0750）
├── .council/                         # COUNCIL_HOME
...
| 容器路径 | 主机路径 | 模式 | 说明 |
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Council 二进制文件 + 所有 Nix 依赖 |
| `/data` | `/var/lib/council` | `rw` | 所有状态、配置、工作区 |
| `/home/council` | `${stateDir}/home` | `rw` | 持久化 Agent home——`pip install --user`、工具缓存 |
