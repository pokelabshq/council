...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.council/.env ] && grep -q "^GITHUB_TOKEN=" ~/.council/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.council/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Poke Council\",
    \"comments\": [
...
```bash
source "${COUNCIL_HOME:-$HOME/.council}/skills/github/github-auth/scripts/gh-env.sh"
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by Poke Council. Code looks clean — good test coverage, no security concerns."
