...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 Council 对话 — 架构与配置概览"
---

# 消息网关

通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Feishu/Lark、WeCom、Weixin、BlueBubbles（iMessage）、QQ、Yuanbao、Microsoft Teams、LINE、ntfy 或浏览器与 Council 对话。网关是一个单一后台进程，连接所有已配置的平台，管理会话，运行 cron 任务，并传递语音消息。

完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with Council](/guides/use-voice-mode-with-council)。
...
```bash
council gateway setup        # 交互式配置所有消息平台
```
...
```bash
council gateway              # 在前台运行
council gateway setup        # 交互式配置消息平台
council gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo council gateway install --system   # 仅 Linux：安装开机启动的系统服务
council gateway start        # 启动默认服务
council gateway stop         # 停止默认服务
council gateway status       # 检查默认服务状态
council gateway status --system         # 仅 Linux：显式检查系统服务
```
...
在 `~/.council/gateway.json` 中配置各平台的覆盖设置：
...
# 其他配对命令：
council pairing list          # 查看待审核和已批准的用户
council pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，Council 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。

如果你觉得繁忙确认消息过多——尤其是使用语音输入或快速连续发送消息时——可设置 `display.busy_ack_enabled: false`。你的输入仍会正常排队/引导/中断，只是聊天回复被静默。

## 工具进度通知

在 `~/.council/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.council/config.yaml` 中的 `display.background_process_notifications` 控制：
...
council gateway start                 # 启动服务
council gateway stop                  # 停止服务
council gateway status                # 检查状态
journalctl --user -u council-gateway -f  # 查看日志
...
除非你确实有此需要，否则避免同时安装用户和系统网关单元。Council 检测到两者同时存在时会发出警告，因为 start/stop/status 行为会变得不明确。

:::info 多个安装
如果你在同一台机器上运行多个 Council 安装（使用不同的 `COUNCIL_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.council` 使用 `council-gateway`；其他安装使用 `council-gateway-<hash>`。`council gateway` 命令会自动针对当前 `COUNCIL_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
council gateway install               # 安装为 launchd agent
council gateway start                 # 启动服务
council gateway stop                  # 停止服务
council gateway status                # 检查状态
tail -f ~/.council/logs/gateway.log   # 查看日志
```

生成的 plist 文件位于 `~/Library/LaunchAgents/ai.council.gateway.plist`。它包含三个环境变量：
...
- **COUNCIL_HOME** — 将网关限定到你的 Council 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `council gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `COUNCIL_HOME` 目录都有自己的 launchd 标签。默认的 `~/.council` 使用 `ai.council.gateway`；其他安装使用 `ai.council.gateway-<suffix>`。
:::
...
| 平台 | 工具集 | 功能 |
|----------|---------|--------------|
| CLI | `council-cli` | 完全访问 |
| Telegram | `council-telegram` | 完整工具，包括终端 |
| Discord | `council-discord` | 完整工具，包括终端 |
| WhatsApp | `council-whatsapp` | 完整工具，包括终端 |
| Slack | `council-slack` | 完整工具，包括终端 |
| Google Chat | `council-google_chat` | 完整工具，包括终端 |
| Signal | `council-signal` | 完整工具，包括终端 |
| SMS | `council-sms` | 完整工具，包括终端 |
| Email | `council-email` | 完整工具，包括终端 |
| Home Assistant | `council-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `council-mattermost` | 完整工具，包括终端 |
| Matrix | `council-matrix` | 完整工具，包括终端 |
| DingTalk | `council-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `council-feishu` | 完整工具，包括终端 |
| WeCom | `council-wecom` | 完整工具，包括终端 |
| WeCom Callback | `council-wecom-callback` | 完整工具，包括终端 |
| Weixin | `council-weixin` | 完整工具，包括终端 |
| BlueBubbles | `council-bluebubbles` | 完整工具，包括终端 |
| QQBot | `council-qqbot` | 完整工具，包括终端 |
| Yuanbao | `council-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `council-teams` | 完整工具，包括终端 |
| API Server | `council-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `council-webhook` | 完整工具，包括终端 |
...
1. **网关日志**（`~/.council/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
