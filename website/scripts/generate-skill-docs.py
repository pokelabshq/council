...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/pokelabshq/council/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    council_meta = (fm.get("metadata") or {}).get("council") or {}
    tags = council_meta.get("tags") or []
    related = council_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `council skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Poke Council"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Council ships with a large built-in skill library copied into `~/.council/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Council also syncs bundled skills on `council update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.council/skills/` tree, it is still shipped with Council; restore it with `council skills reset <name> --restore`.",
        "",
