...
let
  nodejs = nodejs_22;
  councilVenv = callPackage ./python.nix {
    inherit uv2nix pyproject-nix pyproject-build-systems;
    dependency-groups = [ "all" ] ++ extraDependencyGroups;
  };

  councilNpmLib = callPackage ./lib.nix {
    inherit npm-lockfile-fix nodejs;
  };

  councilTui = callPackage ./tui.nix {
    inherit councilNpmLib;
  };

  councilWeb = callPackage ./web.nix {
    inherit councilNpmLib;
  };
  ...
  # to a dev checkout — the loader reads them from COUNCIL_BUNDLED_PLUGINS.
  ...
  checkPackageCollisions = ''
    ...
    core = set()
    venv_sp = pathlib.Path('${councilVenv}/${sitePackagesPath}')
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in council sealed venv', file=sys.stderr)
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${councilVenv}/bin/${name} $out/bin/${name} \
          ...
          ${lib.optionalString (rev != null) ''--set COUNCIL_REVISION ${rev} \''}
    ...
    ${lib.optionalString (extraPythonPackages != [ ]) ''
      ...
      ${councilVenv}/bin/python3 -c "${checkPackageCollisions}"
      echo "=== No collisions ==="
    ''}
  ...
  passthru = {
    ...
    devShellHook = ''
      ...
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        ...
        source .venv/bin/activate
        export COUNCIL_PYTHON=${councilVenv}/bin/python3
      fi
    '';
  };
