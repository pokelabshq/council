...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      ...
      configKeys = pkgs.runCommand "council-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${councilVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "council-cross-eval" { } (
          if failures != [] then
        ...
        cli-commands = pkgs.runCommand "council-cli-commands" { } ''
          set -e
        ...
        # Verify COUNCIL_NODE is set in wrapper and points to Node 20+
        ...
        council-node = pkgs.runCommand "council-node-version" { } ''
          ...
          NODE_MAJOR=$("$COUNCIL_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All COUNCIL_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify COUNCIL_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "council-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(COUNCIL_MANAGED=true "$@" 2>&1 || true)
        ...
        in pkgs.runCommand "council-extra-python-packages" { } ''
          ...
          grep -q "PYTHONPATH" ${councilWithExtra}/bin/council || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${councilWithExtra}/bin/council || \
        ...
        in pkgs.runCommand "council-extra-dependency-groups" { } ''
          ...
          echo "derivation: ${councilWithGroups}"
          echo "venv: ${councilWithGroups.councilVenv}"
        ...
        messaging-variant = pkgs.runCommand "council-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.councilVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "council-config-roundtrip" {
          ...
          merge_and_load() {
            local council_home="$1"
            export COUNCIL_HOME="$council_home"
            ${configMergeScript} ${nixSettings} "$council_home/config.yaml"
            ${councilVenv}/bin/python3 -c '
import json, sys
