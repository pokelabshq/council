...
#
# Container mode: council runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $COUNCIL_HOME/.env
# and read by council at startup — no container recreation needed for env changes.
#
# Tool resolution: the council wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    generatedConfigFile = pkgs.writeText "council-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "council-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'COUNCIL_DOC_EOF'\n${value}\nCOUNCIL_DOC_EOF"
        ) cfg.documents
      )
    );
    ...
    # council user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "council-container-entrypoint" ''
      set -eu

      COUNCIL_UID="''${COUNCIL_UID:?COUNCIL_UID must be set}"
      COUNCIL_GID="''${COUNCIL_GID:?COUNCIL_GID must be set}"

      # ── Group: ensure a group with GID=$COUNCIL_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$COUNCIL_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="council"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$COUNCIL_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$COUNCIL_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$COUNCIL_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$COUNCIL_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="council"
        TARGET_HOME="/home/council"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$COUNCIL_UID" -g "$COUNCIL_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$COUNCIL_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$COUNCIL_UID:$COUNCIL_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure COUNCIL_HOME is owned by the target user.
      ...
      if [ -n "''${COUNCIL_HOME:-}" ] && [ -d "$COUNCIL_HOME" ]; then
        find "$COUNCIL_HOME" \! -user "$COUNCIL_UID" -exec chown "$COUNCIL_UID:$COUNCIL_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/council-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/council-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/council ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/council
        chmod 0440 /etc/sudoers.d/council
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$COUNCIL_UID" --regid="$COUNCIL_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $COUNCIL_HOME/.env.
    ...
    # Default: /var/lib/council/workspace → /data/workspace.
  ...
  in {
    options.services.ai-council = with lib; {
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/council";
        description = "State directory. Contains .council/ subdir (COUNCIL_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Council config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $COUNCIL_HOME/.env at activation time.
          Council reads this file on every startup via load_council_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $COUNCIL_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $COUNCIL_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `council gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the council user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the council plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Council discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the council CLI to environment.systemPackages and export
          COUNCIL_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.council symlink to the service
            stateDir. These users are automatically added to the council group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the council CLI to system PATH and export COUNCIL_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.council/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.COUNCIL_HOME = "${cfg.stateDir}/.council";
      })
      ...
      # Wire extraPackages into the council user's per-user profile so the
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.council        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.council/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.council/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.council/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.council/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.council/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      # ── Activation: link config + auth + documents ────────────────────
      {
        system.activationScripts."ai-council-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          ...
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.council ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.council ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.council -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.council/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.council/$_subdir"
            chmod 2770 "${cfg.stateDir}/.council/$_subdir"
            find "${cfg.stateDir}/.council/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.council/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.council/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.council/config.yaml
            chmod 0640 ${cfg.stateDir}/.council/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.council/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.council/.managed
          chmod 0644 ${cfg.stateDir}/.council/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.council/.container-mode <<'COUNCIL_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    council_bin=${containerDataDir}/current-package/bin/council
    COUNCIL_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.council/.container-mode
            chmod 0644 ${cfg.stateDir}/.council/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.council/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.council";
              in ''
          ...
          # Create ~/.council -> stateDir/.council for each hostUser so the
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.council/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.council/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.council/auth.json
              fi
            ''}
          ''}
          ...
          # Council reads $COUNCIL_HOME/.env at startup via load_council_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.council/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'COUNCIL_NIX_ENV_EOF'
    ${envFileContent}
    COUNCIL_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.council/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.council/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.council/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.ai-council = {
          ...
          environment = {
            HOME = cfg.stateDir;
            COUNCIL_HOME = "${cfg.stateDir}/.council";
            COUNCIL_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $COUNCIL_HOME/.env by the activation script. load_council_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/council"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the council group can read/write them.
            UMask = "0007";
        ...
        systemd.services.ai-council = {
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              COUNCIL_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              COUNCIL_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env COUNCIL_UID="$COUNCIL_UID" \
                --env COUNCIL_GID="$COUNCIL_GID" \
                --env COUNCIL_HOME=${containerDataDir}/.council \
                --env COUNCIL_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/council gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
