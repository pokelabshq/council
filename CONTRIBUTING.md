# Contributing to Poke Council

Thank you for contributing to Poke Council! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Council to work everywhere.
...
Bundled skills (in `skills/`) ship with every Council install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `council skills browse` (labeled "official") and install it with `council skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Poke Labs Discord](https://discord.gg/PokeLabs). Users can install it with `council skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.council/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(council_home, config)` for setup-wizard integration
...
- Integrate with `council memory setup` via `post_setup()` — no need to touch core code
...
echo "OPENROUTER_API_KEY=***" >> ~/.council/.env
```
...
├── toolsets.py               # Tool groupings and presets (council-cli, council-telegram, etc.)
├── council_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── council_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, Poke Portal
...
### User configuration (stored in `~/.council/`)

| Path | Purpose |
|------|---------|
| `~/.council/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.council/.env` | API keys and secrets |
| `~/.council/auth.json` | OAuth credentials (Poke Portal) |
| `~/.council/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.council/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.council/state.db` | SQLite session database |
| `~/.council/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.council/cron/` | Scheduled job data |
| `~/.council/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`council_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.council/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (Poke Portal OAuth, OpenRouter API key, or custom endpoint).
...
(for example `_COUNCIL_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.council`:
...
The user may skip setup and keep loading the skill. Council only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `council setup` or update `~/.council/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Council tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Poke Council" is the secondary collaborator. If the contributor's commit shows "Poke Council" as author because they used Council to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Council tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `council --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Council uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.council/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `council_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `council_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Council runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the council wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `council_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `council_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `council_cli/gateway_windows.py::
...
Council has terminal access. Security matters.
...
2. **Test manually**: Run `council` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/pokelabshq/council/issues)
- Include: OS, Python version, Council version (`council version`), full error traceback
...
- **Discord**: [discord.gg/PokeLabs](https://discord.gg/PokeLabs) — for questions, showcasing projects, and sharing skills
