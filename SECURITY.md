# Poke Council Security Policy

This document describes Poke Council's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/pokelabshq/council/security/advisories/new)
or **security@pokelabs.com**. Do not open public issues for
security vulnerabilities. **Poke Council does not operate a bug
bounty program.**
...
- Environment details (`council version`, commit SHA, OS, Python
  version).
...
Poke Council is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Poke Council,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Poke Council access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Poke Council's documentation
...
Poke Council supports two OS-level isolation postures. They address
...
Poke Council supports this in two ways:

- **Poke Council's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Poke Council's in-process heuristics
...
Poke Council filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Poke Council
itself. Bugs in Poke Council's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Poke Council does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Poke Council's own documentation, or reasonable
  operator expectations would predict — including cases where
  Poke Council has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  council-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Poke Council's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Poke Council. Bugs in Council
...
- Poke Council includes supply-chain guards for MCP server
