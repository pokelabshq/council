...
This module provides the glue:
    - ``CouncilTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Poke Council"
"""
...
from council_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__council_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses COUNCIL_HOME so each profile gets its own OAuth tokens.
    Layout: ``COUNCIL_HOME/mcp-tokens/``
    """
    try:
        from council_constants import get_council_home
        base = Path(get_council_home())
    except ImportError:
        base = Path(os.environ.get("COUNCIL_HOME", str(Path.home() / ".council")))
...
# ---------------------------------------------------------------------------
# CouncilTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class CouncilTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        COUNCIL_HOME/mcp-tokens/<server_name>.json         -- tokens
        COUNCIL_HOME/mcp-tokens/<server_name>.client.json   -- client info
        COUNCIL_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Council records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Council.</p></body></html>"
            ) if code else (
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `council mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Poke Council")
    scope = cfg.get("scope")
