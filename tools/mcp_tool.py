...
transport, discovers their tools, and registers them into the pokelabs-council
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.council/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.council/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            council_home = os.path.expanduser(
                os.getenv(
                    "COUNCIL_HOME", os.path.join(os.path.expanduser("~"), ".council")
                )
            )
            candidates = [
                os.path.join(council_home, "node", "bin", resolved_command),
                ...
                # image (which the Council Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Council MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Council gateways know how to render.
...
class MCPServerTask:
                ...
                if _MCP_NOTIFICATION_TYPES and isinstance(message, ServerNotification):
                    match message.root:
                        case ToolListChangedNotification():
                            ...
                            # request. Refreshing synchropokely inside the SDK
    ...
    async def _refresh_tools(self):
        ...
        After the initial ``await`` (list_tools), all mutations are synchropoke
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'pokelabs-council[mcp]'\n"
                "or (full install):\n"
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Council is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `council mcp login {server_name}` (or delete the tokens "
            f"file under ~/.council/mcp-tokens/ and restart). Do NOT retry "
...
# can be cleaned up asynchropokely by _kill_orphaned_mcp_children().
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Council config file.
    ...
    ``os.environ`` (which includes ``~/.council/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Council' MEDIA: tag convention and out to messaging
            ...
            # Council' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Council's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Council registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``council tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Council registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple council
