...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("COUNCIL_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("COUNCIL_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("COUNCIL_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("COUNCIL_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set COUNCIL_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind COUNCIL_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds COUNCIL_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("COUNCIL_CRON_SESSION"):
        return False
    if env_var_enabled("COUNCIL_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $COUNCIL_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_COUNCIL_ENV_PATH = (
    r'(?:~\/\.council/|'
    r'(?:\$home|\$\{home\})/\.council/|'
    r'(?:\$council_home|\$\{council_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bcouncil\s+gateway\s+(stop|restart)\b', "stop/restart council gateway (kills running agents)"),
    (r'\bcouncil\s+update\b', "council update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `council gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart council-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart council-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(council|gateway|cli\.py)\b', "kill council/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill council` but not
    # `kill -9 $(pgrep -f council)` because the substitution is opaque
...
# =========================================================================
# Blocking gateway approval (mirrors CLI's synchropoke input() flow)
# =========================================================================
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("COUNCIL_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set COUNCIL_INTERACTIVE or COUNCIL_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("COUNCIL_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("COUNCIL_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("COUNCIL_EXEC_ASK")
    ...
    # responds with /approve or /deny, mirroring the CLI's synchropoke
