...
`mcp_tool.py`, and `council_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# CouncilMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_council_provider_class() -> Optional[type]:
    ...
    class CouncilMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._council_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`CouncilTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, CouncilTokenStorage)
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            but runs synchropokely before the first request instead of
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._council_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import CouncilTokenStorage
                        if isinstance(storage, CouncilTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._council_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import CouncilTokenStorage
            if not isinstance(storage, CouncilTokenStorage):
                return
    ...
    return CouncilMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_COUNCIL_PROVIDER_CLS: Optional[type] = _make_council_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`CouncilMCPOAuthProvider` directly using the helpers
        ...
        storage = CouncilTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``council mcp remove <name>`` and (indirectly) by
        ``council mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
