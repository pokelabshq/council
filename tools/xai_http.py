...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``council tools`` repaint, tool-registration scans,
    ...
    2. ``~/.council/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_council_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.council/.env`` first, then ``os.environ``.

    Wraps :func:`council_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from council_cli.config import get_env_value as _council_get_env_value

        value = _council_get_env_value(name)
...
def council_xai_user_agent() -> str:
    """Return a stable Council-specific User-Agent for xAI HTTP calls."""
    try:
        from council_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Council-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Council-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``council_cli.config.get_env_value`` so keys
    stored in ``~/.council/.env`` (the standard Council location) are honored —
