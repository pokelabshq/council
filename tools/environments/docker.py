...
from tools.environments.local import _COUNCIL_PROVIDER_ENV_BLOCKLIST
...
def _load_council_env_vars() -> dict[str, str]:
    """Load ~/.council/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Council profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale council-tagged containers left behind by prior processes.
    ...
    * (optional) ``label=council-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a council process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Council
    process leaves its container behind permanently because there's no
    subsequent Council process scheduled to reuse that exact (task, profile)
    pair.
    """
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``COUNCIL_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("COUNCIL_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using COUNCIL_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'council'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.council/sandboxes/). Non-persistent
        ...
        # container shared across sessions").  If a prior Council process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Council secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _COUNCIL_PROVIDER_ENV_BLOCKLIST)
        council_env = _load_council_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = council_env.get(key)
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``council-*`` but were
        ...
        # container) but can happen if a previous Council process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Council exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Council process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Council startup. That covers the
        ...
        # outlives Council processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"council-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``council /quit``
