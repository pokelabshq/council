...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Council with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.council/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Council
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``council tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``council setup`` for
