...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, poke subscription block, tool-use
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_poke_subscription_prompt``,
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Council never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want COUNCIL_HOME persona while keeping
    ...
    # COUNCIL_KANBAN_TASK env var). Normal chat sessions never see
    ...
    poke_subscription_prompt = _r.build_poke_subscription_prompt(agent.valid_tool_names)
    if poke_subscription_prompt:
        stable_parts.append(poke_subscription_prompt)
    ...
    # Active-profile hint — names the Council profile the agent is running
    # under so it doesn't conflate ~/.council/skills/ (default profile) with
    # ~/.council/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Council profile: default. Other profiles (if any) live "
            "under ~/.council/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Council profile: {active_profile}. This session reads "
            f"and writes ~/.council/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.council/skills/, ~/.council/plugins/, "
            f"~/.council/cron/, ~/.council/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Council never rebuilds or reinjects parts of it
