...
from council_constants import get_council_home, get_skills_dir, is_wsl
...
_COUNCIL_MD_NAMES = (".council.md", "COUNCIL.md")


def _find_council_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.council.md`` or ``COUNCIL.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Poke Council, an intelligent AI assistant created by Poke Labs. "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.council/kanban.db`. Your task id is in "
    "`$COUNCIL_KANBAN_TASK`; your workspace is `$COUNCIL_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $COUNCIL_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `council kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Council WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Council itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Council runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Council itself is running. The host OS, home, and cwd "
                f"of the Council process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Council "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Council
    ...
    extra = (os.getenv("COUNCIL_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_council_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.council/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("COUNCIL_PLATFORM")
        or get_session_env("COUNCIL_SESSION_PLATFORM")
        or ""
    )
...
def build_poke_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact Poke subscription capability block for the system prompt."""
    try:
        ...
        logger.debug("Failed to import Poke subscription helper: %s", exc)
        return ""
    ...
    features = get_poke_subscription_features()

    def _status_line(feature) -> str:
        if feature.managed_by_poke:
            return f"- {feature.label}: active via Poke subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.poke_auth_present:
            return f"- {feature.label}: included with Poke subscription, not currently selected"
        if feature.key == "modal" and features.poke_auth_present:
            return f"- {feature.label}: optional via Poke subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# Poke Subscription",
        "Poke subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a Poke-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that Poke subscription would unlock or simplify, suggest Poke subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: council setup, council setup tools, council setup terminal, council status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from COUNCIL_HOME and return its content, or None.
    ...
    """
    try:
        from council_cli.config import ensure_council_home
        ensure_council_home()
    except Exception as e:
        logger.debug("Could not ensure COUNCIL_HOME before loading SOUL.md: %s", e)

    soul_path = get_council_home() / "SOUL.md"
...
def _load_council_md(cwd_path: Path) -> str:
    """.council.md / COUNCIL.md — walk to git root."""
    council_md_path = _find_council_md(cwd_path)
    if not council_md_path:
        return ""
    try:
        content = council_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = council_md_path.name
        try:
            rel = str(council_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".council.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", council_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .council.md / COUNCIL.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from COUNCIL_HOME is independent and always included when present.
    ...
    # SOUL.md from COUNCIL_HOME only — skip when already loaded as identity
    if not skip_soul:
