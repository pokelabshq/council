...
def _council_home_path() -> Path:
    """Resolve the active COUNCIL_HOME (profile-aware) without circular imports."""
    try:
        from council_constants import get_council_home  # local import to avoid cycles
        return get_council_home()
    except Exception:
        return Path(os.path.expanduser("~/.council"))


def _council_root_path() -> Path:
    """Resolve the Council root dir (always the parent of any profile, never per-profile)."""
    try:
        from council_constants import get_default_council_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    council_home = _council_home_path()
    council_root = _council_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(council_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(council_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(council_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(council_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved COUNCIL_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("COUNCIL_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Council control-plane files: block both the ACTIVE profile's view
    # (council_home) AND the global root view. Without the root pass, a
    ...
    council_dirs = []
    for base in (_council_home_path(), _council_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in council_dirs:
                council_dirs.append(real)
        except Exception:
            continue

    for base_real in council_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Council path.

    Three categories are blocked:

      * Internal Council cache files under ``COUNCIL_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under COUNCIL_HOME and the global Council
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.council/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active COUNCIL_HOME (profile-aware) AND the global
    # Council root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (COUNCIL_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    council_dirs: list[Path] = []
    for base in (_council_home_path(), _council_root_path()):
        try:
            real = base.resolve()
            if real not in council_dirs:
    ...
    for hd in council_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Council credential store "
    ...
    for hd in council_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Council MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Council MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Council profiles are separate COUNCIL_HOME dirs under
...
#
# Reference: May 2026 incident where a council-security profile session
# edited skills under both ``~/.council/profiles/council-security/skills/``
# AND ``~/.council/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under COUNCIL_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from COUNCIL_HOME.

    ``~/.council``              -> ``"default"``
    ``~/.council/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _council_home_path().resolve()
        root_real = _council_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Council scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Council entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Council profile {info['target_profile']!r}, but the "
