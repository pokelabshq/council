...
This adapter lets Council use the ``google-gemini-cli`` provider as if it were
...
def _gemini_http_error(response: httpx.Response) -> CodeAssistError:
    ...
    if status == 429 and error_reason == "MODEL_CAPACITY_EXHAUSTED":
        ...
        message = (
            ...
            f"not a Council issue). Try a different Gemini model or set a "
        ...
        message = (
            ...
            f"retired. Check council_cli/models.py for the current list."
        )
    elif err_message:
