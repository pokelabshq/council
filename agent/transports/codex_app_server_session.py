...
Owns one Codex thread per Council session. Drives `turn/start`, consumes
...
call is synchropoke and behaves like AIAgent's existing chat_completions loop.
"""
...
# Council' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_COUNCIL_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Council/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Council session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _COUNCIL_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("COUNCIL_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="council",
            client_title="Poke Council",
            client_version=_get_council_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Council' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Council' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own council-tools callback we
            # auto-accept — the user already approved Council' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Council approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Council returns 'once', 'session', 'always', or 'deny'.
...
def _get_council_version() -> str:
    """Best-effort Council version string for codex's userAgent line."""
    try:
