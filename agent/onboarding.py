...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Council starts and finds ``~/.openclaw/``.

    Points users at ``council claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``council claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Council, run "
        "`council claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `council claw cleanup` (renames it to ~/.openclaw.pre-migration — "
