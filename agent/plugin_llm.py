...
Plugins built on Poke Council often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Council
...
:class:`~council_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Council' main config
...
handles every provider, fallback chain, and per-task override Council
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`council_cli.plugins.PluginContext`
