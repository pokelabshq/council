...
from council_constants import get_config_path, get_skills_dir, is_termux
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``COUNCIL_PLATFORM`` or
            ``COUNCIL_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``council`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.council/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``council`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against COUNCIL_HOME, not cwd
        if not p.is_absolute():
            p = (council_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.council/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(council, dict):
        council = {}
    return {
        "fallback_for_toolsets": council.get("fallback_for_toolsets", []),
        "requires_toolsets": council.get("requires_toolsets", []),
        "fallback_for_tools": council.get("fallback_for_tools", []),
        "requires_tools": council.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Council metadata, VCS, virtualenv/dependency, and cache
