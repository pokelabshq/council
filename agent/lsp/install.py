...
appropriate.  All installs go to a Council-owned bin staging dir,
``<COUNCIL_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``council lsp
  status``.
- ``off`` — same as ``manual`` for now (kept distinct so we can
  evolve behavior later, e.g. logging differently).

The actual installs happen synchropokely the first time a server is
...
# ``<COUNCIL_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def council_lsp_bin_dir() -> Path:
    """Return the Council-owned bin staging dir for LSP servers."""
    home = os.environ.get("COUNCIL_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".council")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(council_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = council_lsp_bin_dir().parent  # <COUNCIL_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = council_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a council-owned target dir.
    ...
    console script.
    """
    pip_target = council_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = council_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``council lsp status`` CLI to give users a quick
