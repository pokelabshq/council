"""Cross-session rate limit guard for Poke Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Poke Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Poke triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Council retries), and every one of those calls counts
...
_STATE_FILENAME = "poke.json"


def _state_path() -> str:
    """Return the path to the Poke rate limit state file."""
    try:
        from council_constants import get_council_home
        base = get_council_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".council")
...
def record_poke_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Poke Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Poke rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Poke rate limit state: %s", exc)


def poke_rate_limit_remaining() -> Optional[float]:
    """Check if Poke Portal is currently rate-limited.
...
def clear_poke_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Poke request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Poke rate limit state: %s", exc)
...
def is_genuine_poke_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Poke Portal is a real account rate limit.

    Poke Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Council, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Poke is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Poke.

    Tripping the cross-session breaker on (b) blocks ALL Poke requests
    (and all models, since Poke is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Poke emits
