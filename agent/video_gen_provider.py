...
as ``kind: backend``) or ``~/.council/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``council tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$COUNCIL_HOME/cache/videos/``, creating parents as needed."""
    from council_constants import get_council_home

    path = get_council_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$COUNCIL_HOME/cache/videos/``.
