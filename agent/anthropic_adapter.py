"""Anthropic Messages API adapter for Poke Council.

Translates between Council's internal OpenAI-style message format and
...
# Council effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Council historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Council)
    ...
    # 1. Council-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Council configs.
...
# ── Council-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.council/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_COUNCIL_OAUTH_FILE = get_council_home() / ".anthropic_oauth.json"
...
def run_council_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Council-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Council with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from council_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_council_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Council-managed OAuth credentials from ~/.council/.anthropic_oauth.json."""
    if _COUNCIL_OAUTH_FILE.exists():
        try:
            data = json.loads(_COUNCIL_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Council OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    # silently hides reasoning text that Council surfaces in its CLI. We
