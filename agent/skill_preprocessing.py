...
# Matches ${COUNCIL_SKILL_DIR} / ${COUNCIL_SESSION_ID} tokens in SKILL.md.
# Tokens that don't resolve (e.g. ${COUNCIL_SESSION_ID} with no session) are
# left as-is so the user can debug them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(COUNCIL_SKILL_DIR|COUNCIL_SESSION_ID)\}")
...
def substitute_template_vars(
    ...
    """Replace ${COUNCIL_SKILL_DIR} / ${COUNCIL_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "COUNCIL_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "COUNCIL_SESSION_ID" and session_id:
