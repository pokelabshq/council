...
Storage
-------
Bundles live in ``~/.council/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``COUNCIL_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``council bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under COUNCIL_HOME.

    Honors ``COUNCIL_BUNDLES_DIR`` for tests; falls back to
    ``<COUNCIL_HOME>/skill-bundles``.
    """
    override = os.environ.get("COUNCIL_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_council_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `council bundles` CLI subcommand.
# ---------------------------------------------------------------------------
